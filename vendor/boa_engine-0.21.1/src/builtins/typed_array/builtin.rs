...
impl BuiltinTypedArray {
    ...
    pub(crate) fn slice(
        ...
        // g. If srcType is targetType, then
        {
            ...
            if BufferObject::equals(
                ...
                let src_start = min(src_byte_index, src_buf.len());
                let available_byte_count = src_buf.len().saturating_sub(src_start);
                let copy_byte_count = min(byte_count, available_byte_count);
                let src = src_buf.subslice(src_start..);
                let mut target = target_buf.subslice_mut(target_byte_index..);

                #[cfg(debug_assertions)]
                {
                    assert!(src.len() >= copy_byte_count);
                    assert!(target.len() >= byte_count);
                }
                ...
                if copy_byte_count > 0 {
                    unsafe {
                        memcpy(src.as_ptr(), target.as_ptr(), copy_byte_count);
                    }
                }
            }
        }
    ...
    pub(crate) fn sort(
        ...
        for (j, item) in sorted.into_iter().enumerate() {
            ...
            if let Err(error) = ta.set(j, item, true, context) {
                let detached_during_compare = compare_fn.is_some()
                    && error
                        .as_native()
                        .is_some_and(|native| native.message() == "ArrayBuffer was already detached");
                if detached_during_compare {
                    break;
                }
                return Err(error);
            }
