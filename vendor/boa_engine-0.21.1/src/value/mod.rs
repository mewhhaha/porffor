...
#[cfg(feature = "float16")]
pub(crate) fn round_to_f16(value: f64) -> float16::f16 {
    const F16_SIGN_MASK: u16 = 0x8000;
    const F16_EXP_MASK: u16 = 0x7C00;
    const F16_QNAN: u16 = 0x7E00;
    const F16_MIN_NORMAL_EXP: i32 = -14;
    const F16_MIN_SUBNORMAL_EXP: i32 = -24;
    const F16_MAX_EXP: i32 = 15;
    const F64_EXP_BIAS: i32 = 1023;
    const F64_FRAC_BITS: u32 = 52;
    const F16_FRAC_BITS: u32 = 10;

    fn round_ties_to_even(significand: u64, shift: u32) -> u64 {
        if shift == 0 {
            return significand;
        }
    ...
    let sign = ((bits >> 48) as u16) & F16_SIGN_MASK;
    let exponent = ((bits >> F64_FRAC_BITS) & 0x7FF) as i32;
    let fraction = bits & ((1u64 << F64_FRAC_BITS) - 1);
    ...
    if exponent == 0x7FF {
        ...
        let payload = ((fraction >> (F64_FRAC_BITS - F16_FRAC_BITS)) as u16) & 0x03FF;
        return float16::f16::from_bits(sign | F16_QNAN | payload);
    }
    ...
    let unbiased_exponent = exponent - F64_EXP_BIAS;
    let significand = (1u64 << F64_FRAC_BITS) | fraction;

    if unbiased_exponent > F16_MAX_EXP {
        return float16::f16::from_bits(sign | F16_EXP_MASK);
    }

    if unbiased_exponent < F16_MIN_NORMAL_EXP {
        if unbiased_exponent < F16_MIN_SUBNORMAL_EXP - 1 {
            return float16::f16::from_bits(sign);
        }

        let shift = (28 - unbiased_exponent) as u32;
        let rounded = round_ties_to_even(significand, shift);
        if rounded == 0 {
            return float16::f16::from_bits(sign);
        }
        if rounded >= 1 << F16_FRAC_BITS {
            return float16::f16::from_bits(sign | (1 << F16_FRAC_BITS));
        }
        return float16::f16::from_bits(sign | rounded as u16);
    }

    let mut exponent_bits = (unbiased_exponent + 15) as u16;
    let rounded = round_ties_to_even(significand, F64_FRAC_BITS - F16_FRAC_BITS);

    if rounded >= 1 << (F16_FRAC_BITS + 1) {
        exponent_bits += 1;
        if exponent_bits >= 0x1F {
            return float16::f16::from_bits(sign | F16_EXP_MASK);
        }
        return float16::f16::from_bits(sign | (exponent_bits << F16_FRAC_BITS));
    }

    let fraction_bits = (rounded as u16) & ((1 << F16_FRAC_BITS) - 1);
    float16::f16::from_bits(sign | (exponent_bits << F16_FRAC_BITS) | fraction_bits)
}
...
impl JsValue {
    ...
    pub fn to_f16(&self, context: &mut Context) -> JsResult<float16::f16> {
        self.to_number(context).map(round_to_f16)
    }
