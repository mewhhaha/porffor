...
/// Build a property escape set, preserving ECMAScript's case-folding order for negated escapes.
fn codepoints_from_property_intervals(
    intervals: &'static [Interval],
    negate: bool,
    icase: bool,
) -> CodePointSet {
    let cps = CodePointSet::from_sorted_disjoint_intervals(intervals.to_vec());
    match (negate, icase) {
        (false, false) => cps,
        (true, false) => cps.inverted(),
        (false, true) => unicode::add_icase_code_points(cps),
        (true, true) => unicode::add_icase_code_points(cps.inverted()),
    }
}
                    ...
                    match to_char_sat(c) {
                        ...
                        'b' => {
                            ...
                            result.push(ir::Node::WordBoundary {
                                invert: false,
                                unicode_icase: self.flags.icase
                    ...
                    'p' | 'P' if self.flags.unicode => {
                        ...
                        match self.try_consume_unicode_property_escape()? {
                            PropertyEscapeKind::CharacterClass(s) => Ok(Some(ClassAtom::Range {
                                iv: codepoints_from_property_intervals(s, negate, self.flags.icase),
                                negate: false,
                            })),
                    ...
                    0x70 /* p */ => {
                        ...
                        match self.try_consume_unicode_property_escape()? {
                            PropertyEscapeKind::CharacterClass(intervals) => Ok(
                                CharacterClassEscape(codepoints_from_property_intervals(
                                    intervals,
                                    false,
                                    self.flags.icase,
                                )),
                    ...
                    0x50 /* P */ => {
                        ...
                        match self.try_consume_unicode_property_escape()? {
                            PropertyEscapeKind::CharacterClass(s) => Ok(CharacterClassEscape(
                                codepoints_from_property_intervals(s, true, self.flags.icase),
                            )),
        ...
        match to_char_sat(c) {
            ...
            'p' | 'P' if self.flags.unicode || self.flags.unicode_sets => {
                ...
                match property_escape {
                    PropertyEscapeKind::CharacterClass(s) => Ok(ir::Node::Bracket(BracketContents {
                        invert: false,
                        cps: codepoints_from_property_intervals(s, negate, self.flags.icase),
                    })),
