...
use super::{parse_ixdtf, validate_fractional_second_precision, ParseVariant};
...
pub(crate) fn parse_allowed_timezone_formats(
    ...
    {
        (offset, annotation)
    } else if let Ok(record) = IxdtfParser::from_str(s).parse_time() {
        if validate_fractional_second_precision(&record).is_err() {
            return None;
        }
        (record.offset, record.tz)
