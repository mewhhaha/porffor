...
impl TimeDuration {
    ...
    pub(crate) fn total(&self, unit: Unit) -> TemporalResult<FiniteF64> {
        ...
        exact_ratio_to_f64(self.0, unit_nanoseconds.get() as i128)
    }
...
const EXACT_RATIO_FRACTIONAL_DIGITS: usize = 80;

fn exact_ratio_to_decimal_string(numerator: i128, denominator: i128) -> TemporalResult<String> {
    ...
    decimal.push_str(&integer.to_string());
    ...
    decimal.push('.');
    for _ in 0..EXACT_RATIO_FRACTIONAL_DIGITS {
        remainder *= 10;
...
fn exact_ratio_to_f64(numerator: i128, denominator: i128) -> TemporalResult<FiniteF64> {
    let decimal = exact_ratio_to_decimal_string(numerator, denominator)?;
    let parsed = decimal.parse::<f64>().map_err(|_| {
        TemporalError::assert().with_message("failed to parse exact ratio as f64")
    })?;
    FiniteF64::try_from(parsed)
}

fn round_exact_ratio_to_increment(
    numerator: i128,
    denominator: i128,
    increment: RoundingIncrement,
    mode: RoundingMode,
    ...
    let scaled_divisor = denominator
        .unsigned_abs()
        .checked_mul(increment.as_extended_increment().get())
        .ok_or_else(|| {
            TemporalError::range().with_message("exact ratio divisor exceeded valid range.")
        })?;
    let scaled_divisor = NonZeroU128::new(scaled_divisor).temporal_unwrap()?;
    let rounded_scaled = IncrementRounder::<i128>::from_signed_num(numerator, scaled_divisor)?
        .round(mode);

    Ok(rounded_scaled / denominator)
}
...
impl InternalDurationRecord {
    ...
    fn nudge_calendar_unit(
        ...
        let rounded_unit = round_exact_ratio_to_increment(
            total_numerator,
            total_denominator,
            options.increment,
    ...
    fn nudge_to_day_or_time(
        ...
        if time_duration.sign() == Sign::Zero {
            return Ok(NudgeRecord {
                normalized: *self,
                total: None,
                nudge_epoch_ns: dest_epoch_ns,
                expanded: false,
            });
        }
