...
impl Duration {
    fn collapse_unsafe_slot(value: i128) -> TemporalResult<i128> {
        Ok(FiniteF64::try_from(value)?.as_inner() as i128)
    }
    ...
    pub(crate) fn from_internal(
        ...
        match largest_unit {
            ...
            Unit::Millisecond => {
                ...
                // d. Set microseconds to microseconds modulo 1000.
                (milliseconds, microseconds) = microseconds.div_rem_euclid(&1_000);
                milliseconds = Self::collapse_unsafe_slot(milliseconds)?;
            }
            ...
            Unit::Microsecond => {
                ...
                // b. Set nanoseconds to nanoseconds modulo 1000.
                (microseconds, nanoseconds) = nanoseconds.div_rem_euclid(&1_000);
                microseconds = Self::collapse_unsafe_slot(microseconds)?;
            }
            // 10. Else,
            Unit::Nanosecond => {
                nanoseconds = Self::collapse_unsafe_slot(nanoseconds)?;
            }
            _ => temporal_assert!(false, "unexpected largest unit in Duration::from_internal"),
        }
...
pub fn duration_to_formattable(
    ...
    let has_time = precision != Precision::Auto
        || date.is_none()
        || hours != 0
        || minutes != 0
        || seconds != 0
        || subseconds != 0;
    let time = has_time.then_some(FormattableTimeDuration::Seconds(
        hours as u64,
        minutes as u64,
        seconds,
        Some(subseconds),
