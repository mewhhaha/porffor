...
use super::{
    duration::normalized::InternalDurationRecord, Duration, PlainDate, PlainDateTime,
};
...
impl PlainYearMonth {
    ...
    pub(crate) fn add_duration(
        ...
        let date = if !calendar.is_iso() {
            ...
            //
            // Computing the last day of the current month directly avoids constructing the
            // out-of-range `nextMonth` date at the representable upper boundary.
            let date = (1..=intermediate_date.days_in_month() as u8)
                .rev()
                .find_map(|day| {
                    calendar
                        .date_from_fields(fields.clone().with_day(day), Overflow::Constrain)
                        .ok()
                })
                .ok_or_else(|| {
                    TemporalError::range().with_message(
                        "Unable to determine the last representable day in the month.",
                    )
                })?;

            // d. Assert: ISODateWithinLimits(date) is true.
            temporal_assert!(date.iso.is_valid());
        ...
        let added_date = calendar.date_add(
            &date,
            &duration_to_add,
            if calendar.is_iso() {
                Overflow::Constrain
