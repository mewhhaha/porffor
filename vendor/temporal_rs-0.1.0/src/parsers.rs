...
fn fractional_time_precision_error() -> TemporalError {
    TemporalError::range().with_enum(ErrorMessage::FractionalTimeMoreThanNineDigits)
}

fn validate_fractional_second_precision(record: &IxdtfParseRecord<'_, Utf8>) -> TemporalResult<()> {
    if let Some(fraction) = record.time.and_then(|time| time.fraction) {
        fraction
            .to_nanoseconds()
            .ok_or_else(fractional_time_precision_error)?;
    }

    if let Some(UtcOffsetRecordOrZ::Offset(offset)) = record.offset {
        if let Some(fraction) = offset.fraction() {
            fraction
                .to_nanoseconds()
                .ok_or_else(fractional_time_precision_error)?;
        }
...
fn parse_ixdtf(source: &[u8], variant: ParseVariant) -> TemporalResult<IxdtfParseRecord<'_, Utf8>> {
    ...
    validate_fractional_second_precision(&record)?;
...
pub fn parse_allowed_calendar_formats(s: &[u8]) -> Option<&[u8]> {
    ...
    } else if let Ok(r) = IxdtfParser::from_utf8(s).parse_time() {
        if validate_fractional_second_precision(&r).is_ok() {
            return Some(r.calendar.unwrap_or(&[]));
        }
