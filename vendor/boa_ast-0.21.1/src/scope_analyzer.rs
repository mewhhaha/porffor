...
impl<'ast> VisitorMut<'ast> for BindingEscapeAnalyzer<'_> {
    ...
    fn visit_export_declaration_mut(
        ...
        match node {
            ExportDeclaration::ReExport { request, kind } => {
                request.visit_with_mut(self)?;
...
impl<'ast> VisitorMut<'ast> for BindingCollectorVisitor<'_> {
    ...
    fn visit_declaration_mut(
        &mut self,
        node: &'ast mut Declaration,
    ) -> ControlFlow<Self::BreakTy> {
        if let Declaration::Lexical(
            LexicalDeclaration::Using(_) | LexicalDeclaration::AwaitUsing(_),
        ) = node
        {
            self.scope.set_needs_environment();
        }
    ...
    fn visit_for_loop_mut(&mut self, node: &'ast mut ForLoop) -> ControlFlow<Self::BreakTy> {
        let scope = match &mut node.inner.init {
            Some(ForLoopInitializer::Lexical(decl)) => {
                ...
                if decl.declaration.is_using() || decl.declaration.is_await_using() {
