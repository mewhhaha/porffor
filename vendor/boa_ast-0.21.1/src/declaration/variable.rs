...
pub enum LexicalDeclaration {
    ...
    /// A `using` declaration creates an immutable lexical binding whose value is disposed when the
    /// owning scope completes.
    Using(VariableList),

    /// An `await using` declaration creates an immutable lexical binding whose value is disposed
    /// asynchronously when the owning scope completes.
...
impl LexicalDeclaration {
    ...
    /// Returns `true` if the declaration is a `using` or `await using` declaration.
    #[must_use]
    pub const fn is_using(&self) -> bool {
        matches!(self, Self::Using(_) | Self::AwaitUsing(_))
    }

    /// Returns `true` if the declaration is an `await using` declaration.
    #[must_use]
    pub const fn is_await_using(&self) -> bool {
        matches!(self, Self::AwaitUsing(_))
    }
}
...
impl VisitWith for LexicalDeclaration {
        ...
        V: Visitor<'a>,
    {
        match self {
            ...
            | Self::Let(vars)
            | Self::Using(vars)
            | Self::AwaitUsing(vars) => visitor.visit_variable_list(vars),
        }
    }
        ...
        V: VisitorMut<'a>,
    {
        match self {
            ...
            | Self::Let(vars)
            | Self::Using(vars)
            | Self::AwaitUsing(vars) => visitor.visit_variable_list_mut(vars),
        }
    }
}
