...
use crate::{
    ...
    parser::{
        ...
        statement::{
            Statement,
            declaration::{LexicalDeclaration, allowed_token_after_let, using_declaration_kind},
    ...
    R: ReadChar,
{
    ...
    fn parse(self, cursor: &mut Cursor<R>, interner: &mut Interner) -> ParseResult<Self::Output> {
        ...
        let init = match cursor.peek(0, interner).or_abrupt()?.kind().clone() {
                ...
                _ => Some(ParsedForInitializer::Regular(
                    Expression::new(false, self.allow_yield, self.allow_await)
                        .parse(cursor, interner)?
                        .into(),
                )),
        ...
        let init = match (init, token.kind()) {
            ...
            (
                Some(ParsedForInitializer::Regular(init)),
                ...
                let init =
                    initializer_to_iterable_loop_initializer(init, position, cursor.strict(), in_loop)?;
    ...
    R: ReadChar,
{
    ...
    let expr = if in_loop {
        Expression::new(true, allow_yield, allow_await).parse(cursor, interner)?
    } else {
        AssignmentExpression::new(true, allow_yield, allow_await).parse(cursor, interner)?
    };

    cursor.expect(Punctuator::CloseParen, "for in/of statement", interner)?;

    let body_position = cursor.peek(0, interner).or_abrupt()?.span().start();
    let body = Statement::new(allow_yield, allow_await, allow_return).parse(cursor, interner)?;

    if body.is_labelled_function() {
        return Err(Error::wrong_labelled_function_declaration(body_position));
    }
