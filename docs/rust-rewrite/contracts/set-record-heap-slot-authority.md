# Set-record heap-slot identity authority

## Closed layout identities

The passive Set record layout contains exactly four capability-free
`SetRecordHeapSlot` identities, in entries-pointer, entries-length,
...
words at offsets 8, 16 and 24. An arbitrary row cannot omit the entries edge or
trace collection accounting state.
...
that no second Rust source constructs free-form Set record rows. The bounded
...
change a linear-memory offset, allocation, emitted Wasm, Set behavior or
collector execution.

```sh
cargo test -p lila-aot-wasm --test set_record_heap_slot_structure
cargo test -p lila-aot-wasm --lib heap::tests::set_record_heap_slot_identities_own_layout_metadata -- --exact --test-threads=1
cargo test -p lila-aot-wasm --lib heap::tests::heap_layout_registry_ -- --test-threads=1
rustfmt --check crates/lila-aot-wasm/src/heap_set_record_layout.rs crates/lila-aot-wasm/src/heap.rs crates/lila-aot-wasm/tests/set_record_heap_slot_structure.rs
git diff --check
```

The canonical structure regression passes `4/4`, the exact owner witness
