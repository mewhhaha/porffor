# AsyncDisposableStack-entry heap-slot identity authority

## Closed layout identities

The passive AsyncDisposableStack entry contains exactly five capability-free
`AsyncDisposableStackEntryHeapSlot` identities in kind, value-tag,
value-payload, method-tag and method-payload order:
...
that no second Rust source constructs free-form AsyncDisposableStack entry
...
retention, await completion, emitted Wasm or collector execution.

```sh
cargo test -p lila-aot-wasm --test async_disposable_stack_entry_heap_slot_structure
cargo test -p lila-aot-wasm --lib heap::tests::async_disposable_stack_entry_heap_slot_identities_own_layout_metadata -- --exact --test-threads=1
cargo test -p lila-aot-wasm --lib heap::tests::heap_layout_registry_ -- --test-threads=1
rustfmt --check crates/lila-aot-wasm/src/heap_async_disposable_stack_entry_layout.rs crates/lila-aot-wasm/src/heap.rs crates/lila-aot-wasm/tests/async_disposable_stack_entry_heap_slot_structure.rs
git diff --check
```
...
Batch Y checkpoint, the recursive structure target passes `4/4`, the exact heap
...
checks are green. No semantic golden or Test262 rerun applies to this passive
