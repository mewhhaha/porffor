...
valid parenthesized async-function-expression dependency remains a parsed graph
node.
...
AsyncFunctionDeclaration, generator-expression and async-generator-expression
sources retain their separate typed codes. GeneratorDeclaration also has a
separate typed code; AsyncGeneratorDeclaration also has a distinct typed code,
while method producers remain `ParseCode::Malformed`.
