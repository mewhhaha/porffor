...
both immediate and heap formatters consume that same prepared local. The stage
...
manual local-lifetime review. `scripts/check-module-boundaries.sh` also pins the
closed BigInt result protocol, the three-state builder Realm source, the two
helper ABI consumers, `new_main`'s exact `GlobalFallback` projection, and the
...
Cargo, focused fixture execution, the pinned BigInt prototype trees, the
ECMA-402 BigInt tree and the broad batch ladder are deferred until independent
review and the repository baseline release them.
