...
let typedOddBuffer = new ArrayBuffer(4, { maxByteLength: 4 });
let typedOddView = new Uint16Array(typedOddBuffer);
typedOddView[1] = 0x5678;
let typedOddValue = typedOddView.at({
  valueOf: function () {
    typedOddBuffer.resize(3);
    return 1;
  }
});

let arrayOddBuffer = new ArrayBuffer(4, { maxByteLength: 4 });
let arrayOddView = new Uint16Array(arrayOddBuffer);
arrayOddView[1] = 0x5678;
let arrayOddValue = Array.prototype.at.call(arrayOddView, {
  valueOf: function () {
    arrayOddBuffer.resize(3);
    return 1;
  }
});

let fixedAtBuffer = new ArrayBuffer(4, { maxByteLength: 4 });
let fixedAtView = new Uint8Array(fixedAtBuffer, 0, 4);
fixedAtBuffer.resize(2);
let fixedAtOutOfBoundsThrows = false;
try {
  fixedAtView.at(0);
} catch (error) {
  fixedAtOutOfBoundsThrows = error instanceof TypeError;
}
fixedAtBuffer.resize(4);
fixedAtView[2] = 11;
let fixedAtRegrowValue = fixedAtView.at(2);
...
inheritedValue === "inherited"
  ...
  && typedOddValue === undefined
  && arrayOddValue === undefined
  && fixedAtOutOfBoundsThrows
  && fixedAtRegrowValue === 11;
