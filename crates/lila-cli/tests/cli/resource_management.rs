...
#[test]
fn wasm_await_using_for_of_lifecycle() {
    let output = Command::new(env!("CARGO_BIN_EXE_lila"))
        .arg("run")
        .arg("--execution-backend")
        .arg("wasm")
        .arg(fixture_path("wasm_await_using_for_of_lifecycle.js"))
        .output()
        .expect("run command should run");
    ...
    assert!(stdout.contains("backend_used: WasmAot"), "{stdout}");
    assert!(stdout.contains("await-using-for-of:true"), "{stdout}");
    assert!(
        !stdout.contains("await-using-for-of:FAILED")
            && !stdout.contains("uncaught throw")
            && !stderr.contains("uncaught throw"),
        "stdout: {stdout}\nstderr: {stderr}"
    );
