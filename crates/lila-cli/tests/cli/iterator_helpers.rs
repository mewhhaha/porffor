...
//! nullish `possible_kinds` (`lila inspect` on `class D extends Iterator {}
...
//! attribution puts a number on it: `lila::main` is 69 bytes larger for
...
//! `lila-ir` and is recorded as an exact patch in
...
/// (`lila-aot-wasm/src/functions.rs`) does not resolve `"some"` on a class
