...
//! against `crates/lila-cli/tests/known-failures.txt`". That file never
...
//! - 3 more in `tests/perf.rs`, 1 in `tests/async_generator.rs`, and 1 in
//!   `tests/cache.rs`. Do not
...
//!   over `crates/lila-cli/tests/cli/*.rs crates/lila-cli/tests/*.rs`.
...
//!   crates/lila-cli/tests/cli/*.rs crates/lila-cli/tests/*.rs
//! grep -rn '#\[ignore' crates/lila-cli/tests/
//! grep -rn '#\[should_panic' crates/lila-cli/tests/
...
impl fmt::Display for LedgerError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        let file = "crates/lila-cli/tests/known-failures.tsv";
        match self {
...
// -------------------------------------------------------------------------
// Routing: which `lila` invocations must run as a guarded subprocess.
// -------------------------------------------------------------------------

/// How `main.rs`'s `Command::output` should execute a `lila` invocation.
///
/// Closed domain, matched exhaustively at its single call site.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum ExecutionPath {
    /// The fast path: call `lila_cli::run_cli_capture` in this process. No
    /// process spawn.
    ///
    ...
    InProcess,
    /// Spawn the real `lila` binary, poll it, and kill it after the hang
...
/// Decide how to run a `lila` invocation, given the current thread's name.
///
...
///
/// - Under `--test-threads=1` every CLI test spawns a real `lila` process,
...
fn repo_root() -> PathBuf {
    manifest_dir()
        ...
        .expect("CARGO_MANIFEST_DIR is crates/lila-cli, so it has two ancestors")
        .to_path_buf()
}
...
fn rung_1c_chunks_cover_every_cli_area_module() {
    ...
    for module in &modules {
        assert!(
            ...
            "crates/lila-cli/tests/cli/main.rs does not declare `mod {module};`, so \
    ...
    for module in &declared {
        assert!(
            ...
            "crates/lila-cli/tests/cli/main.rs declares `mod {module};` but there is no \
