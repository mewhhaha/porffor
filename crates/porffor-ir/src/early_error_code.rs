...
//! parse table in `porffor-front` to it.
//!
//! The domain itself, the fragment table and the classifier live one crate down,
//! in `porffor_front::early_error_code`, because both `porffor_front::parse`
//! (entry files) and `porffor_ir::modules::record::reparse_module` (dependency
//! modules) must name them and `porffor-ir` depends on `porffor-front`. This
//! module holds only what needs a `porffor-ir` type. There is no second copy of
...
/// the module-goal fragments in `porffor-front` stay byte-identical to the ones
/// in `porffor-ir`. It catches both halves of the mistake: a boa parse failure
