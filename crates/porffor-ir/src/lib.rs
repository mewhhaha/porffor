...
const MAX_STATIC_ARRAY_SHAPE_INDEX: usize = 1_000_000;
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct PrivateElementBinding {
    private_name_id: PrivateNameId,
    placement: ClassMethodPlacementIr,
    storage: PrivateStorageKind,
}

#[derive(Debug, Clone, Default)]
struct ClassLoweringContext {
    name: Option<String>,
...
#[derive(Debug, Clone)]
struct GeneratedFunctionOutput {
    function_id: FunctionId,
    return_info: ValueInfo,
    this_info: ValueInfo,
    construct_this_info: Option<ValueInfo>,
}
...
struct FunctionSignature {
    ...
    constructor_instance: ValueInfo,
    this_info: ValueInfo,
    this_observed: bool,
}
                    ...
                    capture.slot = *owner
                        ...
                        .get(
                            self.function_plans[&function_id]
                                ...
                                .iter()
                                .find(|(_, plan)| {
                                    plan.owner_id == capture.owner_id && plan.slot == 0
                                })
                                .map(|(name, _)| name)
...
fn generator_expression_callee(expression: &Expression) -> Option<&GeneratorExpression> {
    match expression {
...
fn class_method_key(method: &ClassMethodDefinition) -> String {
    let span = method.linear_span();
...
impl<'a> ScriptLowerer<'a> {
    fn empty_object_shape() -> HeapShape {
    ...
    fn fresh_constructed_instance_info() -> ValueInfo {
        ValueInfo {
    ...
    fn array_buffer_instance_shape() -> Box<HeapShape> {
        ...
        properties.insert(
            ARRAY_BUFFER_IMMUTABLE_SLOT.to_string(),
            ObjectShapeProperty::Data(ValueInfo::new(ValueKind::Boolean)),
        );
        properties.insert(
            ARRAY_BUFFER_MAX_BYTE_LENGTH_SLOT.to_string(),
            ObjectShapeProperty::Data(ValueInfo::new(ValueKind::Number)),
        );
        properties.insert(
            ARRAY_BUFFER_RESIZABLE_SLOT.to_string(),
            ObjectShapeProperty::Data(ValueInfo::new(ValueKind::Boolean)),
        );
        properties.insert(
            ARRAY_BUFFER_SHARED_SLOT.to_string(),
            ObjectShapeProperty::Data(ValueInfo::new(ValueKind::Boolean)),
        );
    ...
    fn synthetic_realm_global_shape() -> Box<HeapShape> {
        Box::new(HeapShape::Object(ObjectShape {
            prototype: Some(Box::new(Self::empty_object_shape())),
            properties: BTreeMap::new(),
            private_brands: BTreeSet::new(),
    ...
    fn synthetic_realm_record_shape() -> Box<HeapShape> {
        let mut properties = BTreeMap::new();
        properties.insert(
            "global".to_string(),
    ...
    fn data_view_instance_shape() -> Box<HeapShape> {
        ...
        properties.insert(
            DATA_VIEW_LENGTH_TRACKING_SLOT.to_string(),
            ObjectShapeProperty::Data(ValueInfo::new(ValueKind::Boolean)),
        );
        properties.insert(
            "buffer".to_string(),
    ...
    fn typed_array_instance_shape_for_constructor(builtin: StandardBuiltinId) -> Box<HeapShape> {
    ...
    fn typed_array_instance_shape_with_prototype(prototype: Box<HeapShape>) -> Box<HeapShape> {
        let mut properties = BTreeMap::new();
        ...
        properties.insert(
            TYPED_ARRAY_BYTES_PER_ELEMENT_SLOT.to_string(),
            ObjectShapeProperty::Data(ValueInfo::new(ValueKind::Number)),
        );
        properties.insert(
            TYPED_ARRAY_ELEMENT_KIND_SLOT.to_string(),
            ObjectShapeProperty::Data(ValueInfo::new(ValueKind::Number)),
        );
        properties.insert(
    ...
    fn typed_array_prototype_shape() -> Box<HeapShape> {
        let mut properties = BTreeMap::new();
    ...
    fn typed_array_intrinsic_constructor_shape() -> Box<HeapShape> {
    ...
    fn typed_array_constructor_prototype_shape(builtin: StandardBuiltinId) -> Box<HeapShape> {
        let mut properties = BTreeMap::new();
        properties.insert(
            "constructor".to_string(),
            ObjectShapeProperty::Data(Self::function_value_info_with_constructable(
    ...
    fn is_typed_array_constructor_target(target: &TypedExpr) -> bool {
    ...
    fn can_be_typed_array_constructor_target(target: &TypedExpr) -> bool {
    ...
    fn function_heap_shape(constructable: bool) -> Box<HeapShape> {
        let mut properties = BTreeMap::new();
        if constructable {
            properties.insert(
                "prototype".to_string(),
    ...
    fn function_value_info_with_constructable(
        function_id: FunctionId,
        constructable: bool,
    ) -> ValueInfo {
        ValueInfo {
    ...
    fn function_construct_this_info(function_id: FunctionId) -> ValueInfo {
    ...
    fn standard_boxed_prototype_shape(kind: BoxedPrimitiveKind) -> Box<HeapShape> {
        let mut properties = BTreeMap::new();
    ...
    fn function_value_info(&self, function_id: &FunctionId) -> ValueInfo {
        let constructable = self
            .function_signatures
            .get(function_id)
            .map(|signature| signature.constructable)
            .unwrap_or(false);
    ...
    fn new(
        ...
        Self {
            ...
            global_properties: {
                ...
                properties.insert(
                    CREATE_REALM_NAME.to_string(),
                    GlobalPropertyInfo {
                        value_info: Self::host_function_value_info(HostBuiltinId::CreateRealm),
                        proven_present: true,
                        configurable: true,
                        source: GlobalPropertySource::HostBuiltin,
                    },
                ...
                properties.insert(
                    PARSE_INT_NAME.to_string(),
                    GlobalPropertyInfo {
                        value_info: Self::host_function_value_info(HostBuiltinId::ParseInt),
                        proven_present: true,
                        configurable: true,
                        source: GlobalPropertySource::HostBuiltin,
                    },
        ...
        for function_id in &self.analysis.function_order {
            ...
            self.function_signatures.insert(
                ...
                FunctionSignature {
                    ...
                    return_shape: None,
                    return_targets: BTreeSet::new(),
                    constructor_instance: if plan.constructable {
                        Self::fresh_constructed_instance_info()
                    } else {
                        ValueInfo::undefined()
                    },
                    this_info: if plan.flavor == FunctionFlavor::Arrow {
                        ValueInfo::undefined()
                    } else if plan.constructable {
    ...
    fn exact_signature_for_function(
        ...
        if function_observations.len() == 1 {
            return function_observations.pop();
        }
    ...
    fn merge_exact_context_function_observation(
        ...
        if let Some(signature) = self.exact_context_function_observations.get_mut(&key) {
                ...
                .zip(propagated_signature.params.iter())
            {
                ...
                current_param.kind = propagated_param.kind;
                current_param.possible_kinds = propagated_param.possible_kinds;
                current_param.heap_shape = propagated_param.heap_shape.clone();
                current_param.function_targets = propagated_param.function_targets.clone();
            }
        ...
        for function_id in function_ids {
            ...
            if let Some(signature) = self.function_signatures.get_mut(&function_id) {
                    ...
                    .zip(propagated_signature.params.iter())
                {
                    ...
                    let possible_kinds =
                        if current_param.possible_kinds == KindSet::all_runtime_tags() {
                            propagated_param.possible_kinds
                        } else if propagated_param.possible_kinds == KindSet::all_runtime_tags() {
                            current_param.possible_kinds
                        } else {
                            current_param
                                .possible_kinds
                                .union(propagated_param.possible_kinds)
                        };
                    current_param.kind = possible_kinds.as_value_kind();
                    current_param.possible_kinds = possible_kinds;
                    current_param.heap_shape = match (
                        current_param.heap_shape.as_ref(),
                        propagated_param.heap_shape.as_ref(),
                    ) {
                    ...
                    current_param
                        .function_targets
                        .extend(propagated_param.function_targets.iter().cloned());
                }
                if propagated_signature.this_observed {
                    signature.this_info = merged_this;
                    signature.this_observed = true;
    ...
    fn merge_callback_observation_signature(
        ...
        if let Some(signature) = self.exact_context_callback_observations.get_mut(&key) {
                ...
                .zip(propagated_signature.params.iter())
            {
                ...
                let possible_kinds =
                    if current_param.possible_kinds == KindSet::from_kind(ValueKind::Undefined) {
                        propagated_param.possible_kinds
                    } else if propagated_param.possible_kinds
                        == KindSet::from_kind(ValueKind::Undefined)
                    {
                        current_param.possible_kinds
                    } else {
    ...
    fn merge_single_signature_observation(
        ...
        if let Some(signature) = self.function_signatures.get_mut(function_id) {
                ...
                .zip(propagated_signature.params.iter())
            {
                ...
                let possible_kinds = if current_param.possible_kinds == KindSet::all_runtime_tags()
                {
                    propagated_param.possible_kinds
                } else if propagated_param.possible_kinds == KindSet::all_runtime_tags() {
                    current_param.possible_kinds
    ...
    fn prepare_root_function_bindings(&mut self, root_functions: &[PendingFunction<'a>]) {
        ...
        for function in root_functions {
            let bare_function_info = self.function_value_info(&function.id);
            let function_info = self
                .lookup_global_property(&function.name)
                .filter(|info| {
                    info.kind == ValueKind::Function && info.function_targets.contains(&function.id)
                })
        ...
        root_functions
            ...
            .map(|function| StatementIr::Lexical {
                ...
                init: TypedExpr::from_info(
                    self.function_value_info(&function.id),
        ...
        match target {
            Expression::PropertyAccess(PropertyAccess::Simple(access)) => {
                ...
                let key = match target.kind {
                    ...
                    ValueKind::Array | ValueKind::Arguments => {
    ...
    fn infer_statement_throw_info(&self, statement: &StatementIr) -> Option<ValueInfo> {
        match statement {
            ...
            StatementIr::ForOfArray { iterable, body, .. } => self.merge_optional_value_info(
    ...
    fn static_same_value_expr(lhs: &TypedExpr, rhs: &TypedExpr) -> Option<bool> {
        match (&lhs.expr, &rhs.expr) {
            (ExprIr::Undefined, ExprIr::Undefined) | (ExprIr::Null, ExprIr::Null) => Some(true),
            (ExprIr::Boolean(lhs), ExprIr::Boolean(rhs)) => Some(lhs == rhs),
            (ExprIr::String(lhs), ExprIr::String(rhs)) => Some(lhs == rhs),
            (ExprIr::Number(lhs), ExprIr::Number(rhs)) => {
                let lhs = f64::from_bits(*lhs);
                let rhs = f64::from_bits(*rhs);
                if lhs.is_nan() && rhs.is_nan() {
                    Some(true)
                } else {
                    let distinct_zero_signs = lhs == 0.0
                        && rhs == 0.0
                        && lhs.is_sign_negative() != rhs.is_sign_negative();
                    Some(lhs == rhs && !distinct_zero_signs)
                }
            }
            _ => None,
        }
    ...
    fn lower_for_of_loop(&mut self, for_of: &ForOfLoop) -> (StatementIr, ValueKind) {
        ...
        if !iterable.possible_kinds.contains(ValueKind::Array) {
            self.unsupported("for-of non-array iterable");
            return (StatementIr::Empty, ValueKind::Undefined);
        }
        let before_vars = self.var_bindings.clone();
        let before_globals = self.global_properties.clone();
        self.push_scope();
        let element_info = match iterable.heap_shape.as_deref() {
            ...
            if let Some(init) = parameter.init() {
                if default_param_uses_current_or_later_name(
                    ...
                    return FunctionIr {
                        ...
                        is_nested: function.parent_owner_id != SCRIPT_OWNER_ID,
                        is_expression: function.is_expression,
                        is_named_expression: function.is_expression
                            && function.self_binding_name.is_some(),
                        captures_lexical_this: function.captures.contains_key(LEXICAL_THIS_NAME),
                        captures_lexical_arguments: function
                            .captures
    ...
    fn lower_lexical_declaration(
        ...
        for variable in list.as_ref() {
            ...
            statements.push(StatementIr::Lexical { mode, name, init });
        }
    ...
    fn lower_template_literal(&mut self, template: &TemplateLiteral) -> TypedExpr {
        ...
        for element in template.elements() {
            let part = match element {
                TemplateElement::String(sym) => TypedExpr::from_info(
    ...
    fn lower_class_common(
            ...
            match element {
                ClassElement::MethodDefinition(method) => {
                    let kind = match method.kind() {
                        ...
                        MethodDefinitionKind::Set => ClassFunctionKind::Setter,
                ...
                | ClassElement::StaticFieldDefinition(field) => {
                    ...
                    fields.push(FieldPlan {
                        ...
                        placement: if matches!(element, ClassElement::StaticFieldDefinition(_)) {
                            ClassMethodPlacementIr::Static
                        } else {
                            ClassMethodPlacementIr::Instance
                        },
        ...
        for (block_id, block) in &static_blocks {
        ...
        TypedExpr::from_info(
            ...
            ExprIr::ClassDefinition(Box::new(ClassDefinitionIr {
                ...
                static_blocks: static_blocks
                    .iter()
                    .map(|(function_id, _)| ClassStaticBlockIr {
        ...
        for (index, parameter) in parameters.as_ref().iter().enumerate() {
            ...
            if let Some(init) = parameter.init() {
                if default_param_uses_current_or_later_name(
                    ...
                    lowerer.unsupported_with_message(format!(
                        "unsupported in porffor wasm-aot first slice: self- or later-param read in default initializer for `{function_name}`"
                    ));
                }
            }
            lowerer.declare_binding(
    ...
    fn lower_generated_block_function(
        ...
        current_this_info: ValueInfo,
        current_construct_this_info: Option<ValueInfo>,
        _class_context: ClassLoweringContext,
        prefix_statements: Vec<StatementIr>,
    ...
    fn lower_call(&mut self, callee: &Expression, args: &[Expression]) -> TypedExpr {
            ...
            if let Expression::Identifier(identifier) = access.target() {
                ...
                if target_name == "floatArrayConstructors" {
                    if let PropertyAccessField::Const(field) = access.field() {
                        ...
                        if field_name == "concat" && args.len() == 1 {
                            for arg in args {
                                self.lower_expression(arg);
                            }
                            let elements = [
                            ...
                            .into_iter()
                            .map(|builtin| self.function_value_expr(builtin.function_id()))
                            .collect::<Vec<_>>();
                            return Self::array_literal_from_lowered(elements);
                        }
                    }
                }
            }
                ...
                PropertyAccess::Simple(access) => {
                    ...
                    let callee = match receiver.kind {
                            ...
                            if let PropertyAccessField::Const(field) = access.field() {
                                ...
                                if let Some(builtin) = builtin {
                                    ...
                                    return self.unsupported_expr(
                                        "indirect call: unsupported string property",
                                    );
                                }
                            } else {
                                return self
                        ...
                        "from"
                                    ...
                                    "of" if Self::is_typed_array_constructor_target(&receiver) => {
                        ...
                        == Some(StandardBuiltinId::StringPrototypeSubstring)
                    {
                        ...
                        let end = args
                            ...
                            .map(|arg| self.lower_expression(arg))
                            .unwrap_or_else(TypedExpr::undefined);
                        let length = if matches!(end.kind, ValueKind::Undefined) {
                            end
                        } else {
                            TypedExpr::from_info(
                                ValueInfo::new(ValueKind::Number),
        ...
        if !self.exact_context_callback_targets.is_empty() && !callee.function_targets.is_empty() {
        ...
        let Some(mut function_id) = self.resolve_single_function_target(&callee) else {
            if !callee.function_targets.is_empty() {
                ...
                for function_id in &callee.function_targets {
        ...
        let (effective_function_id, args, info) =
            self.lower_call_args_with_target(&function_id, args);
            ...
            match statement.as_ref() {
                ...
                Statement::Throw(throw_stmt) => {
                    ...
                    object_shape.properties.insert(
                        PORFFOR_GENERATOR_THROW_SLOT.to_string(),
                    ...
                    return TypedExpr::from_info(
                        ...
                        ExprIr::ObjectLiteral(vec![ObjectPropertyIr::Data {
                            key: PORFFOR_GENERATOR_THROW_SLOT.to_string(),
                            value: thrown,
                            is_shorthand: false,
                        }]),
                    );
                }
                _ => return self.unsupported_expr("unsupported expression form"),
            }
        }
    ...
    fn lower_new(&mut self, new_expr: &New) -> TypedExpr {
        ...
        if !signature.constructable || signature.flavor == FunctionFlavor::Arrow {
            let args = new_expr
                .arguments()
                .iter()
                .map(|arg| self.lower_expression(arg))
        ...
        if !exact_prepass_call {
            if let Some(signature) = self.function_signatures.get_mut(function_id) {
                for (param, arg) in signature.params.iter_mut().zip(lowered_args.iter()) {
                    ...
                    if previous_kind == ValueKind::Dynamic || previous_kind == ValueKind::Undefined
                    {
                        param.heap_shape = arg.heap_shape.clone();
                        param.function_targets = arg.function_targets.clone();
                    } else if previous_kind == arg.kind {
    ...
    fn standard_builtin_call_info(
        ...
        match builtin {
            StandardBuiltinId::FunctionConstructor => {
                if context == "construct" && args.is_empty() {
                    Some(Self::function_value_info_with_constructable(
                        StandardBuiltinId::FunctionConstructor.function_id(),
                        true,
                    ))
                } else {
            ...
            StandardBuiltinId::ObjectDefineProperties => Some(ValueInfo {
                kind: ValueKind::Object,
            ...
            StandardBuiltinId::ObjectPreventExtensions => {
                let Some(target) = args.first() else {
                    self.unsupported_with_message(format!(
                        "unsupported in porffor wasm-aot first slice: Object.preventExtensions requires object"
                    ));
                    return None;
                };
                if !target
            ...
            StandardBuiltinId::ReflectGet => Some(ValueInfo {
                kind: ValueKind::Dynamic,
                possible_kinds: KindSet::all_runtime_tags(),
                heap_shape: None,
                function_targets: BTreeSet::new(),
            }),
            StandardBuiltinId::ReflectHas => Some(ValueInfo::new(ValueKind::Boolean)),
            StandardBuiltinId::ArrayConstructor => {
                let mut shape = ArrayShape::default();
                if !(args.len() == 1
            ...
            StandardBuiltinId::ArrayPrototypeValues => Some(ValueInfo {
                kind: ValueKind::Object,
                possible_kinds: KindSet::from_kind(ValueKind::Object),
                heap_shape: Some(Box::new(Self::empty_object_shape())),
                function_targets: BTreeSet::new(),
    ...
    fn merge_function_param_infos(&mut self, function_id: &FunctionId, args: &[ValueInfo]) {
        ...
        for (param, arg) in signature.params.iter_mut().zip(args.iter()) {
            ...
            let previous_kind = param.kind;
            let possible_kinds = if param.possible_kinds == KindSet::all_runtime_tags() {
                arg.possible_kinds
            } else {
                param.possible_kinds.union(arg.possible_kinds)
            };
    ...
    fn observe_exact_callback_param_infos(
        ...
        for (param, arg) in observation.params.iter_mut().zip(args.iter()) {
            ...
            let previous_kind = param.kind;
            let possible_kinds = if param.possible_kinds == KindSet::from_kind(ValueKind::Undefined)
            {
                arg.possible_kinds
            } else {
                param.possible_kinds.union(arg.possible_kinds)
            ...
            param.kind = possible_kinds.as_value_kind();
            param.possible_kinds = possible_kinds;
            if previous_kind == ValueKind::Undefined {
                param.heap_shape = arg.heap_shape.clone();
                param.function_targets = arg.function_targets.clone();
            } else if previous_kind == arg.kind {
                if param.heap_shape != arg.heap_shape {
    ...
    fn array_flat_map_result_info(
        &self,
        receiver: &TypedExpr,
        mapper: Option<&TypedExpr>,
    ) -> ValueInfo {
        ...
        if let Some(constructor_info) = self.read_object_shape(receiver, "constructor") {
            if let Some(species_info) = self.read_object_shape(
                ...
                if !species_info.possible_kinds.is_subset_of(
                    ...
                    return self.array_concat_result_info(receiver, &[]);
                }
            }
        }
        ...
        let output_pattern = match mapper_result.heap_shape.as_deref() {
            Some(HeapShape::Array(mapped_shape)) => mapped_shape.elements.clone(),
            _ if !mapper_result.possible_kinds.contains(ValueKind::Array) => vec![mapper_result],
            _ => return default(),
        };

        let mut shape = ArrayShape::default();
        shape.elements.reserve(
            receiver_shape
                .elements
                .len()
                .saturating_mul(output_pattern.len()),
        );
        for _ in &receiver_shape.elements {
            shape.elements.extend(output_pattern.iter().cloned());
        }
    ...
    fn lower_array_literal(&mut self, array: &ArrayLiteral) -> TypedExpr {
            ...
            .any(|element| matches!(element, Some(Expression::Spread(_))))
        {
            ...
            for element in array.as_ref() {
                ...
                if let Expression::Spread(spread) = element {
                    ...
                    if !spread_value.possible_kinds.contains(ValueKind::Array) {
                        return self.unsupported_expr("array literal non-array spread");
                    }
            ...
            return TypedExpr::from_info(
                ...
                ExprIr::CallMethod {
                    receiver: Box::new(TypedExpr::from_info(
                        ValueInfo {
                            kind: ValueKind::Array,
                            possible_kinds: KindSet::from_kind(ValueKind::Array),
                            heap_shape: Some(Box::new(HeapShape::Array(ArrayShape::default()))),
    ...
    fn lower_regexp_literal(&mut self, regexp: &RegExpLiteral) -> TypedExpr {
        ...
        shape.properties.insert(
            "source".to_string(),
            ObjectShapeProperty::Data(source.value_info()),
        );
        properties.push(ObjectPropertyIr::Data {
        ...
        let flags_value = TypedExpr::from_info(
            Self::string_value_info(&flags),
            ExprIr::String(flags.clone()),
        );
        shape.properties.insert(
            "flags".to_string(),
            ObjectShapeProperty::Data(flags_value.value_info()),
        );
        properties.push(ObjectPropertyIr::Data {
            key: "flags".to_string(),
            value: flags_value,
        ...
        for (name, flag) in [
            ...
            let value = TypedExpr::from_info(
                ...
                ExprIr::Boolean(flags.contains(flag)),
            );
            shape.properties.insert(
                name.to_string(),
                ObjectShapeProperty::Data(value.value_info()),
            );
            properties.push(ObjectPropertyIr::Data {
                key: name.to_string(),
                value,
        ...
        match access {
            PropertyAccess::Simple(access) => {
                ...
                match target.kind {
                    ...
                    ValueKind::Undefined => self.lower_object_property_key(
                        TypedExpr::from_info(
                            ValueInfo {
                                kind: ValueKind::Dynamic,
                                possible_kinds: Self::object_like_kind_set(),
    ...
    fn lower_object_property_key(
        ...
        if let PropertyKeyIr::StaticString(name) = &key {
            ...
            if let Some(info) = self.read_object_shape(&target, name) {
                self.mark_host_builtins_from_info(&info);
                return TypedExpr::from_info(
                    info,
                    ExprIr::PropertyRead {
            ...
            if name == "from" && Self::is_typed_array_constructor_target(&target) {
                return self.function_value_expr(StandardBuiltinId::TypedArrayFrom.function_id());
            }
            if name == "of" && Self::is_typed_array_constructor_target(&target) {
                return self.function_value_expr(StandardBuiltinId::TypedArrayOf.function_id());
            }
            if name == "toString"
    ...
    fn lower_string_index_key(
        ...
        if let PropertyAccessField::Const(name) = field {
            ...
            return self.unsupported_expr("unsupported string dot access");
        }
        let PropertyAccessField::Expr(expr) = field else {
    ...
    fn lower_assign(&mut self, op: AssignOp, lhs: &AssignTarget, rhs: &Expression) -> TypedExpr {
        match op {
            ...
            AssignOp::Add | AssignOp::Sub | AssignOp::Mul | AssignOp::Div | AssignOp::Mod => {
                ...
                let expr = if self.lookup_binding(&name).is_some() {
                    ExprIr::CompoundAssignIdentifier {
                        name,
                        op,
                        value: Box::new(value),
            ...
            PropertyAccess::Simple(access) => {
                ...
                match target.kind {
                    ...
                    ValueKind::Arguments => {
                        ...
                        let index = self.lower_expression(expr);
                        let key = if let ExprIr::String(key) = &index.expr {
                            PropertyKeyIr::StaticString(key.clone())
    ...
    fn lower_unary(&mut self, op: UnaryOp, target: &Expression) -> TypedExpr {
        ...
        match op {
            ...
            UnaryOp::Minus => {
                ...
                if !lowered_target.possible_kinds.contains(ValueKind::Number) {
            ...
            RelationalOp::Equal | RelationalOp::NotEqual => {
                if self
                    ...
                    && self
                        ...
                        .is_some()
                    || lhs.possible_kinds.contains(ValueKind::Function)
                        && rhs.possible_kinds.is_subset_of(KindSet::NULLISH)
                    || rhs.possible_kinds.contains(ValueKind::Function)
                        && lhs.possible_kinds.is_subset_of(KindSet::NULLISH)
                {
                    let op = match relational {
                        RelationalOp::Equal => EqualityBinaryOp::LooseEqual,
                        RelationalOp::NotEqual => EqualityBinaryOp::LooseNotEqual,
                        _ => unreachable!(),
                    };
                    TypedExpr::from_info(
                        ValueInfo::new(ValueKind::Boolean),
    ...
    fn lower_logical(
        ...
        let definitely_truthy = lhs
            .possible_kinds
            .is_subset_of(Self::object_like_kind_set())
            || lhs.possible_kinds == KindSet::from_kind(ValueKind::Function);
        ...
        for key in order {
            ...
            for function_id in &info.function_targets {
                ...
                if !signature
                    ...
                    .is_subset_of(KindSet::PRIMITIVE_ONLY)
                {
                    self.unsupported_expr("heap ToPrimitive hook must return primitive");
                    return None;
                }
                return_kinds = return_kinds.union(signature.return_possible_kinds);
            }
    ...
    fn apply_shape_write(
        ...
        match (shape.as_mut(), &path[0]) {
            ...
            (
                HeapShape::Object(object),
                PropertyKeyIr::ArrayIndex(_) | PropertyKeyIr::ArrayLength,
            ) => {
                if !object
                    .properties
                    .contains_key(TYPED_ARRAY_VIEWED_ARRAY_BUFFER_SLOT)
                {
                    target.heap_shape = None;
                }
            }
    ...
    fn infer_var_binding_info_from_statement(
        ...
        match statement {
            ...
            Statement::Labelled(labelled) => Self::labelled_base_statement(labelled)
        ...
        if let Some(signature) = self.function_signatures.get_mut(function_id) {
            if let Some(param) = signature.params.get_mut(index) {
                ...
                let possible_kinds = if param.possible_kinds == KindSet::all_runtime_tags() {
                    ...
                    param.possible_kinds.union(KindSet::from_kind(kind))
                };
                param.kind = possible_kinds.as_value_kind();
                param.possible_kinds = possible_kinds;
            }
