...
use super::early::{module_early_errors, module_parse_failure_diagnostic};
...
impl ImportPhaseIr {
    ...
    fn from_ast(phase: ImportPhase) -> Self {
        match phase {
...
///
/// Equality is `ModuleRequestsEqual`: specifier *and* attributes. `attributes`
/// is kept sorted by key at construction so derived `Eq` is that relation.
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
...
pub fn parse_module_record(
    ...
    let (module, interner) = reparse_module(source)
        .map_err(|message| vec![module_parse_failure_diagnostic(&message)])?;
    let record = build_module_record(&module, &interner, source, id, key);
...
fn build_module_record(
    ...
    // deduplicated by ModuleRequestsEqual — specifier *and* attributes.
...
fn reparse_module(source: &SourceUnit) -> Result<(Module, Interner), String> {
    ...
    let module = match panic::catch_unwind(AssertUnwindSafe(|| {
        ...
        Err(payload) => {
            return Err(format!(
                ...
                module_parser_abort_message(&payload)
            ));
        }
    };
    Ok((module, interner))
...
fn module_parser_abort_message(payload: &(dyn core::any::Any + Send)) -> String {
...
pub fn scan_module_requests(source: &SourceUnit) -> Result<Vec<ModuleRequestIr>, String> {
    let (module, interner) = reparse_module(source)?;
    let record = build_module_record(&module, &interner, source, 0, String::new());
    let mut requests = record.requested_modules;
...
mod tests {
    ...
    fn record_of(source: &str) -> SourceTextModuleRecordIr {
        parse_module_record(&source_unit(source), 0, "main.mjs".to_string())
    ...
    fn requested_modules_dedup_by_specifier_and_attributes() {
        ...
        // ModuleRequestsEqual compares specifier *and* attributes, so the two
    ...
    fn record_with_id(source: &str, id: ModuleUnitId) -> SourceTextModuleRecordIr {
        parse_module_record(&source_unit(source), id, "main.mjs".to_string())
            .expect("module record should build")
    }
