...
mod json_stringify_replacer_invocation {
    ...
    pub(super) struct JsonStringifyReplacerFunctionLocals(TaggedLocals);

    impl JsonStringifyReplacerFunctionLocals {
        pub(super) const fn new(payload: u32, tag: u32) -> Self {
            Self(TaggedLocals::new(payload, tag))
        }
    }

    pub(super) struct JsonStringifyReplacerReceiverLocals(TaggedLocals);

    impl JsonStringifyReplacerReceiverLocals {
        pub(super) const fn new(payload: u32, tag: u32) -> Self {
            Self(TaggedLocals::new(payload, tag))
        }
    }

    pub(super) struct JsonStringifyReplacerPropertyKeyLocals(TaggedLocals);

    impl JsonStringifyReplacerPropertyKeyLocals {
        pub(super) const fn new(payload: u32, tag: u32) -> Self {
            Self(TaggedLocals::new(payload, tag))
        }
    }

    pub(super) struct JsonStringifyReplacerValueLocals(TaggedLocals);

    impl JsonStringifyReplacerValueLocals {
        pub(super) const fn new(payload: u32, tag: u32) -> Self {
            Self(TaggedLocals::new(payload, tag))
        }
    }

    #[must_use = "a JSON.stringify replacer invocation must be emitted"]
    pub(super) struct JsonStringifyReplacerInvocationLocals {
    ...
    impl JsonStringifyReplacerInvocationLocals {
        ...
        pub(super) const fn into_parts(
            self,
        ) -> (TaggedLocals, TaggedLocals, TaggedLocals, TaggedLocals) {
            (
                self.replacer.0,
                self.receiver.0,
                self.property_key.0,
...
impl<'a> FunctionBuilder<'a> {
            ...
            JsonBuiltin::Stringify => {
                ...
                self.emit_json_apply_replacer_with_this(
                    JsonStringifyReplacerInvocationLocals::new(
                        JsonStringifyReplacerFunctionLocals::new(
                            replacer_payload_local,
                            replacer_tag_local,
                        ),
                        JsonStringifyReplacerReceiverLocals::new(
                            wrapper_payload_local,
                            wrapper_tag_local,
                        ),
                        JsonStringifyReplacerPropertyKeyLocals::new(
                            key_payload_local,
                            key_tag_local,
                        ),
                        JsonStringifyReplacerValueLocals::new(value_payload_local, value_tag_local),
                    ),
                    function,
                )?;
    ...
    pub(super) fn emit_json_parse_builtin(
        &mut self,
        function: &mut Function,
    ) -> Result<(), EmitError> {
        self.emit_json_builtin(JsonBuiltin::Parse, function)
    }
    ...
    pub(super) fn emit_json_stringify_builtin(
        ...
        self.emit_json_builtin(JsonBuiltin::Stringify, function)
    }
    ...
    pub(super) fn emit_json_raw_json_builtin(
        ...
        self.emit_json_builtin(JsonBuiltin::RawJson, function)
    }
    ...
    pub(super) fn emit_json_is_raw_json_builtin(
        ...
        self.emit_json_builtin(JsonBuiltin::IsRawJson, function)
    }
    ...
    fn emit_json_create_data_property(
        ...
        let accepted_payload = self.reserve_temp_local();
        let accepted_tag = self.reserve_temp_local();

        // CreateDataProperty ignores a false result, but propagates an abrupt
        // DefineOwnProperty completion. The intrinsic Reflect operation owns
        // that distinction for ordinary objects, arrays and Proxy targets.
        // Its input descriptor is private: inherited get/set fields must not
        // participate in the canonical ToPropertyDescriptor conversion.
        self.emit_alloc_plain_object_with_prototype(None, None, function)?;
        function.instruction(&Instruction::LocalSet(descriptor_payload));
        function.instruction(&Instruction::I64Const(ValueKind::Object.tag() as i64));
        ...
        for (name, payload, tag) in [
            ...
            ("writable", attribute_value, attribute_tag),
            ("enumerable", attribute_value, attribute_tag),
            ("configurable", attribute_value, attribute_tag),
        ] {
        ...
        let define_property = self
            ...
            .cloned()
            .expect("JSON.parse roots the canonical Boolean property-definition operation");
        self.emit_direct_js_call(
            &define_property,
            None,
            &[
                (holder_payload_local, holder_tag_local),
                (key_payload_local, key_tag),
                (descriptor_payload, descriptor_tag),
            ],
            accepted_payload,
            accepted_tag,
            function,
    ...
    fn emit_json_apply_replacer_with_this(
        &mut self,
        invocation: JsonStringifyReplacerInvocationLocals,
        function: &mut Function,
    ) -> Result<(), EmitError> {
        let (replacer, receiver, property_key, value) = invocation.into_parts();
        self.emit_is_callable_i32(replacer.tag, replacer.payload, function)?;
        function.instruction(&Instruction::If(BlockType::Empty));
        // Invoke the replacer, leaving any throw completion in place rather than
        // returning `self.result_local` directly: the thrown value lands in the
        // invocation's value role, so it must be moved into the result slot
        // before the helper returns. Using the auto-returning call
    ...
    pub(crate) fn emit_json_stringify_array_payload(
        ...
        self.emit_json_apply_replacer_with_this(
            JsonStringifyReplacerInvocationLocals::new(
                ...
                JsonStringifyReplacerReceiverLocals::new(array_payload_local, array_tag_local),
                JsonStringifyReplacerPropertyKeyLocals::new(key_payload_local, key_tag_local),
                JsonStringifyReplacerValueLocals::new(value_payload_local, value_tag_local),
            ),
            function,
        )?;
    ...
    pub(crate) fn emit_json_stringify_object_payload(
        ...
        self.emit_json_apply_replacer_with_this(
            JsonStringifyReplacerInvocationLocals::new(
                ...
                JsonStringifyReplacerReceiverLocals::new(object_payload_local, object_tag_local),
                JsonStringifyReplacerPropertyKeyLocals::new(key_payload_local, key_tag_local),
                JsonStringifyReplacerValueLocals::new(value_payload_local, value_tag_local),
            ),
            function,
    ...
    fn emit_parse_json_value_at_index(
        ...
        function.instruction(&Instruction::LocalSet(value_tag_local));
        function.instruction(&Instruction::I64Const(
            JsonParseFrameState::ArrayFirstOrEnd.word() as i64,
        ));
        function.instruction(&Instruction::LocalSet(nested_state_local));
        self.emit_increment_local(index_local, 1, function);
        function.instruction(&Instruction::Br(1));
        ...
        function.instruction(&Instruction::LocalSet(value_tag_local));
        function.instruction(&Instruction::I64Const(
            JsonParseFrameState::ObjectFirstKeyOrEnd.word() as i64,
        ));
        function.instruction(&Instruction::LocalSet(nested_state_local));
        self.emit_increment_local(index_local, 1, function);
        function.instruction(&Instruction::Br(1));
    ...
    pub(crate) fn emit_try_parse_json_text(
        ...
        function.instruction(&Instruction::I64GeS);
        function.instruction(&Instruction::If(BlockType::Empty));
        let root_frame_state =
            self.emit_validate_json_parse_frame_state_local(nested_state_local, function);
        ...
        let frame_state =
            self.emit_validate_json_parse_frame_state_local(frame_state_local, function);
        self.load_i64_to_local_from_offset(
            frame_local,
        ...
        self.emit_json_parse_frame_state_is_i32(
            &frame_state,
        ...
        function.instruction(&Instruction::LocalSet(frame_len_local));
        function.instruction(&Instruction::Else);
        self.emit_store_json_parse_frame_state(
            frame_local,
        ...
        self.emit_store_json_parse_frame_state(
            frame_local,
            JsonParseFrameState::ArrayCommaOrEnd,
            function,
        );
        function.instruction(&Instruction::LocalGet(nested_state_local));
        function.instruction(&Instruction::I64Const(0));
        ...
        self.emit_store_json_parse_frame_state(
            frame_local,
            JsonParseFrameState::ObjectValue,
            function,
        );
        function.instruction(&Instruction::Br(1));
        function.instruction(&Instruction::End);
        ...
        self.emit_store_json_parse_frame_state(
            frame_local,
            JsonParseFrameState::ObjectCommaOrEnd,
            function,
        );
        function.instruction(&Instruction::LocalGet(nested_state_local));
        function.instruction(&Instruction::I64Const(0));
        ...
        self.release_validated_json_parse_frame_state_local(frame_state);
