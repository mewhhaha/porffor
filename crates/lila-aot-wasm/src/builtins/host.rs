...
impl<'a> FunctionBuilder<'a> {
    ...
    pub(crate) fn compile_host_create_realm_builtin(
        ...
        let regexp_string_iterator_prototype_local = self.reserve_temp_local();
        ...
        let object_proto_key_local = self.reserve_temp_local();
        let object_proto_getter_local = self.reserve_temp_local();
        let object_proto_setter_local = self.reserve_temp_local();
        for (builtin, accessor_local) in [
            (
                StandardBuiltinId::ObjectPrototypeProtoGetter,
                object_proto_getter_local,
            ),
            (
                StandardBuiltinId::ObjectPrototypeProtoSetter,
                object_proto_setter_local,
            ),
        ] {
            let meta = self
                .functions
        ...
        function.instruction(&Instruction::LocalSet(object_proto_key_local));
        function.instruction(&Instruction::I64Const(ValueKind::Function.tag() as i64));
        function.instruction(&Instruction::LocalSet(tag_local));
        self.emit_object_define_accessor(
            object_prototype_local,
            object_proto_key_local,
        ...
        self.release_temp_local(object_proto_setter_local);
        self.release_temp_local(object_proto_getter_local);
        self.release_temp_local(object_proto_key_local);
        ...
        let regexp_string_iterator_next = self.emit_materialize_created_realm_iterator_next(
            CreatedRealmIteratorNextTarget::RegExpString,
            &iterator_next_publication_context,
            function,
        )?;
        self.emit_publish_created_realm_iterator_next(regexp_string_iterator_next, function)?;
        let regexp_iterator_tag_key = self.reserve_temp_local();
        let regexp_iterator_tag_value = self.reserve_temp_local();
        function.instruction(&Instruction::I64Const(
            self.strings
                .property_key_symbol_payload("Symbol.toStringTag"),
        ));
        function.instruction(&Instruction::LocalSet(regexp_iterator_tag_key));
        function.instruction(&Instruction::I64Const(
            self.strings.payload("RegExp String Iterator"),
        ));
        function.instruction(&Instruction::LocalSet(regexp_iterator_tag_value));
        function.instruction(&Instruction::I64Const(ValueKind::String.tag() as i64));
        function.instruction(&Instruction::LocalSet(tag_local));
        self.emit_object_append_data_property_with_flags(
            regexp_string_iterator_prototype_local,
            regexp_iterator_tag_key,
            regexp_iterator_tag_value,
            tag_local,
            false,
            false,
            true,
        ...
        self.release_temp_local(regexp_iterator_tag_value);
        self.release_temp_local(regexp_iterator_tag_key);
        ...
        self.release_temp_local(regexp_string_iterator_prototype_local);
