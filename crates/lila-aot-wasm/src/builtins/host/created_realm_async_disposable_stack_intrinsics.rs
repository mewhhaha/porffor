use super::*;
use lila_ir::ASYNC_DISPOSABLE_STACK_NAME;

#[must_use = "created-Realm AsyncDisposableStack intrinsics must be published"]
pub(super) struct CreatedRealmAsyncDisposableStackIntrinsics {
    prototype_local: u32,
    constructor_local: u32,
}

enum AsyncDisposableStackProperty {
    Method(&'static [&'static str]),
    Getter(&'static str),
}

impl<'a> FunctionBuilder<'a> {
    pub(super) fn emit_materialize_created_realm_async_disposable_stack_intrinsics(
        &mut self,
    ...
    ) -> Result<CreatedRealmAsyncDisposableStackIntrinsics, EmitError> {
        let constructor_meta = self
            .functions
            .get(&StandardBuiltinId::AsyncDisposableStackConstructor.function_id())
            .cloned()
            .ok_or_else(|| EmitError::unsupported("missing AsyncDisposableStack metadata"))?;
        ...
        for (builtin, property) in [
            (
                ...
                AsyncDisposableStackProperty::Method(&["adopt"]),
            ),
            (
                StandardBuiltinId::AsyncDisposableStackPrototypeDefer,
                AsyncDisposableStackProperty::Method(&["defer"]),
            ),
            (
                StandardBuiltinId::AsyncDisposableStackPrototypeMove,
                AsyncDisposableStackProperty::Method(&["move"]),
            ),
            (
                StandardBuiltinId::AsyncDisposableStackPrototypeUse,
                AsyncDisposableStackProperty::Method(&["use"]),
            ),
            (
                StandardBuiltinId::AsyncDisposableStackPrototypeDisposeAsync,
                AsyncDisposableStackProperty::Method(&["disposeAsync", "Symbol.asyncDispose"]),
            ),
            (
                StandardBuiltinId::AsyncDisposableStackPrototypeDisposedGetter,
                AsyncDisposableStackProperty::Getter("disposed"),
            ),
        ] {
            let meta = self
                .functions
            ...
            match property {
                AsyncDisposableStackProperty::Method(names) => {
                    for name in names {
                ...
                AsyncDisposableStackProperty::Getter(name) => {
        ...
        function.instruction(&Instruction::LocalSet(key_local));
        function.instruction(&Instruction::I64Const(
            self.strings.payload(ASYNC_DISPOSABLE_STACK_NAME),
        ));
        ...
        Ok(CreatedRealmAsyncDisposableStackIntrinsics {
            prototype_local,
            constructor_local,
        })
    }

    pub(super) fn emit_publish_created_realm_async_disposable_stack_intrinsics(
        &mut self,
        intrinsics: CreatedRealmAsyncDisposableStackIntrinsics,
        global_local: u32,
        function: &mut Function,
    ) -> Result<(), EmitError> {
        let CreatedRealmAsyncDisposableStackIntrinsics {
            prototype_local,
            constructor_local,
        } = intrinsics;
        let tag_local = self.reserve_temp_local();
