...
use set_path_realm::{
    object_mutation_error_realm, set_path_realm_environment_argument, ObjectMutationErrorRealm,
...
enum ObjectReadRevocationErrorRealm {
    TrustedCurrentEnvironment,
    MainRealmFallback,
}

fn outlined_object_read_realm_argument(
    source: ObjectReadErrorRealmSource,
...
mod tests {
    ...
    fn object_read_realm_projection_excludes_ordinary_lexical_environments() {
        ...
        for source in [
            ...
            match outlined_object_read_realm_argument(source) {
                OutlinedObjectReadRealmArgument::TrustedCurrentEnvironment => {}
                OutlinedObjectReadRealmArgument::MainRealmFallback => {
                    panic!("trusted object-read source lost its outlined Realm argument")
                }
            }
            match object_read_revocation_error_realm(source) {
                ObjectReadRevocationErrorRealm::TrustedCurrentEnvironment => {}
                ObjectReadRevocationErrorRealm::MainRealmFallback => {
                    panic!("trusted object-read source lost its revocation-error Realm")
                }
            }
        }

        match outlined_object_read_realm_argument(ObjectReadErrorRealmSource::GlobalFallback) {
            OutlinedObjectReadRealmArgument::MainRealmFallback => {}
            OutlinedObjectReadRealmArgument::TrustedCurrentEnvironment => {
                panic!("global fallback exposed an outlined lexical environment as Realm state")
            }
        }
        match object_read_revocation_error_realm(ObjectReadErrorRealmSource::GlobalFallback) {
            ObjectReadRevocationErrorRealm::MainRealmFallback => {}
            ObjectReadRevocationErrorRealm::TrustedCurrentEnvironment => {
                panic!("global fallback exposed a lexical environment as revocation Realm state")
            }
        }
    }
}
...
impl AccessorGetterLocals {
    pub(crate) const fn new(value: TaggedLocals) -> Self {
        Self(value)
    }
}

pub(crate) struct AccessorSetterLocals(TaggedLocals);

impl AccessorSetterLocals {
    pub(crate) const fn new(value: TaggedLocals) -> Self {
        Self(value)
    }
}

pub(crate) enum AccessorDescriptorLocals {
    Getter(AccessorGetterLocals),
    Setter(AccessorSetterLocals),
    GetterAndSetter {
        getter: AccessorGetterLocals,
        setter: AccessorSetterLocals,
    },
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct StoredDescriptorDataLocals(TaggedLocals);

impl StoredDescriptorDataLocals {
    pub(crate) const fn new(value: TaggedLocals) -> Self {
        Self(value)
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct StoredDescriptorGetterLocals(TaggedLocals);

impl StoredDescriptorGetterLocals {
    pub(crate) const fn new(value: TaggedLocals) -> Self {
        Self(value)
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct StoredDescriptorSetterLocals(TaggedLocals);

impl StoredDescriptorSetterLocals {
    pub(crate) const fn new(value: TaggedLocals) -> Self {
        Self(value)
    }
}
...
///
/// This role is distinct from the trap result, so swapping the two adjacent
/// scratch pairs is rejected at the acquisition boundary.
#[must_use = "Proxy OwnPropertyKeys trap locals must be consumed by trap acquisition"]
pub(crate) struct ProxyOwnKeysTrapLocals(TaggedLocals);

impl ProxyOwnKeysTrapLocals {
    pub(crate) const fn new(payload: u32, tag: u32) -> Self {
        Self(TaggedLocals::new(payload, tag))
    }
}

/// The Proxy `[[OwnPropertyKeys]]` trap result passed to post-trap validation.
///
/// Acquisition returns this non-copyable authority and one validator consumes
/// it, keeping validation attached to the exact result destination.
#[must_use = "a Proxy OwnPropertyKeys trap result must be consumed by one validator"]
pub(crate) struct ProxyOwnKeysTrapResultLocals(TaggedLocals);

impl ProxyOwnKeysTrapResultLocals {
    pub(crate) const fn new(payload: u32, tag: u32) -> Self {
        Self(TaggedLocals::new(payload, tag))
    }
}
...
///
/// Calling a trap can leave an abrupt completion in the shared helper ABI. The
/// post-trap invariant must not inspect its value until that completion has
/// been routed, or a target descriptor mismatch can replace the value that the
/// trap threw. Only `emit_normal_proxy_get_trap_result` turns this pending role
/// into the normal-only role accepted by the invariant.
#[must_use = "a pending Proxy Get trap result must be normalized before inspection"]
struct PendingProxyGetTrapResultLocals(TaggedLocals);

impl PendingProxyGetTrapResultLocals {
...
///
/// This type is deliberately not `Copy`: the invariant consumes the witness,
/// and a pending trap result cannot be substituted for it at the call site.
#[must_use = "a normal Proxy Get trap result must be consumed by its invariant"]
struct NormalProxyGetTrapResultLocals(TaggedLocals);

impl NormalProxyGetTrapResultLocals {
...
/// A value-free projection of `[[GetOwnProperty]]` carried in Wasm locals.
/// Namespace descriptors still read their live binding before discarding its
/// value, so a projection cannot bypass module TDZ.
///
/// Presence is deliberately independent of the stored descriptor word. The
/// all-false data descriptor has word zero, so using that word as an absence
/// sentinel would let a Proxy hide a present non-configurable property. Private
/// fields plus named predicates keep the two locals from being transposed at a
/// consumer. This record never allocates a JavaScript descriptor object.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct OwnDescriptorFactLocals {
    present: u32,
    descriptor: u32,
}

impl OwnDescriptorFactLocals {
    fn new(present: u32, descriptor: u32) -> Self {
        Self {
            present,
            descriptor,
        }
    ...
    pub(crate) fn emit_present_i32(self, function: &mut Function) {
        function.instruction(&Instruction::LocalGet(self.present));
...
/// The stored `[[Value]]` needed by Proxy `[[Get]]` and `[[Set]]` frozen-data
/// invariants.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct DescriptorDataValueLocals(TaggedLocals);

/// The stored `[[Get]]` needed by Proxy `[[Get]]`'s accessor invariant.
///
/// Some pre-existing storage uses zero for an omitted getter while newer
/// descriptor paths store tagged ECMAScript `undefined`. The named predicate
/// is the one normalization boundary for those equivalent representations.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct DescriptorGetterLocals(TaggedLocals);

impl DescriptorGetterLocals {
    fn emit_undefined_i32(self, function: &mut Function) {
        function.instruction(&Instruction::LocalGet(self.0.tag));
...
/// The exact direct-own-descriptor projection consumed by Proxy `[[Get]]`.
///
/// The descriptor fact, data value and accessor getter are distinct typed
/// roles. The observation is storage-only: constructing this record may not
/// invoke the getter it records.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct ProxyGetDescriptorLocals {
    fact: OwnDescriptorFactLocals,
    data_value: DescriptorDataValueLocals,
    getter: DescriptorGetterLocals,
}

impl ProxyGetDescriptorLocals {
    fn new(
        fact: OwnDescriptorFactLocals,
        data_value: DescriptorDataValueLocals,
        getter: DescriptorGetterLocals,
...
/// Which accessor endpoint a value-bearing descriptor projection observes.
///
/// Proxy `[[Get]]` and `[[Set]]` need opposite endpoints. Keeping them as
/// distinct variants prevents a caller from silently using a setter when the
/// `[[Get]]` invariant asks whether the getter is undefined.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum DescriptorAccessorProjectionLocals {
    Getter(DescriptorGetterLocals),
    Setter(DescriptorSetterLocals),
}

/// The three allocation-free views emitted by the one direct
/// `[[GetOwnProperty]]` representation loop.
///
/// Adding another view requires updating every exhaustive match below. A
/// consumer cannot request only part of either richer Proxy observation.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum DirectOwnDescriptorProjectionLocals {
    Fact(OwnDescriptorFactLocals),
    ProxyGet(ProxyGetDescriptorLocals),
...
impl DirectOwnDescriptorProjectionLocals {
    fn fact(self) -> OwnDescriptorFactLocals {
        match self {
            Self::Fact(fact) => fact,
            Self::ProxyGet(descriptor) => descriptor.fact,
    ...
    fn data_value(self) -> Option<DescriptorDataValueLocals> {
        match self {
            Self::Fact(_) => None,
            Self::ProxyGet(descriptor) => Some(descriptor.data_value),
            Self::ProxySet(descriptor) => Some(descriptor.data_value),
        }
    }

    fn accessor(self) -> Option<DescriptorAccessorProjectionLocals> {
        match self {
            Self::Fact(_) => None,
            Self::ProxyGet(descriptor) => Some(DescriptorAccessorProjectionLocals::Getter(
                descriptor.getter,
            )),
            Self::ProxySet(descriptor) => Some(DescriptorAccessorProjectionLocals::Setter(
                descriptor.setter,
            )),
        }
    }
...
/// Whether a 10.1.6.3 compatibility check is impossible, unconditional, or
/// gated by one or two run-time presence flags.
///
/// This is deliberately private and has no equality, default or copy
/// capability. Consumers must exhaustively translate the descriptor lattice
/// and immediately emit the predicate they receive.
...
impl DescriptorCompatibilityPredicate {
    fn from_presence<T>(presence: &Presence<T, u32>) -> Self {
        match presence {
            Presence::Absent => Self::Never,
            Presence::Present(_) => Self::Always,
            Presence::Runtime { present, .. } => Self::AtRuntime {
                first: *present,
                second: None,
            },
        }
    ...
    fn from_kind_terms(terms: &KindTerms<WasmLocals>) -> Self {
        ...
        match terms.runtime {
            ...
            [Some(first), None] | [None, Some(first)] => Self::AtRuntime {
                first,
                second: None,
            },
            [Some(first), Some(second)] => Self::AtRuntime {
                first,
                second: Some(second),
            },
        }
...
/// The carrier for a converted descriptor whose temporary locals still need
/// to be materialized and released. Boolean fields retain their tags because
/// materialization publishes them as JavaScript values.
struct ReservedPropertyDescriptorCarrier;

impl DescriptorCarrier for ReservedPropertyDescriptorCarrier {
    type Value = TaggedLocals;
    type Flag = TaggedLocals;
    type RuntimeFlag = u32;
}
...
/// same two-case split as `lila_ir`'s `CompleteDescriptor`, at the
/// Wasm-word level.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl DescriptorKindMarker for DataKind {
    const SEED: DescriptorWord = StoredPropertyAttributes::Data {
        writable: false,
        enumerable: false,
        configurable: false,
    }
...
impl DescriptorKindMarker for AccessorKind {
    const SEED: DescriptorWord = StoredPropertyAttributes::Accessor {
        enumerable: false,
        configurable: false,
    }
    .descriptor_word();
...
impl<'a> FunctionBuilder<'a> {
    fn emit_descriptor_compatibility_check(
        &mut self,
        predicate: DescriptorCompatibilityPredicate,
        function: &mut Function,
        emit_check: impl FnOnce(&mut Self, &mut Function) -> Result<(), EmitError>,
    ) -> Result<(), EmitError> {
        match predicate {
            DescriptorCompatibilityPredicate::Never => return Ok(()),
            DescriptorCompatibilityPredicate::Always => emit_check(self, function)?,
            DescriptorCompatibilityPredicate::AtRuntime { first, second } => {
    ...
    pub(crate) fn emit_validate_array_named_descriptor(
        ...
        let stored = StoredDescriptorLocals::new(
            StoredDescriptorDataLocals::new(TaggedLocals::new(
                self.reserve_temp_local(),
                self.reserve_temp_local(),
            )),
            StoredDescriptorGetterLocals::new(TaggedLocals::new(
                self.reserve_temp_local(),
                self.reserve_temp_local(),
            )),
            StoredDescriptorSetterLocals::new(TaggedLocals::new(
                self.reserve_temp_local(),
                self.reserve_temp_local(),
            )),
        );
        ...
        for local in [
            stored.setter.0.tag,
            stored.setter.0.payload,
            stored.getter.0.tag,
            stored.getter.0.payload,
            stored.data.0.tag,
            stored.data.0.payload,
        ] {
    ...
    pub(crate) fn emit_validate_stored_descriptor(
        ...
        if let Some(configurable_payload_local) = descriptor.configurable.value().copied() {
            self.emit_descriptor_compatibility_check(
                DescriptorCompatibilityPredicate::from_presence(&descriptor.configurable),
                function,
                |_, function| {
        ...
        if let Some(enumerable_payload_local) = descriptor.enumerable.value().copied() {
            self.emit_descriptor_compatibility_check(
                DescriptorCompatibilityPredicate::from_presence(&descriptor.enumerable),
                function,
                |_, function| {
                    ...
                    function.instruction(&Instruction::LocalGet(existing_descriptor_kind_local));
                    function
                        .instruction(&Instruction::I64Const(DescriptorMask::ENUMERABLE.as_i64()));
                    function.instruction(&Instruction::I64And);
                    function.instruction(&Instruction::I64Const(0));
                    function.instruction(&Instruction::I64Ne);
        ...
        // 10.1.6.3 step 4.c, derived from the same 6.2.6 classification as the
        // ordinary-object path. Each side owns its predicate; a generic
        // descriptor has neither and therefore changes no kind.
        self.emit_array_descriptor_kind_change_rejection(
        ...
        if let Some(writable_payload_local) = descriptor.writable.value().copied() {
            self.emit_descriptor_compatibility_check(
                DescriptorCompatibilityPredicate::from_presence(&descriptor.writable),
                function,
                |_, function| {
                    function.instruction(&Instruction::LocalGet(existing_descriptor_kind_local));
                    function.instruction(&Instruction::I64Const(DescriptorMask::WRITABLE.as_i64()));
        ...
        if let Some(value) = descriptor.value.value().copied() {
            self.emit_descriptor_compatibility_check(
                DescriptorCompatibilityPredicate::from_presence(&descriptor.value),
                function,
                |builder, function| {
        ...
        for (field, existing) in [
            (descriptor.get, stored.getter.0),
            (descriptor.set, stored.setter.0),
        ] {
            if let Some(value) = field.value().copied() {
                self.emit_descriptor_compatibility_check(
    ...
    /// 10.1.6.3 step 4.c for the array named-property path.
    ///
    /// The incoming side predicate comes from [`classify`] through
    /// [`KindTerms`], rather than from the old `requested_data_descriptor:
    /// bool` plus a second hand-written OR over four presence locals. Calling
    /// this once per side makes `Generic` the case where neither call emits a
    /// rejection and keeps the two kind-change obligations independent.
    fn emit_array_descriptor_kind_change_rejection(
        ...
        self.emit_descriptor_compatibility_check(
            DescriptorCompatibilityPredicate::from_kind_terms(terms),
            function,
            |_, function| {
                function.instruction(&Instruction::LocalGet(existing_descriptor_kind_local));
                function.instruction(&Instruction::I64Const(DescriptorMask::ACCESSOR.as_i64()));
        ...
        while property_index < properties.len() {
            ...
            match property {
                ...
                ObjectPropertyIr::Getter {
                    ...
                    let accessors = match setter_locals {
                        Some((setter_payload, setter_tag)) => {
                            AccessorDescriptorLocals::GetterAndSetter {
                                getter: AccessorGetterLocals::new(TaggedLocals::new(
                                    getter_payload,
                        ...
                        None => AccessorDescriptorLocals::Getter(AccessorGetterLocals::new(
                            TaggedLocals::new(getter_payload, getter_tag),
                        )),
                    };
                ...
                ObjectPropertyIr::Setter {
                    ...
                    self.emit_object_define_enumerable_accessor(
                        ...
                        AccessorDescriptorLocals::Setter(AccessorSetterLocals::new(
                            TaggedLocals::new(setter_payload, setter_tag),
                        )),
                        function,
                    )?;
                    self.release_temp_local(setter_tag);
        ...
        match target.kind {
                    ...
                    || matches!(key, PropertyKeyIr::StaticString(name) if name == "length")
                {
                    ...
                    self.emit_typed_array_witness(
                        ...
                        TypedArrayWitnessUse::Accessor {
                            kind: TypedArrayAccessorKind::Length,
                            result_local: typed_array_length_local,
                        },
                        function,
                    )?;
            ...
            ValueKind::Array => match key {
                ...
                PropertyKeyIr::StringExpr(_) => {
                    ...
                    // `crates/lila-cli/tests/cli/throw_propagation.rs`.
                    self.emit_object_read_with_key_tag(
                        target_local,
                        target_tag_local,
                        target_local,
    ...
    fn emit_typed_array_element_read_from_locals(
        ...
        let index_valid_local = self.reserve_temp_local();
        ...
        self.emit_typed_array_witness(
            ...
            TypedArrayWitnessUse::IntegerIndexedProperty {
                index_local,
                result_local: index_valid_local,
            },
            function,
        )?;
        function.instruction(&Instruction::LocalGet(index_valid_local));
        function.instruction(&Instruction::I64Const(0));
        function.instruction(&Instruction::I64Ne);
        function.instruction(&Instruction::If(BlockType::Empty));
        ...
        for kind in TypedArrayElementKind::ALL {
            ...
            match kind {
                TypedArrayElementKind::Float16 => {
                    let half_local = self.reserve_temp_local();
                    ...
                    let float_bits_local = self.reserve_temp_local();
                    let normalized_exponent_local = self.reserve_temp_local();
                    ...
                    self.emit_half_bits_to_f64_payload(
                        ...
                        fraction_local,
                        float_bits_local,
                        normalized_exponent_local,
                        function,
                    );
                    function.instruction(&Instruction::I64ReinterpretF64);
                    function.instruction(&Instruction::LocalSet(payload_local));
                    self.release_temp_local(normalized_exponent_local);
                    self.release_temp_local(float_bits_local);
                    self.release_temp_local(fraction_local);
                    self.release_temp_local(exponent_local);
                    self.release_temp_local(sign_local);
                    self.release_temp_local(half_local);
                }
                TypedArrayElementKind::Float32 | TypedArrayElementKind::Float64 => {
                    if kind == TypedArrayElementKind::Float32 {
                        self.emit_array_buffer_memory_load(
                            buffer_flags_local,
                            ValType::F32,
                ...
                | TypedArrayElementKind::Uint32 => {
                    let (private_load, shared_load, signed) = match kind {
                        TypedArrayElementKind::Int8 => (
                        ...
                        | TypedArrayElementKind::BigUint64 => unreachable!("integer element arm"),
                    };
                    self.emit_array_buffer_memory_load(
                        buffer_flags_local,
                        ValType::I32,
                ...
                TypedArrayElementKind::BigInt64 | TypedArrayElementKind::BigUint64 => {
                    ...
                    function.instruction(&Instruction::I64Const(ValueKind::BigInt.tag() as i64));
                    function.instruction(&Instruction::LocalSet(tag_local));
                    if kind == TypedArrayElementKind::BigUint64 {
                        function.instruction(&Instruction::LocalGet(payload_local));
                        function.instruction(&Instruction::I64Const(0));
        ...
        for _ in TypedArrayElementKind::ALL {
            function.instruction(&Instruction::End);
        }
        function.instruction(&Instruction::End);
    ...
    fn emit_indexed_element_write_dispatch(
        ...
        if self.outline_indexed_element_write {
            if let Some(helper) = self.indexed_element_write_helper_function_index() {
                ...
                {
                    return Err(EmitError::unsupported(format!(
                        "unsupported in lila wasm-aot first slice: standard builtin `{}` reached the indexed-element write composite, which has no realm-environment parameter",
                        builtin.debug_name()
                    )));
                }
        ...
        match target.kind {
            ...
            ValueKind::Arguments => {
                ...
                if matches!(key, PropertyKeyIr::StaticString(name) if name == "length") {
                    self.emit_arguments_length_write(
                        target_local,
                        payload_local,
                        tag_local,
                        function,
    ...
    pub(crate) fn emit_validate_typed_array_from_constructed_target(
        ...
        let target_view = TypedArrayViewLocals::new(
            target_payload_local,
            buffer_payload_local,
            byte_offset_local,
            stored_byte_length_local,
    ...
    pub(crate) fn emit_integer_typed_array_value_i64(
        ...
        self.emit_to_uint32_i64_from_number_payload(number_payload_local, uint32_local, function);
        ...
        for kind in TypedArrayElementKind::ALL {
            ...
            match kind {
                TypedArrayElementKind::Float16 => {
                    ...
                    function.instruction(&Instruction::I32WrapI64);
                    function
                        .instruction(&Instruction::I32Store16(Self::memarg16_in(memory_index, 0)));
                ...
                TypedArrayElementKind::Float32 | TypedArrayElementKind::Float64 => {
                    ...
                    if kind == TypedArrayElementKind::Float32 {
                        ...
                        function.instruction(&Instruction::F32Store(Self::memarg32_in(
                            memory_index,
                            0,
                        )));
                    } else {
                        function.instruction(&Instruction::F64Store(Self::memarg64_in(
                            memory_index,
                            0,
                        )));
                    }
                ...
                TypedArrayElementKind::Uint8Clamped => {
                    ...
                    function.instruction(&Instruction::End);
                    function
                        .instruction(&Instruction::I32Store8(Self::memarg8_in(memory_index, 0)));
                }
                TypedArrayElementKind::Int8
                | TypedArrayElementKind::Int16
                | TypedArrayElementKind::Int32
                ...
                | TypedArrayElementKind::Uint32 => {
                    ...
                    let store = match kind {
                        TypedArrayElementKind::Int8 | TypedArrayElementKind::Uint8 => {
                            Instruction::I32Store8(Self::memarg8_in(memory_index, 0))
                        }
                        TypedArrayElementKind::Int16 | TypedArrayElementKind::Uint16 => {
                            Instruction::I32Store16(Self::memarg16_in(memory_index, 0))
                        }
                        TypedArrayElementKind::Int32 | TypedArrayElementKind::Uint32 => {
                            Instruction::I32Store(Self::memarg32_in(memory_index, 0))
                        }
                        TypedArrayElementKind::Float16
                        | TypedArrayElementKind::Float32
                        | TypedArrayElementKind::Float64
                        ...
                        | TypedArrayElementKind::BigUint64 => {
                            unreachable!("integer element arm")
                        }
                    };
                ...
                TypedArrayElementKind::BigInt64 | TypedArrayElementKind::BigUint64 => {
                    ...
                    function.instruction(&Instruction::LocalGet(number_payload_local));
                    function
                        .instruction(&Instruction::I64Store(Self::memarg64_in(memory_index, 0)));
                }
            }
            function.instruction(&Instruction::Else);
        }
    ...
    pub(crate) fn compile_array_index_to_local(
        ...
        let PropertyKeyIr::ArrayIndex(expr) = key else {
            self.release_temp_local(index_local);
            return Err(EmitError::unsupported(
                "unsupported in lila wasm-aot first slice: array index kind",
            ));
        };
        self.compile_expr_payload(expr, function)?;
        function.instruction(&Instruction::LocalSet(index_local));
    ...
    pub(crate) fn emit_object_read_with_key_tag(
        ...
        let trap_result = self.emit_normal_proxy_get_trap_result(
            PendingProxyGetTrapResultLocals::new(payload_local, tag_local),
            function,
        );
    ...
    fn emit_proxy_get_invariant_check(
        ...
        let descriptor = self.reserve_proxy_get_descriptor_locals();
        self.emit_direct_own_descriptor_for_proxy_get(target, key, descriptor, function)?;
        ...
        function.instruction(&Instruction::I32Eqz);
        descriptor.getter.emit_undefined_i32(function);
        function.instruction(&Instruction::I32And);
        trap_result.emit_undefined_i32(function);
        function.instruction(&Instruction::I32Eqz);
        function.instruction(&Instruction::I32And);
        function.instruction(&Instruction::If(BlockType::Empty));
        self.emit_throw_runtime_error(
        ...
        function.instruction(&Instruction::I32And);
        function.instruction(&Instruction::If(BlockType::Empty));
        self.emit_proxy_get_descriptor_same_value_i32(
            &trap_result,
        ...
        self.release_proxy_get_descriptor_locals(descriptor);
        Ok(())
    }

    pub(crate) fn reserve_own_descriptor_fact_locals(&mut self) -> OwnDescriptorFactLocals {
        let present = self.reserve_temp_local();
        let descriptor = self.reserve_temp_local();
        OwnDescriptorFactLocals::new(present, descriptor)
    }

    pub(crate) fn release_own_descriptor_fact_locals(&mut self, fact: OwnDescriptorFactLocals) {
        self.release_temp_local(fact.descriptor);
        self.release_temp_local(fact.present);
    }

    fn reserve_proxy_get_descriptor_locals(&mut self) -> ProxyGetDescriptorLocals {
        let fact = self.reserve_own_descriptor_fact_locals();
        let data_value = DescriptorDataValueLocals(TaggedLocals::new(
            self.reserve_temp_local(),
            self.reserve_temp_local(),
        ));
        let getter = DescriptorGetterLocals(TaggedLocals::new(
            self.reserve_temp_local(),
            self.reserve_temp_local(),
        ));
        ProxyGetDescriptorLocals::new(fact, data_value, getter)
    }

    fn release_proxy_get_descriptor_locals(&mut self, descriptor: ProxyGetDescriptorLocals) {
        self.release_temp_local(descriptor.getter.0.tag);
        self.release_temp_local(descriptor.getter.0.payload);
    ...
    fn emit_own_descriptor_from_entries(
        ...
        self.load_i64_to_local_from_offset(object_local, pointer_offset, buffer_local, function);
        self.load_i64_to_local_from_offset(object_local, length_offset, len_local, function);
        function.instruction(&Instruction::I64Const(0));
        function.instruction(&Instruction::LocalSet(index_local));
        function.instruction(&Instruction::Block(BlockType::Empty));
        function.instruction(&Instruction::Loop(BlockType::Empty));
        ...
        if let Some(accessor) = projection.accessor() {
            let (accessor, payload_offset, tag_offset) = match accessor {
                DescriptorAccessorProjectionLocals::Getter(getter) => (
                    getter.0,
                    HEAP_OBJECT_GETTER_PAYLOAD_OFFSET,
                    HEAP_OBJECT_GETTER_TAG_OFFSET,
                ),
                DescriptorAccessorProjectionLocals::Setter(setter) => (
                    setter.0,
                    HEAP_OBJECT_SETTER_PAYLOAD_OFFSET,
                    HEAP_OBJECT_SETTER_TAG_OFFSET,
                ),
            ...
            self.load_i64_to_local_from_offset(entry_local, tag_offset, accessor.tag, function);
        }
        function.instruction(&Instruction::Br(2));
        function.instruction(&Instruction::End);
        function.instruction(&Instruction::LocalGet(index_local));
    ...
    /// Emit the allocation-free direct-target part of `[[GetOwnProperty]]`.
    ///
    /// Proxy is a closed branch so it cannot fall through to the Proxy record's
    /// ordinary storage. The recursive Proxy descriptor-record protocol remains
    /// T11 work; this batch shares every direct runtime representation used by
    /// the public descriptor builtin and Proxy invariants.
    ...
    fn emit_direct_own_descriptor_for_proxy_get(
        &mut self,
        target: ProxyTargetLocals,
        key: PropertyKeyLocals,
        descriptor: ProxyGetDescriptorLocals,
        function: &mut Function,
    ) -> Result<(), EmitError> {
        self.emit_direct_own_descriptor(
            target.0.payload,
            ...
            key.0.payload,
            key.0.tag,
            DirectOwnDescriptorProjectionLocals::ProxyGet(descriptor),
            function,
        )
    }
    ...
    fn emit_direct_own_descriptor(
        ...
        if let Some(accessor) = projection.accessor() {
            // Start absent accessors at tagged ECMAScript `undefined`. Storage
            // projections overwrite this only for a present descriptor; the
            // Proxy-Get getter predicate also accepts the older raw-zero form.
            let accessor = match accessor {
                DescriptorAccessorProjectionLocals::Getter(getter) => getter.0,
                DescriptorAccessorProjectionLocals::Setter(setter) => setter.0,
            };
            function.instruction(&Instruction::I64Const(0));
            function.instruction(&Instruction::LocalSet(accessor.payload));
            function.instruction(&Instruction::I64Const(ValueKind::Undefined.tag() as i64));
            function.instruction(&Instruction::LocalSet(accessor.tag));
        }
        function.instruction(&Instruction::I64Const(0));
        function.instruction(&Instruction::LocalSet(handled_local));
        ...
        for branch in ObjectInternalMethodBranch::ORDER.iter().copied() {
            match branch {
                ...
                ObjectInternalMethodBranch::Namespace => {
                    self.emit_is_module_namespace_i32(
                        target_payload_local,
                        target_tag_local,
                        function,
                    );
                    ...
                    if let Some(value) = projection.data_value() {
                        function.instruction(&Instruction::LocalGet(value_payload_local));
                        function.instruction(&Instruction::LocalSet(value.0.payload));
                        function.instruction(&Instruction::LocalGet(value_tag_local));
                        function.instruction(&Instruction::LocalSet(value.0.tag));
                    }
                    function.instruction(&Instruction::End);
                }
                ObjectInternalMethodBranch::IntegerIndexed => {
                    ...
                    self.emit_typed_array_integer_index_validity_i32(
                        ...
                        key_tag_local,
                        fact.present,
                        branch_handled_local,
                        function,
                    )?;
                    function.instruction(&Instruction::LocalGet(branch_handled_local));
                    function.instruction(&Instruction::I64Const(0));
                    function.instruction(&Instruction::I64Ne);
                    function.instruction(&Instruction::If(BlockType::Empty));
                    function.instruction(&Instruction::I64Const(1));
                ...
                ObjectInternalMethodBranch::Array => {
                    ...
                    function.instruction(&Instruction::LocalGet(fact.descriptor));
                    function.instruction(&Instruction::I64Const(0));
                    function.instruction(&Instruction::I64Ne);
                    function.instruction(&Instruction::I64ExtendI32U);
                    function.instruction(&Instruction::LocalSet(fact.present));
                    if let (Some(data_value), Some(accessor)) =
                        (projection.data_value(), projection.accessor())
                    {
                        function.instruction(&Instruction::LocalGet(fact.present));
                        function.instruction(&Instruction::I64Const(0));
                        function.instruction(&Instruction::I64Ne);
                        ...
                        match accessor {
                            DescriptorAccessorProjectionLocals::Getter(getter) => {
                                // Array accessor entries store [[Get]] in the
                                // same raw tagged slot as a data [[Value]]. This
                                // reader does not invoke the getter.
                    ...
                    function.instruction(&Instruction::LocalSet(fact.descriptor));
                    self.emit_array_length_writable_i64(target_payload_local, flag_local, function);
                    function.instruction(&Instruction::LocalGet(flag_local));
                    function.instruction(&Instruction::I64Const(0));
                    function.instruction(&Instruction::I64Ne);
                    function.instruction(&Instruction::If(BlockType::Empty));
                ...
                ObjectInternalMethodBranch::Arguments => {
                    ...
                    for (
                        ...
                        (
                            ...
                            // The callee special slot stores [[Get]] in the
                            // same value pair when its descriptor is accessor.
                            HEAP_ARGUMENTS_CALLEE_VALUE_PAYLOAD_OFFSET,
                            HEAP_ARGUMENTS_CALLEE_VALUE_TAG_OFFSET,
                        ...
                        function.instruction(&Instruction::LocalGet(fact.descriptor));
                        function.instruction(&Instruction::I64Const(
                            ARRAY_DESCRIPTOR_OWN_PROPERTY as i64,
                        ));
                        function.instruction(&Instruction::I64And);
                        function.instruction(&Instruction::I64Const(0));
                        function.instruction(&Instruction::I64Ne);
                            ...
                            (projection.data_value(), projection.accessor())
                        {
                            ...
                            let (accessor, payload_offset, tag_offset) = match accessor {
                                DescriptorAccessorProjectionLocals::Getter(getter) => {
                                    (getter.0, getter_payload_offset, getter_tag_offset)
                                }
                                DescriptorAccessorProjectionLocals::Setter(setter) => {
                                    (setter.0, setter_payload_offset, setter_tag_offset)
                                }
                            };
                            self.load_i64_to_local_from_offset(
                                target_payload_local,
                ...
                ObjectInternalMethodBranch::Ordinary => {
                    ...
                    // Proxy-Get/Set projections also retain their stored value,
                    ...
                    for (target_tag, target_global, key, descriptor) in [
                        ...
                        function.instruction(&Instruction::If(BlockType::Empty));
                        function.instruction(&Instruction::LocalGet(target_tag_local));
                        function.instruction(&Instruction::I64Const(target_tag.tag() as i64));
                        function.instruction(&Instruction::I64Eq);
                        function.instruction(&Instruction::LocalGet(target_payload_local));
                        function.instruction(&Instruction::GlobalGet(target_global));
                        function.instruction(&Instruction::I64Eq);
                        function.instruction(&Instruction::I32And);
                        let (key_tag, key_payload) = if key == "prototype" {
                            (ValueKind::String, self.strings.payload(key))
                        } else {
                            (
                                ValueKind::Symbol,
                                self.strings.property_key_symbol_payload(key),
                            )
                        };
                        function.instruction(&Instruction::LocalGet(key_tag_local));
                        function.instruction(&Instruction::I64Const(key_tag.tag() as i64));
                        function.instruction(&Instruction::I64Eq);
                        function.instruction(&Instruction::I32And);
                        function.instruction(&Instruction::I64Const(key_payload));
                        function.instruction(&Instruction::LocalSet(key_constant_local));
                        self.emit_property_key_payload_equality_i32(
                            key_constant_local,
                            key_payload_local,
        ...
        self.release_temp_local(index_local);
        self.release_temp_local(boxed_kind_local);
        self.release_temp_local(branch_handled_local);
    ...
    pub(crate) fn emit_proxy_own_keys_trap_result(
        ...
        key_payload_local: u32,
        function: &mut Function,
    ) -> Result<ProxyOwnKeysTrapResultLocals, EmitError> {
    ...
    fn emit_proxy_own_keys_validated_snapshot(
        &mut self,
        target: ProxyTargetLocals,
        own_keys_result: ProxyOwnKeysTrapResultLocals,
        list_len_local: u32,
        snapshot_payload_local: u32,
        function: &mut Function,
    ) -> Result<(), EmitError> {
        let target_payload_local = target.0.payload;
        let target_tag_local = target.0.tag;
        let own_keys_payload_local = own_keys_result.0.payload;
        let own_keys_tag_local = own_keys_result.0.tag;
        let index_local = self.reserve_temp_local();
    ...
    /// 6.2.6.5 ToPropertyDescriptor. The returned witness owns the converted
    /// descriptor's temporary locals until a materializer consumes it.
    pub(crate) fn emit_to_property_descriptor(
        ...
        for field in TO_PROPERTY_DESCRIPTOR_ORDER {
            ...
            if matches!(
                ...
                // converted descriptor never carries the original tagged value.
                function.instruction(&Instruction::LocalGet(present_local));
                function.instruction(&Instruction::I64Const(0));
                function.instruction(&Instruction::I64Ne);
                function.instruction(&Instruction::If(BlockType::Empty));
        ...
        let descriptor = PartialDescriptor::<ReservedPropertyDescriptorCarrier> {
            ...
            writable: Presence::Runtime {
                ...
                value: TaggedLocals::new(writable_payload_local, writable_tag_local),
            },
            ...
            enumerable: Presence::Runtime {
                ...
                value: TaggedLocals::new(enumerable_payload_local, enumerable_tag_local),
            },
            configurable: Presence::Runtime {
                present: configurable_present_local,
                value: TaggedLocals::new(configurable_payload_local, configurable_tag_local),
            },
        }
    ...
    /// Materializes a present Property Descriptor as the ordinary object used
    /// by the two Object static builtins. Consuming the witness releases
    /// every local reserved by [`Self::emit_to_property_descriptor`].
    pub(crate) fn emit_from_present_property_descriptor(
        ...
        for field in [set, get, configurable, enumerable, writable, value] {
            match field {
                Presence::Absent => {}
    ...
    pub(crate) fn emit_object_read_ordinary_inner(
        ...
        // Integer-indexed [[Get]] owns canonical numeric keys even when the
        // element is absent. Such a miss must stop this prototype walk.
        let numeric_key_tag_local = self.reserve_temp_local();
        let numeric_index_payload_local = self.reserve_temp_local();
        let integer_index_local = self.reserve_temp_local();
        let valid_index_local = self.reserve_temp_local();
        let integer_indexed_local = self.reserve_temp_local();
        self.emit_property_key_tag_from_payload(key_local, numeric_key_tag_local, function);
        self.emit_typed_array_canonical_numeric_index_i32(
            current_local,
            current_tag_local,
            key_local,
            numeric_key_tag_local,
            numeric_index_payload_local,
            integer_indexed_local,
            function,
        )?;
        function.instruction(&Instruction::LocalGet(integer_indexed_local));
        function.instruction(&Instruction::I64Const(0));
        function.instruction(&Instruction::I64Ne);
        function.instruction(&Instruction::If(BlockType::Empty));
        function.instruction(&Instruction::I64Const(1));
        ...
        function.instruction(&Instruction::Br(1));
        function.instruction(&Instruction::End);
        self.release_temp_local(integer_indexed_local);
        self.release_temp_local(valid_index_local);
        self.release_temp_local(integer_index_local);
        self.release_temp_local(numeric_index_payload_local);
        self.release_temp_local(numeric_key_tag_local);
    ...
    pub(crate) fn emit_object_define_data_with_flag_locals(
        ...
        // A complete data descriptor whose six field presences are known when
        // the compiler runs.
    ...
    pub(crate) fn emit_object_define_accessor(
        ...
        object_local: u32,
        key_local: u32,
        accessors: AccessorDescriptorLocals,
        function: &mut Function,
    ) -> Result<(), EmitError> {
        let configurable_payload_local = self.reserve_temp_local();
        let enumerable_payload_local = self.reserve_temp_local();
    ...
    pub(crate) fn emit_object_define_accessor_with_flag_local(
        ...
        let (get, set) = match accessors {
            AccessorDescriptorLocals::Getter(AccessorGetterLocals(getter)) => {
                (Presence::Present(getter), Presence::Absent)
            }
            AccessorDescriptorLocals::Setter(AccessorSetterLocals(setter)) => {
                (Presence::Absent, Presence::Present(setter))
            }
            AccessorDescriptorLocals::GetterAndSetter {
    ...
    pub(crate) fn emit_object_define_entry_validated(
        ...
        if let Some(configurable_payload_local) = descriptor.configurable.value().copied() {
            self.emit_descriptor_compatibility_check(
                ...
                |builder, function| {
                    ...
                    function.instruction(&Instruction::I64Ne);
                    function.instruction(&Instruction::If(BlockType::Empty));
                    builder.emit_object_mutation_type_error(TYPE_ERROR_NAME, function)?;
        ...
        if descriptor.value.value().is_some() {
            self.emit_descriptor_compatibility_check(
                DescriptorCompatibilityPredicate::from_presence(&descriptor.value),
                function,
                |builder, function| {
                    ...
                    function.instruction(&Instruction::I32And);
                    function.instruction(&Instruction::If(BlockType::Empty));
                    builder.load_i64_to_local_from_offset(
                        entry_local,
                        HEAP_OBJECT_DATA_TAG_OFFSET,
                        getter_tag_local,
                        function,
        ...
        if descriptor.get.value().is_some() {
            self.emit_descriptor_compatibility_check(
                DescriptorCompatibilityPredicate::from_presence(&descriptor.get),
                function,
                |builder, function| {
                    function.instruction(&Instruction::LocalGet(existing_descriptor_kind_local));
                    function.instruction(&Instruction::I64Const(DescriptorMask::ACCESSOR.as_i64()));
                    ...
                    builder.emit_tagged_payload_equality_i32(
                        stored_data_tag_local,
                        stored_data_payload_local,
                        getter_tag_local,
                        getter_payload_local,
        ...
        // Step 4.d, `[[Set]]`.
        if descriptor.set.value().is_some() {
            self.emit_descriptor_compatibility_check(
                DescriptorCompatibilityPredicate::from_presence(&descriptor.set),
                function,
                |builder, function| {
                    function.instruction(&Instruction::LocalGet(existing_descriptor_kind_local));
                    function.instruction(&Instruction::I64Const(DescriptorMask::ACCESSOR.as_i64()));
        ...
        match descriptor.value {
            ...
            Presence::Absent => {}
            // The incoming value was copied into the seeded storage locals
            // before the existing entry was found.
            Presence::Present(_) => {}
            Presence::Runtime {
        ...
        function.instruction(&Instruction::I64Eqz);
        function.instruction(&Instruction::If(BlockType::Empty));
        self.emit_object_mutation_type_error_without_message(function)?;
        self.emit_return_current_completion(function);
        function.instruction(&Instruction::End);
    ...
    ///
    /// That disjunction is `statically_true OR (the run-time terms)`. The
    /// closed compatibility predicate preserves both halves and makes a
    /// statically-true side unconditional.
    ...
    pub(crate) fn emit_create_data_property_or_throw(
        ...
        function.instruction(&Instruction::I64Eqz);
        function.instruction(&Instruction::If(BlockType::Empty));
        self.emit_object_mutation_type_error(non_configurable_message, function)?;
        ...
        function.instruction(&Instruction::I64Eqz);
        function.instruction(&Instruction::If(BlockType::Empty));
        self.emit_object_mutation_type_error(non_extensible_message, function)?;
        if let Some(close) = iterator_close_on_throw {
            self.emit_iterator_close_preserving_current_throw(close, function)?;
        }
        self.emit_return_current_completion(function);
    ...
    pub(crate) fn emit_set_path_realm_environment_argument(&mut self, function: &mut Function) {
        match set_path_realm_environment_argument(self.object_mutation_error_realm_source()) {
    ...
    fn emit_object_mutation_type_error(
        ...
        match object_mutation_error_realm(self.object_mutation_error_realm_source()) {
            ObjectMutationErrorRealm::TrustedCurrentEnvironment => self
                .emit_throw_current_function_realm_type_error(
                    message,
                    self.result_local,
                    self.result_tag_local,
            ...
            ObjectMutationErrorRealm::MainRealmFallback => self.emit_throw_runtime_error(
                TYPE_ERROR_NAME,
                message,
                self.result_local,
                self.result_tag_local,
    ...
    fn emit_object_mutation_type_error_without_message(
        ...
        match object_mutation_error_realm(self.object_mutation_error_realm_source()) {
            ObjectMutationErrorRealm::TrustedCurrentEnvironment => self
                .emit_throw_current_function_realm_type_error_without_message(
                    self.result_local,
                    self.result_tag_local,
                    function,
                ),
            ObjectMutationErrorRealm::MainRealmFallback => self
                .emit_throw_runtime_type_error_without_message(
                    self.result_local,
                    self.result_tag_local,
                    function,
                ),
    ...
    fn emit_object_mutation_type_error_to_active_handler(
        &mut self,
        message: &str,
        function: &mut Function,
    ) -> Result<(), EmitError> {
        self.emit_object_mutation_type_error(message, function)?;
    ...
    pub(crate) fn emit_proxy_set_false_result_throw(
        ...
        self.emit_object_mutation_type_error("Proxy set trap returned false", function)?;
        self.emit_return_current_completion(function);
        Ok(())
    }

    /// Assignment throws for a falsy Proxy `set` result only in strict mode;
    /// internal `Set(..., Throw=true)` callers use the unconditional owner.
    pub(crate) fn emit_assignment_proxy_set_false_result_if_strict(
        &mut self,
        function: &mut Function,
    ) -> Result<(), EmitError> {
        match self.object_write_strict_flag_local {
            Some(strict_local) => {
                ...
                function.instruction(&Instruction::I64Ne);
                function.instruction(&Instruction::If(BlockType::Empty));
                self.emit_proxy_set_false_result_throw(function)?;
                function.instruction(&Instruction::End);
            }
            None => {
                if self.is_current_function_strict() {
    ...
    pub(crate) fn emit_object_write(
        ...
        function.instruction(&Instruction::If(BlockType::Empty));
        function.instruction(&Instruction::Block(BlockType::Empty));
        self.emit_is_module_namespace_i32(object_local, object_tag_local, function);
        function.instruction(&Instruction::If(BlockType::Empty));
        self.emit_namespace_set_rejection(function)?;
        function.instruction(&Instruction::Br(1));
        function.instruction(&Instruction::End);
        ...
        function.instruction(&Instruction::I64Eq);
        function.instruction(&Instruction::If(BlockType::Empty));
        self.emit_object_mutation_type_error("Proxy handler is null", function)?;
        self.emit_return_current_completion(function);
        function.instruction(&Instruction::End);
        self.load_i64_to_local_from_offset(
            object_local,
        ...
        self.emit_is_module_namespace_i32(prototype_local, prototype_tag_local, function);
        function.instruction(&Instruction::If(BlockType::Empty));
        self.emit_namespace_set_rejection(function)?;
        function.instruction(&Instruction::I64Const(1));
        function.instruction(&Instruction::LocalSet(prototype_proxy_set_handled_local));
    ...
    pub(crate) fn emit_ordinary_set_data_on_receiver_result_with_depth(
        ...
        self.emit_is_module_namespace_i32(receiver_payload_local, receiver_tag_local, function);
        function.instruction(&Instruction::If(BlockType::Empty));
        self.emit_namespace_property(
            receiver_payload_local,
            key_local,
        ...
        self.emit_arguments_special_property_receiver_set(
            TaggedLocals::new(receiver_payload_local, receiver_tag_local),
            key_local,
            TaggedLocals::new(value_payload_local, value_tag_local),
            found_local,
    ...
    fn emit_ordinary_set_result_via_selected_helper(
        ...
        let helper_result_tag_local = self.reserve_temp_local();
        self.emit_set_path_realm_environment_argument(function);
        function.instruction(&Instruction::LocalSet(realm_environment_local));
        function.instruction(&Instruction::I64Const(ValueKind::Number.tag() as i64));
    ...
    pub(crate) fn emit_ordinary_set_result_with_receiver_fallback(
        ...
        function.instruction(&Instruction::I64Eqz);
        function.instruction(&Instruction::If(BlockType::Empty));
        self.emit_arguments_special_property_set(
            TaggedLocals::new(current_payload_local, current_tag_local),
            TaggedLocals::new(receiver_payload_local, receiver_tag_local),
    ...
    pub(crate) fn emit_object_delete(
        ...
        function.instruction(&Instruction::LocalGet(object_tag_local));
        function.instruction(&Instruction::LocalSet(current_tag_local));
        function.instruction(&Instruction::I64Const(INSPECT_CURRENT_TARGET));
        function.instruction(&Instruction::LocalSet(traversal_state_local));
        self.emit_property_key_tag_from_payload(key_local, key_tag_local, function);
        self.emit_property_key_value_payload_to_local(key_local, trap_key_payload_local, function);
        ...
        function.instruction(&Instruction::I64Const(DELETE_COMPLETE));
        function.instruction(&Instruction::LocalSet(traversal_state_local));
        function.instruction(&Instruction::Else);
        function.instruction(&Instruction::LocalGet(trap_tag_local));
        function.instruction(&Instruction::I64Const(ValueKind::Undefined.tag() as i64));
        ...
        function.instruction(&Instruction::LocalGet(target_tag_local));
        function.instruction(&Instruction::LocalSet(current_tag_local));
        function.instruction(&Instruction::I64Const(FOLLOW_PROXY_TARGET));
        function.instruction(&Instruction::LocalSet(traversal_state_local));
        function.instruction(&Instruction::Else);
        self.emit_throw_runtime_error(
            TYPE_ERROR_NAME,
        ...
        function.instruction(&Instruction::LocalGet(traversal_state_local));
        function.instruction(&Instruction::I64Const(FOLLOW_PROXY_TARGET));
        function.instruction(&Instruction::I64Eq);
        function.instruction(&Instruction::If(BlockType::Empty));
        function.instruction(&Instruction::I64Const(INSPECT_CURRENT_TARGET));
        ...
        self.release_temp_local(target_tag_local);
        self.release_temp_local(target_payload_local);
        self.release_temp_local(traversal_state_local);
        self.release_temp_local(current_tag_local);
        self.release_temp_local(current_payload_local);
        Ok(())
    }
    ...
    pub(crate) fn emit_proxy_set_invariant_check(
        ...
        function.instruction(&Instruction::I32And);
        function.instruction(&Instruction::If(BlockType::Empty));
        self.emit_object_mutation_type_error(
    ...
    pub(crate) fn emit_ordinary_prevent_extensions_i32(
        ...
        function.instruction(&Instruction::I64Ne);
        function.instruction(&Instruction::LocalGet(typed_array_buffer_flags_local));
        function.instruction(&Instruction::I64Const(ArrayBufferFlag::Shared.word() as i64));
        function.instruction(&Instruction::I64And);
        function.instruction(&Instruction::I64Eqz);
        function.instruction(&Instruction::I32And);
        function.instruction(&Instruction::If(BlockType::Empty));
