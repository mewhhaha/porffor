...
impl<'a> FunctionBuilder<'a> {
    pub(crate) fn install_object_constructor_intrinsics(
        ...
        let proto_getter_meta = self
            ...
            .ok_or_else(|| {
                EmitError::unsupported(
                    "unsupported in lila wasm-aot first slice: missing builtin meta `get Object.prototype.__proto__`",
                )
            })?;
        let proto_setter_meta = self
            ...
            .ok_or_else(|| {
                EmitError::unsupported(
                    "unsupported in lila wasm-aot first slice: missing builtin meta `set Object.prototype.__proto__`",
                )
            })?;
