...
impl<'a> FunctionBuilder<'a> {
    pub(crate) fn install_iterator_constructor_intrinsics(
        ...
        let constructor_getter_meta = self
            ...
            .ok_or_else(|| {
                EmitError::unsupported(
                    "unsupported in lila wasm-aot first slice: missing builtin meta `get %IteratorPrototype%.constructor`",
                )
            })?;
        let constructor_setter_meta = self
            ...
            .ok_or_else(|| {
                EmitError::unsupported(
                    "unsupported in lila wasm-aot first slice: missing builtin meta `set %IteratorPrototype%.constructor`",
                )
            })?;
        let from_meta = self
            .functions
        ...
        let iterator_meta = self
            ...
            .ok_or_else(|| {
                EmitError::unsupported(
                    "unsupported in lila wasm-aot first slice: missing builtin meta `%IteratorPrototype%[Symbol.iterator]`",
                )
            })?;
        let dispose_meta = self
            ...
            .ok_or_else(|| {
                EmitError::unsupported(
                    "unsupported in lila wasm-aot first slice: missing builtin meta `%IteratorPrototype%[Symbol.dispose]`",
                )
            })?;
        let to_string_tag_getter_meta = self
            ...
            .ok_or_else(|| {
                EmitError::unsupported(
                    "unsupported in lila wasm-aot first slice: missing builtin meta `get %IteratorPrototype%[Symbol.toStringTag]`",
                )
            })?;
        let to_string_tag_setter_meta = self
            ...
            .ok_or_else(|| {
                EmitError::unsupported(
                    "unsupported in lila wasm-aot first slice: missing builtin meta `set %IteratorPrototype%[Symbol.toStringTag]`",
                )
            })?;
