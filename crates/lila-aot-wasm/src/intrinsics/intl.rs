...
impl<'a> FunctionBuilder<'a> {
        ...
        let to_string_meta = self
            ...
            .ok_or_else(|| {
                EmitError::unsupported(
                    "unsupported in lila wasm-aot first slice: missing builtin meta `Intl.Locale.prototype.toString`",
                )
            })?;
    ...
    pub(crate) fn install_intl_date_time_format_constructor_intrinsics(
        ...
        let supported_locales_of_meta = self
            ...
            .ok_or_else(|| {
                EmitError::unsupported(
                    "unsupported in lila wasm-aot first slice: missing builtin meta `Intl.DateTimeFormat.supportedLocalesOf`",
                )
            })?;
        self.emit_object_define_function_data(
            object_local,
