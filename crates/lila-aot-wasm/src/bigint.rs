...
pub(crate) const BIGINT_SHIFT_RESOURCE_MESSAGE: &str =
    "BigInt shift result exceeds the engine resource limit";

const DIGIT_MASK: i64 = 0xffff_ffff;
// `emit_bigint_digits_alloc` reserves one extra 8-byte slot. This is the
// largest digit count whose byte range is representable by memory32 without
// wrapping an address or an allocation size.
...
impl<'a> FunctionBuilder<'a> {
    ...
    /// Applies BigInt bitwise complement through the existing exact
    /// infinite-two's-complement XOR implementation: `~x` is `x ^ -1n`.
    ///
    /// The synthetic right operand is already numeric, so this helper performs
    /// no second evaluation or coercion and preserves the unary operator's
    /// single `ToNumeric` boundary.
    pub(crate) fn emit_bigint_complement_to_locals(
        ...
        let minus_one_payload_local = self.reserve_temp_local();
        let minus_one_tag_local = self.reserve_temp_local();
        function.instruction(&Instruction::I64Const(-1));
        function.instruction(&Instruction::LocalSet(minus_one_payload_local));
        function.instruction(&Instruction::I64Const(ValueKind::BigInt.tag() as i64));
        function.instruction(&Instruction::LocalSet(minus_one_tag_local));
        self.emit_bigint_binary_op_to_locals(
            BigIntHelperOp::BitXor,
            operand_payload_local,
            operand_tag_local,
            minus_one_payload_local,
            minus_one_tag_local,
            out_payload_local,
            out_tag_local,
            function,
        )?;
        self.release_temp_local(minus_one_tag_local);
        self.release_temp_local(minus_one_payload_local);
        Ok(())
    }
    ...
    /// Applies `&`, `|` or `^` using the infinite two's-complement BigInt
    /// model. A single extra base-2^32 digit carries the sign extension, so a
    /// finite working width is exact for every pair of finite operands.
    #[allow(clippy::too_many_arguments)]
    fn emit_bigint_bitwise(
        ...
        let index = self.reserve_temp_local();
        let lhs_digit = self.reserve_temp_local();
        let rhs_digit = self.reserve_temp_local();
        let lhs_carry = self.reserve_temp_local();
        let rhs_carry = self.reserve_temp_local();
        let digit = self.reserve_temp_local();
        let carry = self.reserve_temp_local();
        ...
        function.instruction(&Instruction::I64Const(0));
        function.instruction(&Instruction::End);
        function.instruction(&Instruction::LocalSet(lhs_carry));
        function.instruction(&Instruction::LocalGet(rhs_sign));
        function.instruction(&Instruction::I64Const(0));
        function.instruction(&Instruction::I64LtS);
        function.instruction(&Instruction::If(BlockType::Result(ValType::I64)));
        ...
        function.instruction(&Instruction::I64Const(0));
        function.instruction(&Instruction::End);
        function.instruction(&Instruction::LocalSet(rhs_carry));
        ...
        function.instruction(&Instruction::LocalGet(lhs_carry));
        function.instruction(&Instruction::I64Add);
        function.instruction(&Instruction::LocalSet(lhs_digit));
        function.instruction(&Instruction::LocalGet(lhs_digit));
        function.instruction(&Instruction::I64Const(32));
        function.instruction(&Instruction::I64ShrU);
        function.instruction(&Instruction::LocalSet(lhs_carry));
        ...
        function.instruction(&Instruction::I64Const(DIGIT_MASK));
        function.instruction(&Instruction::I64Xor);
        function.instruction(&Instruction::LocalGet(rhs_carry));
        function.instruction(&Instruction::I64Add);
        function.instruction(&Instruction::LocalSet(rhs_digit));
        function.instruction(&Instruction::LocalGet(rhs_digit));
        function.instruction(&Instruction::I64Const(32));
        function.instruction(&Instruction::I64ShrU);
        function.instruction(&Instruction::LocalSet(rhs_carry));
        ...
        self.release_temp_local(carry);
        self.release_temp_local(digit);
        self.release_temp_local(rhs_carry);
        self.release_temp_local(lhs_carry);
        self.release_temp_local(rhs_digit);
        self.release_temp_local(lhs_digit);
        self.release_temp_local(index);
        Ok(())
    }

    /// Applies signed BigInt shifts. A negative shift count reverses the
    /// direction; its magnitude is never narrowed or masked.
    #[allow(clippy::too_many_arguments)]
    fn emit_bigint_shift(
        ...
        self.emit_bigint_shift_left(
            lhs_sign, lhs_ptr, lhs_len, rhs_ptr, rhs_len, res_sign, res_ptr, res_len, function,
        )?;
        function.instruction(&Instruction::Else);
    ...
    fn emit_bigint_shift_left(
        ...
        let word_shift = self.reserve_temp_local();
        let bit_shift = self.reserve_temp_local();
        let index = self.reserve_temp_local();
        let dest_index = self.reserve_temp_local();
        let digit = self.reserve_temp_local();
        let high = self.reserve_temp_local();
        let carry = self.reserve_temp_local();
        ...
        // More than 64 count bits necessarily exceeds memory32's result
        ...
        function.instruction(&Instruction::I64ShrU);
        function.instruction(&Instruction::LocalSet(word_shift));
        function.instruction(&Instruction::LocalGet(count));
        function.instruction(&Instruction::I64Const(31));
        function.instruction(&Instruction::I64And);
        function.instruction(&Instruction::LocalSet(bit_shift));

        function.instruction(&Instruction::LocalGet(word_shift));
        function.instruction(&Instruction::I64Const(MAX_BIGINT_SHIFT_DIGITS));
        function.instruction(&Instruction::I64GtU);
        function.instruction(&Instruction::If(BlockType::Empty));
        self.emit_throw_runtime_error(
            RANGE_ERROR_NAME,
        ...
        // The digit count may fit memory32 in isolation but not above the
        // current heap cursor. Reject that address-space exhaustion before
        // the allocator can wrap/trap.
        ...
        function.instruction(&Instruction::LocalGet(digit));
        function.instruction(&Instruction::LocalGet(bit_shift));
        function.instruction(&Instruction::I64Shl);
        function.instruction(&Instruction::LocalSet(high));
        function.instruction(&Instruction::LocalGet(high));
        function.instruction(&Instruction::LocalGet(carry));
        ...
        function.instruction(&Instruction::LocalGet(word_shift));
        function.instruction(&Instruction::I64Add);
        function.instruction(&Instruction::LocalSet(dest_index));
        ...
        self.release_temp_local(high);
        self.release_temp_local(digit);
        self.release_temp_local(dest_index);
        self.release_temp_local(index);
        self.release_temp_local(bit_shift);
        self.release_temp_local(word_shift);
        self.release_temp_local(count);
        Ok(())
    }
    ...
    fn emit_bigint_shift_right(
        ...
        let index = self.reserve_temp_local();
        let source_index = self.reserve_temp_local();
        let digit = self.reserve_temp_local();
        let high = self.reserve_temp_local();
        let discarded = self.reserve_temp_local();
        let carry = self.reserve_temp_local();
        ...
        self.emit_bigint_saturated_right_shift(
            lhs_sign, lhs_ptr, res_sign, res_ptr, res_len, function,
        )?;
        function.instruction(&Instruction::Else);
        ...
        function.instruction(&Instruction::LocalGet(base_len));
        function.instruction(&Instruction::I64Const(1));
        function.instruction(&Instruction::I64Add);
        function.instruction(&Instruction::LocalSet(res_len));
        self.emit_bigint_digits_alloc(res_len, res_ptr, function)?;
        ...
        function.instruction(&Instruction::I64Const(0));
        function.instruction(&Instruction::LocalSet(discarded));
        function.instruction(&Instruction::I64Const(0));
        function.instruction(&Instruction::LocalSet(index));
        function.instruction(&Instruction::Block(BlockType::Empty));
        function.instruction(&Instruction::Loop(BlockType::Empty));
        ...
        function.instruction(&Instruction::BrIf(1));
        self.emit_bigint_digit_load(lhs_ptr, index, digit, function);
        function.instruction(&Instruction::LocalGet(discarded));
        function.instruction(&Instruction::LocalGet(digit));
        function.instruction(&Instruction::I64Or);
        function.instruction(&Instruction::LocalSet(discarded));
        function.instruction(&Instruction::LocalGet(index));
        ...
        function.instruction(&Instruction::LocalGet(word_shift));
        function.instruction(&Instruction::I64Add);
        function.instruction(&Instruction::LocalSet(source_index));
        self.emit_bigint_digit_load(lhs_ptr, source_index, digit, function);
        function.instruction(&Instruction::LocalGet(digit));
        function.instruction(&Instruction::LocalGet(bit_shift));
        function.instruction(&Instruction::I64ShrU);
        function.instruction(&Instruction::LocalSet(digit));
        function.instruction(&Instruction::LocalGet(source_index));
        function.instruction(&Instruction::I64Const(1));
        function.instruction(&Instruction::I64Add);
        function.instruction(&Instruction::LocalSet(source_index));
        ...
        self.release_temp_local(carry);
        self.release_temp_local(discarded);
        self.release_temp_local(high);
        self.release_temp_local(digit);
        self.release_temp_local(source_index);
        self.release_temp_local(index);
    ...
    /// Produces the arithmetic-right-shift limit: zero for non-negative lhs,
    /// minus one for negative lhs.
