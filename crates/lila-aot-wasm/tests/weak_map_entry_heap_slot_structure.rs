...
const LAYOUT_SOURCE: &str = include_str!("../src/heap_weak_map_entry_layout.rs");
const WEAK_EDGE_SOURCE: &str = include_str!("../src/heap_weak_edges.rs");
const HEAP_SOURCE: &str = include_str!("../src/heap.rs");
const LIB_SOURCE: &str = include_str!("../src/lib.rs");
const CONTRACT: &str =
    include_str!("../../../docs/rust-rewrite/contracts/weak-map-entry-heap-slot-authority.md");
...
#[test]
fn weak_map_entry_heap_slot_is_the_exact_capability_free_domain() {
    ...
    assert!(
        !LAYOUT_SOURCE.lines().any(|line| {
            line.trim_start().starts_with("impl ") && line.contains(" for WeakMapEntryHeapSlot")
        }),
    ...
    for capability in [
        ...
        assert!(
            !LAYOUT_SOURCE.contains(&format!("impl {capability} for WeakMapEntryHeapSlot")),
...
#[test]
fn one_exhaustive_projection_owns_five_non_pointer_rows() {
    ...
    assert_eq!(implementation.matches("pointer: false").count(), 5);
    ...
    for row in [
        concat!(
            "Self::Present=>WeakMapEntryHeapSlotMetadata{record:\"weak-map-entry\",",
            "name:\"present\",offset:HEAP_WEAK_MAP_ENTRY_PRESENT_OFFSET,",
            "width:8,pointer:false,},"
        ),
        concat!(
            "Self::KeyTag=>WeakMapEntryHeapSlotMetadata{record:\"weak-map-entry\",",
            "name:\"key_tag\",offset:HEAP_WEAK_MAP_ENTRY_KEY_TAG_OFFSET,",
            "width:8,pointer:false,},"
        ),
        concat!(
            "Self::KeyPayload=>WeakMapEntryHeapSlotMetadata{record:\"weak-map-entry\",",
            "name:\"key_payload\",offset:HEAP_WEAK_MAP_ENTRY_KEY_PAYLOAD_OFFSET,",
            "width:8,pointer:false,},"
        ),
        concat!(
            "Self::ValueTag=>WeakMapEntryHeapSlotMetadata{record:\"weak-map-entry\",",
            "name:\"value_tag\",offset:HEAP_WEAK_MAP_ENTRY_VALUE_TAG_OFFSET,",
            "width:8,pointer:false,},"
        ),
        concat!(
            "Self::ValuePayload=>WeakMapEntryHeapSlotMetadata{record:\"weak-map-entry\",",
            "name:\"value_payload\",offset:HEAP_WEAK_MAP_ENTRY_VALUE_PAYLOAD_OFFSET,",
            "width:8,pointer:false,},"
        ),
    ] {
...
#[test]
fn typed_registry_and_weak_edges_preserve_ephemeron_authority() {
    ...
    assert_eq!(
        registry,
        concat!(
            ":&[WeakMapEntryHeapSlot]=&[",
            "WeakMapEntryHeapSlot::Present,WeakMapEntryHeapSlot::KeyTag,",
            "WeakMapEntryHeapSlot::KeyPayload,WeakMapEntryHeapSlot::ValueTag,",
...
#[test]
fn weak_map_entry_layout_has_one_private_recursive_owner() {
    assert_eq!(
        LIB_SOURCE
    ...
    assert!(!LIB_SOURCE.contains("\npub mod heap_weak_map_entry_layout;\n"));
    assert!(!HEAP_SOURCE.contains("record: \"weak-map-entry\""));

    let source_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("src");
    assert_eq!(
        recursive_rust_source_count(&source_root, "record: \"weak-map-entry\""),
        7
    );
    assert_eq!(
        recursive_rust_source_count(&source_root, "pub(crate) enum WeakMapEntryHeapSlot {"),
        1
    );
    assert!(CONTRACT.contains("WeakMapEntryHeapSlot"));
    assert!(TASK.contains("weak-map-entry-heap-slot-authority.md"));
}
