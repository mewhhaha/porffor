...
const LAYOUT_SOURCE: &str = include_str!("../src/heap_promise_reaction_layout.rs");
const HEAP_SOURCE: &str = include_str!("../src/heap.rs");
const LIB_SOURCE: &str = include_str!("../src/lib.rs");
const CONTRACT: &str =
    include_str!("../../../docs/rust-rewrite/contracts/promise-reaction-heap-slot-authority.md");
...
#[test]
fn promise_reaction_heap_slot_is_the_exact_capability_free_domain() {
    ...
    assert!(
        !LAYOUT_SOURCE.lines().any(|line| {
            line.trim_start().starts_with("impl ") && line.contains(" for PromiseReactionHeapSlot")
        }),
    ...
    for capability in [
        ...
        assert!(
            !LAYOUT_SOURCE.contains(&format!("impl {capability} for PromiseReactionHeapSlot")),
...
fn one_exhaustive_projection_owns_seven_exact_rows_and_retention_classes() {
    let implementation = bounded(
        ...
        "pub(crate) const HEAP_PROMISE_REACTION_LAYOUT",
    );
    ...
    for row in [
        concat!(
            ...
            "record:\"promise-reaction-record\",name:\"capability\",",
            "offset:HEAP_PROMISE_REACTION_CAPABILITY_OFFSET,width:8,pointer:true,},"
        ),
        concat!(
            "Self::HandlerTag=>PromiseReactionHeapSlotMetadata{",
            "record:\"promise-reaction-record\",name:\"handler_tag\",",
            "offset:HEAP_PROMISE_REACTION_HANDLER_TAG_OFFSET,width:8,pointer:false,},"
        ),
        concat!(
            "Self::HandlerPayload=>PromiseReactionHeapSlotMetadata{",
            "record:\"promise-reaction-record\",name:\"handler_payload\",",
            "offset:HEAP_PROMISE_REACTION_HANDLER_PAYLOAD_OFFSET,width:8,pointer:true,},"
        ),
        concat!(
            "Self::Realm=>PromiseReactionHeapSlotMetadata{",
            "record:\"promise-reaction-record\",name:\"realm\",",
            "offset:HEAP_PROMISE_REACTION_REALM_OFFSET,width:8,pointer:true,},"
        ),
        concat!(
            "Self::Next=>PromiseReactionHeapSlotMetadata{",
            "record:\"promise-reaction-record\",name:\"next\",",
            "offset:HEAP_PROMISE_REACTION_NEXT_OFFSET,width:8,pointer:true,},"
        ),
        concat!(
            "Self::Type=>PromiseReactionHeapSlotMetadata{",
            "record:\"promise-reaction-record\",name:\"type\",",
            "offset:HEAP_PROMISE_REACTION_TYPE_OFFSET,width:8,pointer:false,},"
        ),
        concat!(
            "Self::CallbackKind=>PromiseReactionHeapSlotMetadata{",
            "record:\"promise-reaction-record\",name:\"callback_kind\",",
            "offset:HEAP_PROMISE_REACTION_CALLBACK_KIND_OFFSET,width:8,pointer:false,},"
        ),
    ] {
...
#[test]
fn typed_registry_preserves_capability_handler_realm_next_type_callback_order() {
...
#[test]
fn promise_reaction_layout_has_one_private_recursive_owner() {
    assert_eq!(
        LIB_SOURCE
    ...
    assert!(!LIB_SOURCE.contains("\npub mod heap_promise_reaction_layout;\n"));
    assert!(!HEAP_SOURCE.contains("record: \"promise-reaction-record\""));

    let source_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("src");
    assert_eq!(
        recursive_rust_source_count(&source_root, "record: \"promise-reaction-record\""),
        7
    );
    assert_eq!(
        recursive_rust_source_count(&source_root, "pub(crate) enum PromiseReactionHeapSlot {"),
        1
    );
    assert!(CONTRACT.contains("PromiseReactionHeapSlot"));
    assert!(TASK.contains("promise-reaction-heap-slot-authority.md"));
}
