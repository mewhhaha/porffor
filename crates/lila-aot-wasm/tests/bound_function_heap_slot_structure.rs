...
const LAYOUT_SOURCE: &str = include_str!("../src/heap_bound_function_layout.rs");
const HEAP_SOURCE: &str = include_str!("../src/heap.rs");
const LIB_SOURCE: &str = include_str!("../src/lib.rs");
const CONTRACT: &str =
    include_str!("../../../docs/rust-rewrite/contracts/bound-function-heap-slot-authority.md");
...
#[test]
fn bound_function_heap_slot_is_the_exact_capability_free_domain() {
    ...
    assert!(
        !LAYOUT_SOURCE.lines().any(|line| {
            line.trim_start().starts_with("impl ") && line.contains(" for BoundFunctionHeapSlot")
        }),
    ...
    for capability in [
        ...
        assert!(
            !LAYOUT_SOURCE.contains(&format!("impl {capability} for BoundFunctionHeapSlot")),
...
#[test]
fn one_exhaustive_projection_owns_six_exact_rows() {
    let implementation = bounded(
        LAYOUT_SOURCE,
        "impl BoundFunctionHeapSlot {",
        "pub(crate) const HEAP_BOUND_FUNCTION_LAYOUT",
    );
    ...
    for row in [
        concat!(
            ...
            "record:\"bound-function\",name:\"target_payload\",",
            "offset:HEAP_BOUND_FUNCTION_TARGET_PAYLOAD_OFFSET,width:8,pointer:true,},"
        ),
        concat!(
            "Self::TargetTag=>BoundFunctionHeapSlotMetadata{",
            "record:\"bound-function\",name:\"target_tag\",",
            "offset:HEAP_BOUND_FUNCTION_TARGET_TAG_OFFSET,width:8,pointer:false,},"
        ),
        concat!(
            "Self::ThisPayload=>BoundFunctionHeapSlotMetadata{",
            "record:\"bound-function\",name:\"this_payload\",",
            "offset:HEAP_BOUND_FUNCTION_THIS_PAYLOAD_OFFSET,width:8,pointer:true,},"
        ),
        concat!(
            "Self::ThisTag=>BoundFunctionHeapSlotMetadata{",
            "record:\"bound-function\",name:\"this_tag\",",
            "offset:HEAP_BOUND_FUNCTION_THIS_TAG_OFFSET,width:8,pointer:false,},"
        ),
        concat!(
            "Self::ArgumentsPayload=>BoundFunctionHeapSlotMetadata{",
            "record:\"bound-function\",name:\"args_payload\",",
            "offset:HEAP_BOUND_FUNCTION_ARGS_PAYLOAD_OFFSET,width:8,pointer:true,},"
        ),
        concat!(
            "Self::SelfPayload=>BoundFunctionHeapSlotMetadata{",
            "record:\"bound-function\",name:\"self_payload\",",
            "offset:HEAP_BOUND_FUNCTION_SELF_PAYLOAD_OFFSET,width:8,pointer:true,},"
        ),
    ] {
...
#[test]
fn typed_registry_preserves_payload_first_inventory_order() {
...
#[test]
fn bound_function_layout_has_one_private_recursive_owner() {
    assert_eq!(
        LIB_SOURCE
    ...
    assert!(!LIB_SOURCE.contains("\npub mod heap_bound_function_layout;\n"));
    assert!(!HEAP_SOURCE.contains("record: \"bound-function\""));

    let source_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("src");
    assert_eq!(
        recursive_rust_source_count(&source_root, "record: \"bound-function\""),
        6
    );
    assert_eq!(
        recursive_rust_source_count(&source_root, "pub(crate) enum BoundFunctionHeapSlot {"),
        1
    );
    assert!(CONTRACT.contains("BoundFunctionHeapSlot"));
    assert!(TASK.contains("bound-function-heap-slot-authority.md"));
}
