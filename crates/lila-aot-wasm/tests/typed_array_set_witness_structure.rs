...
fn set_compiler() -> &'static str {
    SET_SOURCE
        .split_once("    pub(super) fn compile_typed_array_prototype_set_builtin(")
        .expect("missing TypedArray.prototype.set compiler")
        .1
        .split_once("    pub(super) fn emit_typed_array_copy_bytes_in_order(")
...
fn set_uses_two_receiver_witnesses_and_one_typed_source_witness() {
    ...
    assert!(
        receiver_view < witnesses[0]
            ...
            && buffer_addresses[0] < buffer_addresses[1]
    );
}
...
fn typed_source_overlap_is_staged_before_target_writes() {
    ...
    let byte_copies = positions(body, "emit_typed_array_copy_bytes_in_order(");
    ...
    assert!(
        source_witnesses[2] < allocations[0]
            ...
            && byte_copies[2] < allocations[1]
            && allocations[1] < reads[0]
