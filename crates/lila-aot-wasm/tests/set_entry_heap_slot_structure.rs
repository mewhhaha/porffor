...
const LAYOUT_SOURCE: &str = include_str!("../src/heap_set_entry_layout.rs");
const HEAP_SOURCE: &str = include_str!("../src/heap.rs");
const LIB_SOURCE: &str = include_str!("../src/lib.rs");
const CONTRACT: &str =
    include_str!("../../../docs/rust-rewrite/contracts/set-entry-heap-slot-authority.md");
...
#[test]
fn set_entry_heap_slot_is_the_exact_capability_free_domain() {
    let declaration = bounded(LAYOUT_SOURCE, "pub(crate) enum SetEntryHeapSlot {", "\n}");
    assert_eq!(
        declaration
            .lines()
            .map(str::trim)
    ...
    assert!(
        !LAYOUT_SOURCE.lines().any(|line| {
            line.trim_start().starts_with("impl ") && line.contains(" for SetEntryHeapSlot")
        }),
    ...
    for capability in [
        ...
        assert!(
            !LAYOUT_SOURCE.contains(&format!("impl {capability} for SetEntryHeapSlot")),
...
fn one_exhaustive_projection_owns_three_exact_rows() {
    let implementation = bounded(
        ...
        "pub(crate) const HEAP_SET_ENTRY_LAYOUT",
    );
    ...
    for row in [
        concat!(
            "Self::Present=>SetEntryHeapSlotMetadata{record:\"set-entry\",",
            "name:\"present\",offset:HEAP_SET_ENTRY_PRESENT_OFFSET,",
            "width:8,pointer:false,},"
        ),
        concat!(
            "Self::ValueTag=>SetEntryHeapSlotMetadata{record:\"set-entry\",",
            "name:\"value_tag\",offset:HEAP_SET_ENTRY_VALUE_TAG_OFFSET,",
            "width:8,pointer:false,},"
        ),
        concat!(
            "Self::ValuePayload=>SetEntryHeapSlotMetadata{record:\"set-entry\",",
            "name:\"value_payload\",offset:HEAP_SET_ENTRY_VALUE_PAYLOAD_OFFSET,",
...
#[test]
fn typed_registry_preserves_present_tag_payload_order() {
    ...
    assert_eq!(
        ...
        concat!(
            ":&[SetEntryHeapSlot]=&[",
            "SetEntryHeapSlot::Present,SetEntryHeapSlot::ValueTag,",
...
#[test]
fn set_entry_layout_has_one_private_recursive_owner() {
    assert_eq!(
        LIB_SOURCE.matches("\nmod heap_set_entry_layout;\n").count(),
        1
    );
    assert!(!LIB_SOURCE.contains("\npub mod heap_set_entry_layout;\n"));
    assert!(!HEAP_SOURCE.contains("record: \"set-entry\""));

    let source_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("src");
    assert_eq!(
        recursive_rust_source_count(&source_root, "record: \"set-entry\""),
        3
    );
    assert_eq!(
        recursive_rust_source_count(&source_root, "pub(crate) enum SetEntryHeapSlot {"),
        1
    );
    assert!(CONTRACT.contains("SetEntryHeapSlot"));
    assert!(TASK.contains("set-entry-heap-slot-authority.md"));
}
