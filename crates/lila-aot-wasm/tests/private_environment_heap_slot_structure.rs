...
#[test]
fn private_environment_heap_slot_is_the_exact_capability_free_domain() {
    ...
    assert!(
        !LAYOUT_SOURCE.lines().any(|line| {
            line.trim_start().starts_with("impl ")
                && line.contains(" for PrivateEnvironmentHeapSlot")
        }),
    ...
    for capability in [
        ...
        assert!(
            !LAYOUT_SOURCE.contains(&format!("impl {capability} for PrivateEnvironmentHeapSlot")),
...
#[test]
fn one_exhaustive_projection_owns_parent_and_class_scope_rows() {
    let implementation = bounded(
        LAYOUT_SOURCE,
        "impl PrivateEnvironmentHeapSlot {",
        "pub(crate) const HEAP_PRIVATE_ENV_LAYOUT",
    );
    ...
    for row in [
        concat!(
            ...
            "record:\"private-environment\",name:\"parent\",",
            "offset:HEAP_PRIVATE_ENV_PARENT_OFFSET,width:8,pointer:true,},"
        ),
        concat!(
            "Self::ClassScope=>PrivateEnvironmentHeapSlotMetadata{",
            "record:\"private-environment\",name:\"class_scope\",",
            "offset:HEAP_PRIVATE_ENV_CLASS_SCOPE_OFFSET,width:8,pointer:false,},"
        ),
    ] {
...
#[test]
fn typed_registry_preserves_parent_then_class_scope_order() {
...
#[test]
fn private_environment_layout_has_one_private_recursive_owner() {
    assert_eq!(
        LIB_SOURCE
    ...
    assert!(!LIB_SOURCE.contains("\npub mod heap_private_environment_layout;\n"));
    assert!(!HEAP_SOURCE.contains("record: \"private-environment\""));

    let source_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("src");
    assert_eq!(
        recursive_rust_source_count(&source_root, "record: \"private-environment\""),
        2
    );
    assert_eq!(
        recursive_rust_source_count(&source_root, "pub(crate) enum PrivateEnvironmentHeapSlot {"),
        1
    );
