...
const LAYOUT_SOURCE: &str = include_str!("../src/heap_environment_layout.rs");
...
#[test]
fn environment_heap_slot_is_the_exact_capability_free_domain() {
    ...
    assert!(
        !LAYOUT_SOURCE.lines().any(|line| {
            line.trim_start().starts_with("impl ") && line.contains(" for EnvironmentHeapSlot")
        }),
    ...
    for capability in [
        ...
        assert!(
            !LAYOUT_SOURCE.contains(&format!("impl {capability} for EnvironmentHeapSlot")),
...
fn one_exhaustive_projection_owns_three_exact_rows() {
    ...
    for row in [
        ...
        concat!(
            ...
            "name:\"tag\",offset:ENV_SLOT_TAG_OFFSET,width:8,pointer:false,},"
        ),
        concat!(
            "Self::BindingPayload=>EnvironmentHeapSlotMetadata{",
            "record:\"environment-slot\",name:\"payload\",",
            "offset:ENV_SLOT_PAYLOAD_OFFSET,width:8,pointer:true,},"
        ),
    ] {
...
#[test]
fn environment_layout_has_one_private_recursive_owner() {
    assert_eq!(
        LIB_SOURCE
    ...
    assert!(!LIB_SOURCE.contains("\npub mod heap_environment_layout;\n"));
    assert!(!HEAP_SOURCE.contains("record: \"environment\""));
    assert!(!HEAP_SOURCE.contains("record: \"environment-slot\""));
    ...
    assert_eq!(
        recursive_rust_source_count(&source_root, "pub(crate) enum EnvironmentHeapSlot {"),
        1
    );
