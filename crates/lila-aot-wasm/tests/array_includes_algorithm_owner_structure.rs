...
const ARGUMENT_FIXTURE: &str =
    include_str!("../../lila-cli/tests/fixtures/wasm_array_includes_argument_evaluation.js");
...
#[test]
fn direct_includes_branch_delegates_with_the_complete_argument_list() {
    let direct = bounded(
        FUNCTIONS_SOURCE,
    ...
    assert_eq!(
        direct
            .matches("StandardBuiltinId::ArrayPrototypeIncludes,")
            .count(),
        1
    );
    assert_eq!(direct.matches("\"Array.prototype.includes\",").count(), 1);
    assert_eq!(direct.matches("                args,").count(), 1);

    for forbidden in [
        "emit_array_includes_method_call(",
        "args.first()",
        "args.get(1)",
        "compile_expr_to_locals(",
        "emit_array_includes_from_locals(",
    ] {
        assert!(
            !direct.contains(forbidden),
            "direct includes branch must not retain `{forbidden}`"
        );
    }
}

#[test]
fn removed_direct_includes_owner_cannot_be_called() {
    ...
    assert_eq!(
        rust_source
            .matches("emit_array_includes_method_call")
            .count(),
        0
    );
    assert_eq!(
        rust_source
            .matches("fn compile_array_prototype_includes_builtin(")
            .count(),
        1
    );
    assert_eq!(
        ARRAY_SOURCE
            .matches("self.emit_array_includes_from_locals(")
            .count(),
        1
    );
    ...
    assert_eq!(
        standard_arm
            .matches("self.compile_array_prototype_includes_builtin(function)?;")
            .count(),
        1
    );
}
...
#[test]
fn shared_call_boundary_and_canonical_entry_own_argument_order() {
    ...
    for (earlier, later) in [
        ...
        ("self.strings.payload(\"length\")", "self.emit_object_read("),
        (
...
#[test]
fn focused_fixture_observes_ignored_argument_and_spread_evaluation() {
    for marker in [
    ...
    assert!(ARRAY_CLI_TESTS
        .contains("fn run_wasm_backend_evaluates_all_array_includes_arguments_before_search()"));
