...
const LAYOUT_SOURCE: &str = include_str!("../src/heap_temporal_instant_layout.rs");
const HEAP_SOURCE: &str = include_str!("../src/heap.rs");
const LIB_SOURCE: &str = include_str!("../src/lib.rs");
const CONTRACT: &str =
    include_str!("../../../docs/rust-rewrite/contracts/temporal-instant-heap-slot-authority.md");
...
#[test]
fn temporal_instant_heap_slot_is_the_exact_capability_free_domain() {
    ...
    assert_eq!(
        declaration
            ...
            .collect::<Vec<_>>(),
        ["EpochNanosecondsTag,", "EpochNanosecondsPayload,"],
    );
    assert!(!LAYOUT_SOURCE.contains("#[derive"));
    assert!(
        !LAYOUT_SOURCE.lines().any(|line| {
            line.trim_start().starts_with("impl ") && line.contains(" for TemporalInstantHeapSlot")
        }),
    ...
    for capability in [
        ...
        assert!(
            !LAYOUT_SOURCE.contains(&format!("impl {capability} for TemporalInstantHeapSlot")),
...
#[test]
fn one_exhaustive_projection_owns_both_exact_layout_rows() {
    let implementation = bounded(
        LAYOUT_SOURCE,
        "impl TemporalInstantHeapSlot {",
        "pub(crate) const HEAP_TEMPORAL_INSTANT_RECORD_LAYOUT",
    );
...
#[test]
fn typed_registry_preserves_tag_then_payload_order() {
...
#[test]
fn temporal_instant_layout_has_one_private_recursive_owner() {
    assert_eq!(
        LIB_SOURCE
    ...
    assert!(!LIB_SOURCE.contains("\npub mod heap_temporal_instant_layout;\n"));
    assert!(!HEAP_SOURCE.contains("record: \"temporal-instant-record\""));

    let source_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("src");
    assert_eq!(
        recursive_rust_source_count(&source_root, "record: \"temporal-instant-record\""),
        2
    );
    assert_eq!(
        recursive_rust_source_count(&source_root, "pub(crate) enum TemporalInstantHeapSlot {"),
        1
    );
    assert!(CONTRACT.contains("TemporalInstantHeapSlot"));
    assert!(TASK.contains("temporal-instant-heap-slot-authority.md"));
}
