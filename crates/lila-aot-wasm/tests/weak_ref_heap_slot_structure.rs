...
const LAYOUT_SOURCE: &str = include_str!("../src/heap_weak_ref_layout.rs");
const WEAK_EDGE_SOURCE: &str = include_str!("../src/heap_weak_edges.rs");
const HEAP_SOURCE: &str = include_str!("../src/heap.rs");
const LIB_SOURCE: &str = include_str!("../src/lib.rs");
const CONTRACT: &str =
    include_str!("../../../docs/rust-rewrite/contracts/weak-ref-heap-slot-authority.md");
...
#[test]
fn weak_ref_heap_slot_is_the_exact_capability_free_domain() {
    let declaration = bounded(LAYOUT_SOURCE, "pub(crate) enum WeakRefHeapSlot {", "\n}");
    assert_eq!(
        declaration
            .lines()
            .map(str::trim)
            ...
            .collect::<Vec<_>>(),
        ["TargetTag,", "TargetPayload,"],
    );
    assert!(!LAYOUT_SOURCE.contains("#[derive"));
    assert!(
        !LAYOUT_SOURCE.lines().any(|line| {
            line.trim_start().starts_with("impl ") && line.contains(" for WeakRefHeapSlot")
        }),
    ...
    for capability in [
        ...
        assert!(
            !LAYOUT_SOURCE.contains(&format!("impl {capability} for WeakRefHeapSlot")),
...
#[test]
fn one_exhaustive_projection_owns_two_non_pointer_rows() {
    let implementation = bounded(
        LAYOUT_SOURCE,
        "impl WeakRefHeapSlot {",
        "pub(crate) const HEAP_WEAK_REF_RECORD_LAYOUT",
    );
    ...
    for row in [
        concat!(
            ...
            "record:\"weak-ref-record\",name:\"target_tag\",",
            "offset:HEAP_WEAK_REF_TARGET_TAG_OFFSET,width:8,pointer:false,},"
        ),
        concat!(
            "Self::TargetPayload=>WeakRefHeapSlotMetadata{",
            "record:\"weak-ref-record\",name:\"target_payload\",",
            "offset:HEAP_WEAK_REF_TARGET_PAYLOAD_OFFSET,width:8,pointer:false,},"
        ),
    ] {
...
fn typed_registry_and_weak_edge_authority_preserve_non_retention() {
    ...
    assert!(weak_edges.contains(concat!(
        ...
        "record:\"weak-ref-record\",name:\"target\",",
...
#[test]
fn weak_ref_layout_has_one_private_recursive_owner() {
    assert_eq!(
        LIB_SOURCE.matches("\nmod heap_weak_ref_layout;\n").count(),
        1
    );
    assert!(!LIB_SOURCE.contains("\npub mod heap_weak_ref_layout;\n"));
    assert!(!HEAP_SOURCE.contains("record: \"weak-ref-record\""));

    let source_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("src");
    assert_eq!(
        recursive_rust_source_count(&source_root, "record: \"weak-ref-record\""),
        3
    );
    assert_eq!(
        recursive_rust_source_count(&source_root, "pub(crate) enum WeakRefHeapSlot {"),
        1
    );
    assert!(CONTRACT.contains("WeakRefHeapSlot"));
    assert!(TASK.contains("weak-ref-heap-slot-authority.md"));
}
