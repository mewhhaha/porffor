...
const LAYOUT_SOURCE: &str = include_str!("../src/heap_temporal_plain_date_layout.rs");
const HEAP_SOURCE: &str = include_str!("../src/heap.rs");
const LIB_SOURCE: &str = include_str!("../src/lib.rs");
const CONTRACT: &str =
    include_str!("../../../docs/rust-rewrite/contracts/temporal-plain-date-heap-slot-authority.md");
...
#[test]
fn temporal_plain_date_heap_slot_is_the_exact_capability_free_domain() {
    ...
    for capability in [
        ...
        assert!(
            !LAYOUT_SOURCE.contains(&format!("impl {capability} for TemporalPlainDateHeapSlot")),
...
fn one_exhaustive_projection_owns_four_exact_rows() {
    let implementation = bounded(
        ...
        "pub(crate) const HEAP_TEMPORAL_PLAIN_DATE_RECORD_LAYOUT",
    );
    ...
    for row in [
        concat!(
            ...
            "record:\"temporal-plain-date-record\",name:\"iso_year\",",
            "offset:HEAP_TEMPORAL_PLAIN_DATE_ISO_YEAR_OFFSET,width:8,pointer:false,},"
        ),
        concat!(
            "Self::IsoMonth=>TemporalPlainDateHeapSlotMetadata{",
            "record:\"temporal-plain-date-record\",name:\"iso_month\",",
            "offset:HEAP_TEMPORAL_PLAIN_DATE_ISO_MONTH_OFFSET,width:8,pointer:false,},"
        ),
        concat!(
            "Self::IsoDay=>TemporalPlainDateHeapSlotMetadata{",
            "record:\"temporal-plain-date-record\",name:\"iso_day\",",
            "offset:HEAP_TEMPORAL_PLAIN_DATE_ISO_DAY_OFFSET,width:8,pointer:false,},"
        ),
        concat!(
            "Self::CalendarPayload=>TemporalPlainDateHeapSlotMetadata{",
            "record:\"temporal-plain-date-record\",name:\"calendar_payload\",",
...
#[test]
fn typed_registry_preserves_year_month_day_calendar_order() {
    ...
    assert_eq!(
        registry,
        concat!(
            ":&[TemporalPlainDateHeapSlot]=&[",
            "TemporalPlainDateHeapSlot::IsoYear,TemporalPlainDateHeapSlot::IsoMonth,",
...
#[test]
fn temporal_plain_date_layout_has_one_private_recursive_owner() {
    assert_eq!(
        LIB_SOURCE
    ...
    assert!(!LIB_SOURCE.contains("\npub mod heap_temporal_plain_date_layout;\n"));
    assert!(!HEAP_SOURCE.contains("record: \"temporal-plain-date-record\""));

    let source_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("src");
    assert_eq!(
        recursive_rust_source_count(&source_root, "record: \"temporal-plain-date-record\""),
        4
    );
    assert_eq!(
        recursive_rust_source_count(&source_root, "pub(crate) enum TemporalPlainDateHeapSlot {"),
        1
    );
    assert!(CONTRACT.contains("TemporalPlainDateHeapSlot"));
    assert!(TASK.contains("temporal-plain-date-heap-slot-authority.md"));
}
