...
#[test]
fn find_last_index_fallback_delegates_without_changing_strict_typed_array_dispatch() {
    let direct = bounded(
        FUNCTIONS_SOURCE,
    ...
    for marker in [
        "let receiver_has_typed_array_find_last_index =",
    ...
    assert!(!direct.contains("emit_array_find_last_index_method_call"));
}

#[test]
fn removed_array_find_last_index_wrapper_cannot_be_called() {
    ...
    assert_eq!(
        rust_source
            .matches("emit_array_find_last_index_method_call")
            .count(),
        0
    );
    assert_eq!(
    ...
    assert!(!FIND_STRUCTURE_GUARD.contains("emit_array_find_last_index_method_call"));
    ...
    assert_eq!(
        standard
            .matches("FindViaPredicateKind::FindLastIndex")
            .count(),
        1
    );
}
...
#[test]
fn shared_call_boundary_and_canonical_find_last_index_compiler_own_order() {
...
#[test]
fn focused_find_last_index_control_covers_reverse_generic_and_proxy_callbacks() {
    for marker in [
        ...
        "proxyOrder[0] === 2",
        "proxySource.findLastIndex(nonCallableProxy)",
        "proxySource.findLastIndex(revokedCallableProxy.proxy)",
    ] {
        assert!(
            FIND_LAST_FIXTURE.contains(marker),
            "missing FindLastIndex marker `{marker}`"
        );
    }
