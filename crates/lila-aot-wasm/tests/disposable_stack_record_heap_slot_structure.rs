...
const LAYOUT_SOURCE: &str = include_str!("../src/heap_disposable_stack_record_layout.rs");
const HEAP_SOURCE: &str = include_str!("../src/heap.rs");
const LIB_SOURCE: &str = include_str!("../src/lib.rs");
const CONTRACT: &str = include_str!(
    "../../../docs/rust-rewrite/contracts/disposable-stack-record-heap-slot-authority.md"
);
...
#[test]
fn disposable_stack_record_heap_slot_is_the_exact_capability_free_domain() {
    ...
    assert!(
        !LAYOUT_SOURCE.lines().any(|line| {
            line.trim_start().starts_with("impl ")
                && line.contains(" for DisposableStackRecordHeapSlot")
        }),
    ...
    for capability in [
        ...
        assert!(
            !LAYOUT_SOURCE.contains(&format!(
                "impl {capability} for DisposableStackRecordHeapSlot"
            )),
...
fn one_exhaustive_projection_owns_four_exact_rows() {
    let implementation = bounded(
        ...
        "pub(crate) const HEAP_DISPOSABLE_STACK_RECORD_LAYOUT",
    );
    ...
    for row in [
        concat!(
            ...
            "record:\"disposable-stack-record\",name:\"state\",",
            "offset:HEAP_DISPOSABLE_STACK_STATE_OFFSET,width:8,pointer:false,},"
        ),
        concat!(
            "Self::EntriesPointer=>DisposableStackRecordHeapSlotMetadata{",
            "record:\"disposable-stack-record\",name:\"entries_ptr\",",
            "offset:HEAP_DISPOSABLE_STACK_ENTRIES_PTR_OFFSET,width:8,pointer:true,},"
        ),
        concat!(
            "Self::EntriesLength=>DisposableStackRecordHeapSlotMetadata{",
            "record:\"disposable-stack-record\",name:\"entries_len\",",
            "offset:HEAP_DISPOSABLE_STACK_ENTRIES_LEN_OFFSET,width:8,pointer:false,},"
        ),
        concat!(
            "Self::EntriesCapacity=>DisposableStackRecordHeapSlotMetadata{",
            "record:\"disposable-stack-record\",name:\"entries_cap\",",
            "offset:HEAP_DISPOSABLE_STACK_ENTRIES_CAP_OFFSET,width:8,pointer:false,},"
        ),
    ] {
...
#[test]
fn disposable_stack_record_layout_has_one_private_recursive_owner() {
    assert_eq!(
        LIB_SOURCE
    ...
    assert!(!LIB_SOURCE.contains("\npub mod heap_disposable_stack_record_layout;\n"));
    assert!(!HEAP_SOURCE.contains("record: \"disposable-stack-record\""));

    let source_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("src");
    assert_eq!(
        recursive_rust_source_count(&source_root, "record: \"disposable-stack-record\""),
        4
    );
    assert_eq!(
    ...
    assert!(CONTRACT.contains("DisposableStackRecordHeapSlot"));
    assert!(TASK.contains("disposable-stack-record-heap-slot-authority.md"));
}
