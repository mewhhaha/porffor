...
const ARGUMENT_FIXTURE: &str =
    include_str!("../../lila-cli/tests/fixtures/wasm_array_index_of_argument_evaluation.js");
...
#[test]
fn direct_index_of_branch_delegates_with_the_complete_argument_list() {
    let direct = bounded(
        FUNCTIONS_SOURCE,
    ...
    assert_eq!(
        direct
            .matches("StandardBuiltinId::ArrayPrototypeIndexOf,")
            .count(),
        1
    );
    assert_eq!(direct.matches("\"Array.prototype.indexOf\",").count(), 1);
    assert_eq!(direct.matches("                args,").count(), 1);

    for forbidden in [
        "emit_array_index_of_method_call(",
        "args.first()",
        "args.get(1)",
        "compile_expr_to_locals(",
        "emit_array_index_of_from_locals(",
    ] {
        assert!(
            !direct.contains(forbidden),
            "direct indexOf branch must not retain `{forbidden}`"
        );
    }
}

#[test]
fn removed_direct_index_of_owner_cannot_be_called() {
    ...
    assert_eq!(
        rust_source
            .matches("emit_array_index_of_method_call")
            .count(),
        0
    );
    assert_eq!(
        rust_source
            .matches("fn compile_array_prototype_index_of_builtin(")
            .count(),
        1
    );
    assert_eq!(
        ARRAY_SOURCE
            .matches("self.emit_array_index_of_from_locals(")
            .count(),
        1
    );
    ...
    assert_eq!(
        standard_arm
            .matches("self.compile_array_prototype_index_of_builtin(function)?;")
            .count(),
        1
    );
}
...
#[test]
fn shared_call_boundary_and_canonical_entry_own_argument_and_search_order() {
...
#[test]
fn focused_fixture_observes_all_arguments_before_index_search() {
    for marker in [
    ...
    assert!(ARRAY_CLI_TESTS
        .contains("fn run_wasm_backend_evaluates_all_array_index_of_arguments_before_search()"));
    assert!(TYPED_ARRAY_CLI_TESTS
        .contains("fn run_wasm_backend_succeeds_for_typedarray_search_fixture()"));
}
