...
const LAYOUT_SOURCE: &str = include_str!("../src/heap_private_element_entry_layout.rs");
...
#[test]
fn private_element_entry_heap_slot_is_the_exact_capability_free_domain() {
    ...
    assert!(
        !LAYOUT_SOURCE.lines().any(|line| {
            line.trim_start().starts_with("impl ")
                && line.contains(" for PrivateElementEntryHeapSlot")
        }),
    ...
    for capability in [
        ...
        assert!(
            !LAYOUT_SOURCE.contains(&format!(
                "impl {capability} for PrivateElementEntryHeapSlot"
            )),
...
fn one_exhaustive_projection_owns_six_exact_rows_and_retention_classes() {
    let implementation = bounded(
        ...
        "pub(crate) const HEAP_PRIVATE_ELEMENT_ENTRY_LAYOUT",
    );
    ...
    for row in [
        concat!(
            ...
            "record:\"private-element-entry\",name:\"next\",",
            "offset:HEAP_PRIVATE_ELEMENT_ENTRY_NEXT_OFFSET,width:8,pointer:true,},"
        ),
        concat!(
            "Self::Receiver=>PrivateElementEntryHeapSlotMetadata{",
            "record:\"private-element-entry\",name:\"receiver\",",
            "offset:HEAP_PRIVATE_ELEMENT_ENTRY_RECEIVER_OFFSET,width:8,pointer:true,},"
        ),
        concat!(
            "Self::Token=>PrivateElementEntryHeapSlotMetadata{",
            "record:\"private-element-entry\",name:\"token\",",
            "offset:HEAP_PRIVATE_ELEMENT_ENTRY_TOKEN_OFFSET,width:8,pointer:true,},"
        ),
        concat!(
            "Self::Kind=>PrivateElementEntryHeapSlotMetadata{",
            "record:\"private-element-entry\",name:\"kind\",",
            "offset:HEAP_PRIVATE_ELEMENT_ENTRY_KIND_OFFSET,width:8,pointer:false,},"
        ),
        concat!(
            "Self::ValueTag=>PrivateElementEntryHeapSlotMetadata{",
            "record:\"private-element-entry\",name:\"value_tag\",",
            "offset:HEAP_PRIVATE_ELEMENT_ENTRY_VALUE_TAG_OFFSET,width:8,pointer:false,},"
        ),
        concat!(
            "Self::ValuePayload=>PrivateElementEntryHeapSlotMetadata{",
            "record:\"private-element-entry\",name:\"value_payload\",",
            "offset:HEAP_PRIVATE_ELEMENT_ENTRY_VALUE_PAYLOAD_OFFSET,width:8,pointer:true,},"
        ),
    ] {
...
#[test]
fn typed_registry_preserves_link_identity_kind_and_value_order() {
...
#[test]
fn private_element_entry_layout_has_one_private_recursive_owner() {
    assert_eq!(
        LIB_SOURCE
    ...
    assert!(!LIB_SOURCE.contains("\npub mod heap_private_element_entry_layout;\n"));
    assert!(!HEAP_SOURCE.contains("record: \"private-element-entry\""));

    let source_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("src");
    assert_eq!(
        recursive_rust_source_count(&source_root, "record: \"private-element-entry\""),
        6
    );
    assert_eq!(
