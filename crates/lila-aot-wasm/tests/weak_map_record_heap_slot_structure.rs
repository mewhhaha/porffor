...
const LAYOUT_SOURCE: &str = include_str!("../src/heap_weak_map_record_layout.rs");
const ENTRY_LAYOUT_SOURCE: &str = include_str!("../src/heap_weak_map_entry_layout.rs");
const WEAK_EDGE_SOURCE: &str = include_str!("../src/heap_weak_edges.rs");
const HEAP_SOURCE: &str = include_str!("../src/heap.rs");
const LIB_SOURCE: &str = include_str!("../src/lib.rs");
const CONTRACT: &str =
    include_str!("../../../docs/rust-rewrite/contracts/weak-map-record-heap-slot-authority.md");
...
#[test]
fn weak_map_record_heap_slot_is_the_exact_capability_free_domain() {
    ...
    assert!(
        !LAYOUT_SOURCE.lines().any(|line| {
            line.trim_start().starts_with("impl ") && line.contains(" for WeakMapRecordHeapSlot")
        }),
    ...
    for capability in [
        ...
        assert!(
            !LAYOUT_SOURCE.contains(&format!("impl {capability} for WeakMapRecordHeapSlot")),
...
fn one_exhaustive_projection_owns_four_exact_rows() {
    let implementation = bounded(
        ...
        "pub(crate) const HEAP_WEAK_MAP_RECORD_LAYOUT",
    );
    ...
    for row in [
        concat!(
            ...
            "record:\"weak-map-record\",name:\"entries_ptr\",",
            "offset:HEAP_WEAK_MAP_ENTRIES_PTR_OFFSET,width:8,pointer:true,},"
        ),
        concat!(
            "Self::EntriesLength=>WeakMapRecordHeapSlotMetadata{",
            "record:\"weak-map-record\",name:\"entries_len\",",
            "offset:HEAP_WEAK_MAP_ENTRIES_LEN_OFFSET,width:8,pointer:false,},"
        ),
        concat!(
            "Self::EntriesCapacity=>WeakMapRecordHeapSlotMetadata{",
            "record:\"weak-map-record\",name:\"entries_cap\",",
            "offset:HEAP_WEAK_MAP_ENTRIES_CAP_OFFSET,width:8,pointer:false,},"
        ),
        concat!(
            "Self::LiveCount=>WeakMapRecordHeapSlotMetadata{",
            "record:\"weak-map-record\",name:\"live_count\",",
            "offset:HEAP_WEAK_MAP_LIVE_COUNT_OFFSET,width:8,pointer:false,},"
        ),
    ] {
...
#[test]
fn typed_registry_separates_storage_reachability_from_ephemeron_retention() {
    ...
    assert_eq!(
        registry,
        concat!(
            ":&[WeakMapRecordHeapSlot]=&[",
            "WeakMapRecordHeapSlot::EntriesPointer,WeakMapRecordHeapSlot::EntriesLength,",
            "WeakMapRecordHeapSlot::EntriesCapacity,WeakMapRecordHeapSlot::LiveCount,"
        )
    );
    ...
    assert_eq!(entry_implementation.matches("pointer: false").count(), 5);
...
#[test]
fn weak_map_record_layout_has_one_private_recursive_owner() {
    assert_eq!(
        LIB_SOURCE
    ...
    assert!(!LIB_SOURCE.contains("\npub mod heap_weak_map_record_layout;\n"));
    assert!(!HEAP_SOURCE.contains("record: \"weak-map-record\""));

    let source_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("src");
    assert_eq!(
        recursive_rust_source_count(&source_root, "record: \"weak-map-record\""),
        4
    );
    assert_eq!(
        recursive_rust_source_count(&source_root, "pub(crate) enum WeakMapRecordHeapSlot {"),
        1
    );
    assert!(CONTRACT.contains("WeakMapRecordHeapSlot"));
    assert!(TASK.contains("weak-map-record-heap-slot-authority.md"));
}
