...
const LAYOUT_SOURCE: &str = include_str!("../src/heap_disposable_stack_entry_layout.rs");
const HEAP_SOURCE: &str = include_str!("../src/heap.rs");
const LIB_SOURCE: &str = include_str!("../src/lib.rs");
const CONTRACT: &str = include_str!(
    "../../../docs/rust-rewrite/contracts/disposable-stack-entry-heap-slot-authority.md"
);
...
#[test]
fn disposable_stack_entry_heap_slot_is_the_exact_capability_free_domain() {
    ...
    assert!(
        !LAYOUT_SOURCE.lines().any(|line| {
            line.trim_start().starts_with("impl ")
                && line.contains(" for DisposableStackEntryHeapSlot")
        }),
    ...
    for capability in [
        ...
        assert!(
            !LAYOUT_SOURCE.contains(&format!(
                "impl {capability} for DisposableStackEntryHeapSlot"
            )),
...
fn one_exhaustive_projection_owns_five_exact_rows() {
    let implementation = bounded(
        ...
        "pub(crate) const HEAP_DISPOSABLE_STACK_ENTRY_LAYOUT",
    );
    ...
    for row in [
        concat!(
            ...
            "record:\"disposable-stack-entry\",name:\"kind\",",
            "offset:HEAP_DISPOSABLE_STACK_ENTRY_KIND_OFFSET,width:8,pointer:false,},"
        ),
        concat!(
            "Self::ValueTag=>DisposableStackEntryHeapSlotMetadata{",
            "record:\"disposable-stack-entry\",name:\"value_tag\",",
            "offset:HEAP_DISPOSABLE_STACK_ENTRY_VALUE_TAG_OFFSET,width:8,pointer:false,},"
        ),
        concat!(
            "Self::ValuePayload=>DisposableStackEntryHeapSlotMetadata{",
            "record:\"disposable-stack-entry\",name:\"value_payload\",",
        ...
        concat!(
            ...
            "record:\"disposable-stack-entry\",name:\"method_tag\",",
            "offset:HEAP_DISPOSABLE_STACK_ENTRY_METHOD_TAG_OFFSET,width:8,pointer:false,},"
        ),
        concat!(
            "Self::MethodPayload=>DisposableStackEntryHeapSlotMetadata{",
            "record:\"disposable-stack-entry\",name:\"method_payload\",",
...
fn typed_registry_preserves_kind_and_tag_payload_pair_order() {
    ...
    assert_eq!(
        registry,
        concat!(
            ":&[DisposableStackEntryHeapSlot]=&[",
            "DisposableStackEntryHeapSlot::Kind,DisposableStackEntryHeapSlot::ValueTag,",
...
#[test]
fn disposable_stack_entry_layout_has_one_private_recursive_owner() {
    assert_eq!(
        LIB_SOURCE
    ...
    assert!(!LIB_SOURCE.contains("\npub mod heap_disposable_stack_entry_layout;\n"));
    assert!(!HEAP_SOURCE.contains("record: \"disposable-stack-entry\""));

    let source_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("src");
    assert_eq!(
        recursive_rust_source_count(&source_root, "record: \"disposable-stack-entry\""),
        5
    );
    assert_eq!(
    ...
    assert!(CONTRACT.contains("DisposableStackEntryHeapSlot"));
    assert!(TASK.contains("disposable-stack-entry-heap-slot-authority.md"));
}
