...
fn lowering_selects_and_allocates_the_owner_before_any_resource_initializer() {
    ...
    for marker in [
        ...
        "using declaration in an async function or async generator",
    ] {
...
fn backend_exhaustively_selects_local_or_activation_backed_capability_storage() {
    ...
    for marker in [
        ...
        "a detached generator DisposeCapability must be consumed exactly once",
    ] {
    ...
    assert!(
        !CONTROL_FLOW_SOURCE.contains("impl Copy for PlainGeneratorSyncDisposeCapabilityStorage")
    );
    assert!(!CONTROL_FLOW_SOURCE
        .contains("impl Copy for ActivePlainGeneratorSyncDisposeCapabilityLocals"));
    assert!(!CONTROL_FLOW_SOURCE
        .contains("impl Copy for DetachedPlainGeneratorSyncDisposeCapabilityLocals"));
    ...
    assert!(dispatch.contains("compile_plain_generator_sync_disposable_scope("));
...
fn generator_scope_publishes_once_retains_through_body_then_detaches_and_disposes_once() {
    ...
    for marker in [
        ...
        "generator_statement_entry_state",
        "generator_statement_exit_state",
        "emit_state_in_inclusive_range_i32(",
        "HEAP_GENERATOR_RESUME_STATE_OFFSET",
        "initialize_plain_generator_sync_dispose_capability(",
        "compile_generator_block_contents(body, entry_state, true, function)",
        "detach_plain_generator_sync_dispose_capability(",
        "load_detached_plain_generator_sync_disposable_resources(",
        ...
        "release_detached_plain_generator_sync_dispose_capability(detached)",
    ] {
        assert!(
    ...
    assert!(initialize.contains("storage: &PlainGeneratorSyncDisposeCapabilityStorage"));
    ...
    assert_before(
        initialize,
        "self.acquire_sync_disposable_resource_from_locals(",
        "self.append_plain_generator_sync_disposable_resource(",
    );
    assert_before(
        initialize,
        "self.append_plain_generator_sync_disposable_resource(",
    ...
    assert!(detach.contains("storage: PlainGeneratorSyncDisposeCapabilityStorage"));
    assert_before(
        detach,
...
fn planner_derives_nonoverlapping_active_body_and_detached_disposal_peaks() {
    let constants = bounded(
        PLANNING_SOURCE,
        "const PLAIN_GENERATOR_SYNC_DISPOSE_DETACHED_TEMP_LOCALS",
        "const SUPER_PROPERTY_MUTATION_PERSISTENT_TEMP_LOCALS",
    );
    assert!(constants.contains("usize = 5"));
    assert!(constants.contains("PLAIN_GENERATOR_SYNC_DISPOSE_ACTIVE_TEMP_LOCALS: usize = 3 + 5"));
