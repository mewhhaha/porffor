...
const LAYOUT_SOURCE: &str = include_str!("../src/heap_set_iterator_layout.rs");
...
#[test]
fn set_iterator_heap_slot_is_the_exact_capability_free_domain() {
    ...
    assert_eq!(
        declaration
            ...
            .collect::<Vec<_>>(),
        ["SetPayload,", "NextIndex,", "Kind,", "CursorState,"],
    );
    assert!(!LAYOUT_SOURCE.contains("#[derive"));
    assert!(
        !LAYOUT_SOURCE.lines().any(|line| {
            line.trim_start().starts_with("impl ") && line.contains(" for SetIteratorHeapSlot")
        }),
    ...
    for capability in [
        ...
        assert!(
            !LAYOUT_SOURCE.contains(&format!("impl {capability} for SetIteratorHeapSlot")),
...
fn one_exhaustive_projection_owns_four_exact_rows() {
    let implementation = bounded(
        ...
        "pub(crate) const HEAP_SET_ITERATOR_RECORD_LAYOUT",
    );
    ...
    for row in [
        concat!(
            "Self::SetPayload=>SetIteratorHeapSlotMetadata{record:\"set-iterator-record\",",
            "name:\"set_payload\",offset:HEAP_SET_ITERATOR_SET_PAYLOAD_OFFSET,",
            "width:8,pointer:true,},"
        ),
        concat!(
            "Self::NextIndex=>SetIteratorHeapSlotMetadata{record:\"set-iterator-record\",",
            "name:\"next_index\",offset:HEAP_SET_ITERATOR_NEXT_INDEX_OFFSET,",
            "width:8,pointer:false,},"
        ),
        concat!(
            "Self::Kind=>SetIteratorHeapSlotMetadata{record:\"set-iterator-record\",",
            "name:\"kind\",offset:HEAP_SET_ITERATOR_KIND_OFFSET,",
            "width:8,pointer:false,},"
        ),
        concat!(
            "Self::CursorState=>SetIteratorHeapSlotMetadata{record:\"set-iterator-record\",",
            "name:\"cursor_state\",offset:HEAP_SET_ITERATOR_CURSOR_STATE_OFFSET,",
            "width:8,pointer:false,},"
        ),
    ] {
...
fn typed_registry_preserves_payload_index_kind_cursor_order() {
    ...
    assert_eq!(
        ...
        concat!(
            ":&[SetIteratorHeapSlot]=&[",
            "SetIteratorHeapSlot::SetPayload,SetIteratorHeapSlot::NextIndex,",
            "SetIteratorHeapSlot::Kind,SetIteratorHeapSlot::CursorState,"
        )
    );
}

#[test]
fn set_iterator_layout_has_one_private_recursive_owner() {
    assert_eq!(
        LIB_SOURCE
    ...
    assert!(!LIB_SOURCE.contains("\npub mod heap_set_iterator_layout;\n"));
    assert!(!HEAP_SOURCE.contains("record: \"set-iterator-record\""));

    let source_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("src");
    assert_eq!(
        recursive_rust_source_count(&source_root, "record: \"set-iterator-record\""),
        4
    );
    assert_eq!(
        recursive_rust_source_count(&source_root, "pub(crate) enum SetIteratorHeapSlot {"),
        1
    );
