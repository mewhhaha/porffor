...
const ARGUMENT_FIXTURE: &str =
    include_str!("../../lila-cli/tests/fixtures/wasm_array_some_argument_evaluation.js");
...
#[test]
fn array_some_arm_delegates_with_complete_arguments_without_changing_iterator_dispatch() {
    let direct = bounded(
        FUNCTIONS_SOURCE,
    ...
    assert_eq!(
        array_arm
            .matches("StandardBuiltinId::ArrayPrototypeSome,")
            .count(),
        1
    );
    assert_eq!(array_arm.matches("\"Array.prototype.some\",").count(), 1);
    assert_eq!(array_arm.matches("                    args,").count(), 1);

    for forbidden in [
        "emit_array_some_method_call(",
        "for arg in args",
        ...
        assert!(
            ...
            "Array some arm must not retain `{forbidden}`"
        );
    }
}

#[test]
fn removed_direct_some_owner_cannot_be_called() {
    ...
    assert_eq!(
        rust_source.matches("emit_array_some_method_call").count(),
        0
    );
    assert_eq!(
        rust_source
            .matches("fn compile_array_prototype_some_builtin(")
            .count(),
        1
    );
    ...
    assert_eq!(
        standard_arm
            .matches("self.compile_array_prototype_some_builtin(function)?;")
            .count(),
        1
    );
}
...
fn shared_call_boundary_and_canonical_compiler_own_argument_and_iteration_order() {
    ...
    assert!(canonical.contains("ArrayCallbackIterationKind::Some"));
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "shared some loop must not duplicate {forbidden}"
        );
    }
}
...
fn focused_fixture_observes_all_arguments_before_iteration() {
    ...
    assert!(ARRAY_CLI_TESTS
        .contains("fn run_wasm_backend_evaluates_all_array_some_arguments_before_iteration()"));
    assert!(ARRAY_CLI_TESTS
        .contains("fn run_wasm_backend_succeeds_for_supported_array_some_core_fixture()"));
}
