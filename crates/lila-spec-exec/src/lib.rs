...
impl HostSession {
    ...
    fn start_agent(self: &Arc<Self>, source: String) -> Result<(), ExecutionError> {
        ...
        let join = thread::Builder::new()
            .name("lila-test262-agent".to_string())
...
(function () {
  ...
  const stateKey = "__lilaDateTimeFormatState";
  const boundFormatKey = "__lilaBoundFormat";
  ...
  function getOptions(locales, options) {
    ...
    if (
      ...
      state.month = "numeric";
      state.day = "numeric";
      state.__lilaDefaultDateOnly = true;
    }
  ...
  function effectiveStateForValue(state, record) {
    ...
    if (record && state.__lilaDefaultDateOnly === true) {
  ...
  function formatTimePiece(state, hour, minute, second) {
    ...
    if (state.fractionalSecondDigits !== undefined) {
      const ms = state.__lilaMillisecond ?? 0;
  ...
  function formatValue(state, value) {
    ...
    effectiveState.__lilaMillisecond = record ? (record.millisecond ?? 0) : date.getMilliseconds();
    if (
    ...
    if (usesTimeFields(effectiveState)) {
      ...
      if (effectiveState.fractionalSecondDigits !== undefined) {
        ...
        parts.push({
          ...
          value: String(effectiveState.__lilaMillisecond || 0).padStart(3, "0").slice(0, effectiveState.fractionalSecondDigits)
        });
      }
      if (useTwelveHour) {
        parts.push({ type: "literal", value: " " });
...
fn install_host_globals(context: &mut Context, argv: &[String]) -> Result<(), ExecutionError> {
    ...
    context
        .register_global_builtin_callable(
            js_string!("__lilaDetachArrayBuffer"),
            1,
    ...
    context
        .register_global_builtin_callable(
            js_string!("__lilaAgentBroadcast"),
            1,
    ...
    context
        .register_global_builtin_callable(
            js_string!("__lilaAgentReceiveBroadcast"),
            1,
    ...
    context
        .register_global_builtin_callable(
            js_string!("__lilaAgentGetReport"),
            0,
    ...
    context
        .register_global_builtin_callable(
            js_string!("__lilaAgentMonotonicNow"),
            0,
    ...
    let bootstrap = format!(
        r#"
globalThis.__lilaArgv = {argv_literal};
globalThis.$262 = {{
  global: globalThis,
  gc: gc,
  getGlobal(name) {{
  ...
  agent: {{
    start() {{
      return __lilaAgentStart.apply(this, arguments);
    }},
    broadcast() {{
      return __lilaAgentBroadcast.apply(this, arguments);
    }},
    receiveBroadcast() {{
      return __lilaAgentReceiveBroadcast.apply(this, arguments);
    }},
    report() {{
      return __lilaAgentReport.apply(this, arguments);
    }},
    getReport() {{
      return __lilaAgentGetReport.apply(this, arguments);
    }},
    sleep() {{
      return __lilaAgentSleep.apply(this, arguments);
    }},
    monotonicNow() {{
      return __lilaAgentMonotonicNow.apply(this, arguments);
    }},
    leaving() {{
      return __lilaAgentLeaving.apply(this, arguments);
    }}
  }}
}};
"#,
...
fn check_test262_async_done(context: &mut Context) -> Result<(), ExecutionError> {
    let state = context
        .global_object()
        .get(js_string!("__lilaTest262AsyncDone"), context)
        .map_err(|err| format_js_error(err, context))?;
...
mod tests {
    ...
    fn resolves_sibling_fixture_module_from_test_path() {
        let root =
            std::env::temp_dir().join(format!("lila-spec-exec-modules-{}", std::process::id()));
