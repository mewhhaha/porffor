...
//! measured a single process — `porf test262 run language/wasm/pass
...
//! suite falls back to spawning a cold `porf` child per test.
//!
...
//!   `available_parallelism().min(4)` (`porffor-test262/src/lib.rs:233`) = **4**
...
//!   `None`; only `PORFFOR_TEST262_FORCE_CASE_RUNNER=1` turns it on).
