    ...
    pub(crate) fn emit_array_from_async(
        ...
        let constructor_payload_local = self.this_payload_local.ok_or_else(|| {
            EmitError::unsupported(
                "unsupported in porffor wasm-aot first slice: missing Array.fromAsync receiver",
            )
        })?;
        let constructor_tag_local = self.this_tag_local.ok_or_else(|| {
            EmitError::unsupported(
                "unsupported in porffor wasm-aot first slice: missing Array.fromAsync receiver tag",
            )
        })?;
