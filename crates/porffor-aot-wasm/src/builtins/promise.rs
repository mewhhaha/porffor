...
const PROMISE_REACTION_JOB_KIND: u64 = 1;
const PROMISE_RESOLVE_THENABLE_JOB_KIND: u64 = 2;
const PROMISE_REACTION_CALLBACK_DEFAULT: u64 = 0;
const PROMISE_REACTION_CALLBACK_ASYNC_FUNCTION: u64 = 1;
const PROMISE_REACTION_CALLBACK_ASYNC_GENERATOR_AWAIT_RETURN: u64 = 2;
const PROMISE_REACTION_CALLBACK_ASYNC_GENERATOR_AWAIT: u64 = 3;
const PROMISE_REACTION_CALLBACK_ASYNC_GENERATOR_YIELD: u64 = 4;
const PROMISE_REACTION_CALLBACK_ASYNC_GENERATOR_YIELD_RETURN: u64 = 5;
...
impl AsyncAwaitContinuation {
    fn reaction_callback_kind(self) -> u64 {
        match self {
            Self::AsyncFunction => PROMISE_REACTION_CALLBACK_ASYNC_FUNCTION,
            Self::AsyncGeneratorBody => PROMISE_REACTION_CALLBACK_ASYNC_GENERATOR_AWAIT,
            Self::AsyncGeneratorYield => PROMISE_REACTION_CALLBACK_ASYNC_GENERATOR_YIELD,
            Self::AsyncGeneratorYieldReturn => {
                PROMISE_REACTION_CALLBACK_ASYNC_GENERATOR_YIELD_RETURN
            }
        }
    }
}
impl<'a> FunctionBuilder<'a> {
    ...
    fn emit_enqueue_promise_thenable_job(
        ...
        self.store_i64_const_at_offset(
            job_record_local,
            HEAP_PENDING_JOB_KIND_OFFSET,
            PROMISE_RESOLVE_THENABLE_JOB_KIND,
            function,
        );
        ...
        let executor_meta = self
            ...
            .ok_or_else(|| {
                EmitError::unsupported(
                    "unsupported in porffor wasm-aot first slice: missing Promise capability executor builtin",
                )
            })?;
    ...
    fn emit_initialize_promise_reaction(
        ...
        self.store_i64_const_at_offset(
            ...
            PROMISE_REACTION_CALLBACK_DEFAULT,
            function,
        );
        self.store_i64_const_at_offset(
            reaction_record_local,
    ...
    pub(crate) fn emit_async_generator_await_return_reactions(
        ...
        self.store_i64_const_at_offset(
            fulfill_reaction_local,
            HEAP_PROMISE_REACTION_CALLBACK_KIND_OFFSET,
            PROMISE_REACTION_CALLBACK_ASYNC_GENERATOR_AWAIT_RETURN,
            function,
        );
        self.emit_initialize_promise_reaction(
            reject_reaction_local,
    ...
    pub(crate) fn emit_promise_prototype_then(
        ...
        let receiver_payload_local = self.this_payload_local.ok_or_else(|| {
            EmitError::unsupported(
                "unsupported in porffor wasm-aot first slice: missing Promise.prototype.then receiver",
            )
        })?;
        let receiver_tag_local = self.this_tag_local.ok_or_else(|| {
            EmitError::unsupported(
                "unsupported in porffor wasm-aot first slice: missing Promise.prototype.then receiver tag",
            )
        })?;
    ...
    pub(crate) fn emit_promise_prototype_catch(
        ...
        let receiver_payload_local = self.this_payload_local.ok_or_else(|| {
            EmitError::unsupported(
                "unsupported in porffor wasm-aot first slice: missing Promise.prototype.catch receiver",
            )
        })?;
        let receiver_tag_local = self.this_tag_local.ok_or_else(|| {
            EmitError::unsupported(
                "unsupported in porffor wasm-aot first slice: missing Promise.prototype.catch receiver tag",
            )
        })?;
    ...
    pub(crate) fn emit_promise_prototype_finally(
        ...
        let receiver_payload_local = self.this_payload_local.ok_or_else(|| {
            EmitError::unsupported(
                "unsupported in porffor wasm-aot first slice: missing Promise.prototype.finally receiver",
            )
        })?;
        let receiver_tag_local = self.this_tag_local.ok_or_else(|| {
            EmitError::unsupported(
                "unsupported in porffor wasm-aot first slice: missing Promise.prototype.finally receiver tag",
            )
        })?;
    ...
    fn emit_run_promise_reaction_job(
        ...
        function.instruction(&Instruction::I64Const(
            PROMISE_REACTION_CALLBACK_DEFAULT as i64,
        ));
        function.instruction(&Instruction::I64Eq);
        function.instruction(&Instruction::If(BlockType::Empty));
        ...
        function.instruction(&Instruction::LocalGet(callback_kind_local));
        function.instruction(&Instruction::I64Const(
            PROMISE_REACTION_CALLBACK_ASYNC_FUNCTION as i64,
        ));
        function.instruction(&Instruction::I64Eq);
        function.instruction(&Instruction::If(BlockType::Empty));
        self.emit_run_async_continuation_job(
            reaction_record_local,
            argument_payload_local,
            argument_tag_local,
            function,
        ...
        function.instruction(&Instruction::LocalGet(callback_kind_local));
        function.instruction(&Instruction::I64Const(
            PROMISE_REACTION_CALLBACK_ASYNC_GENERATOR_AWAIT_RETURN as i64,
        ));
        function.instruction(&Instruction::I64Eq);
        function.instruction(&Instruction::If(BlockType::Empty));
        self.emit_run_async_generator_await_return_job(
            reaction_record_local,
            argument_payload_local,
            argument_tag_local,
            function,
        ...
        function.instruction(&Instruction::LocalGet(callback_kind_local));
        function.instruction(&Instruction::I64Const(
            PROMISE_REACTION_CALLBACK_ASYNC_GENERATOR_AWAIT as i64,
        ));
        function.instruction(&Instruction::I64Eq);
        function.instruction(&Instruction::If(BlockType::Empty));
        self.emit_run_async_generator_await_job(
            reaction_record_local,
            argument_payload_local,
            argument_tag_local,
            function,
        ...
        function.instruction(&Instruction::LocalGet(callback_kind_local));
        function.instruction(&Instruction::I64Const(
            PROMISE_REACTION_CALLBACK_ASYNC_GENERATOR_YIELD as i64,
        ));
        function.instruction(&Instruction::I64Eq);
        function.instruction(&Instruction::If(BlockType::Empty));
        self.emit_run_async_generator_yield_job(
            reaction_record_local,
            argument_payload_local,
            argument_tag_local,
            function,
        ...
        function.instruction(&Instruction::LocalGet(callback_kind_local));
        function.instruction(&Instruction::I64Const(
            PROMISE_REACTION_CALLBACK_ASYNC_GENERATOR_YIELD_RETURN as i64,
        ));
        function.instruction(&Instruction::I64Eq);
        function.instruction(&Instruction::If(BlockType::Empty));
        self.emit_run_async_generator_yield_return_job(
            reaction_record_local,
            argument_payload_local,
            argument_tag_local,
            function,
    ...
    pub(crate) fn emit_drain_promise_jobs(
        ...
        function.instruction(&Instruction::I64Const(PROMISE_REACTION_JOB_KIND as i64));
        function.instruction(&Instruction::I64Eq);
        function.instruction(&Instruction::If(BlockType::Empty));
        self.emit_run_promise_reaction_job(
            reaction_record_local,
            argument_payload_local,
            argument_tag_local,
            function,
    ...
    pub(crate) fn emit_promise_static_settle(
        ...
        let constructor_payload_local = self.this_payload_local.ok_or_else(|| {
            EmitError::unsupported(
                "unsupported in porffor wasm-aot first slice: missing Promise static method receiver",
            )
        })?;
        let constructor_tag_local = self.this_tag_local.ok_or_else(|| {
            EmitError::unsupported(
                "unsupported in porffor wasm-aot first slice: missing Promise static method receiver tag",
            )
        })?;
    ...
    pub(crate) fn emit_promise_with_resolvers(
        ...
        let constructor_payload_local = self.this_payload_local.ok_or_else(|| {
            EmitError::unsupported(
                "unsupported in porffor wasm-aot first slice: missing Promise.withResolvers receiver",
            )
        })?;
        let constructor_tag_local = self.this_tag_local.ok_or_else(|| {
            EmitError::unsupported(
                "unsupported in porffor wasm-aot first slice: missing Promise.withResolvers receiver tag",
            )
        })?;
        let capability_record_local = self.reserve_temp_local();
        let promise_payload_local = self.reserve_temp_local();
    ...
    pub(crate) fn emit_promise_try(&mut self, function: &mut Function) -> Result<(), EmitError> {
        let constructor_payload_local = self.this_payload_local.ok_or_else(|| {
            EmitError::unsupported(
                "unsupported in porffor wasm-aot first slice: missing Promise.try receiver",
            )
        })?;
        let constructor_tag_local = self.this_tag_local.ok_or_else(|| {
            EmitError::unsupported(
                "unsupported in porffor wasm-aot first slice: missing Promise.try receiver tag",
            )
        })?;
        let capability_record_local = self.reserve_temp_local();
        let promise_payload_local = self.reserve_temp_local();
    ...
    pub(crate) fn emit_promise_race(&mut self, function: &mut Function) -> Result<(), EmitError> {
        let constructor_payload_local = self.this_payload_local.ok_or_else(|| {
            EmitError::unsupported(
                "unsupported in porffor wasm-aot first slice: missing Promise.race receiver",
            )
        })?;
        let constructor_tag_local = self.this_tag_local.ok_or_else(|| {
            EmitError::unsupported(
                "unsupported in porffor wasm-aot first slice: missing Promise.race receiver tag",
            )
        })?;
        let capability_record_local = self.reserve_temp_local();
        let promise_payload_local = self.reserve_temp_local();
    ...
    fn emit_promise_keyed(
        ...
        let constructor_payload_local = self.this_payload_local.ok_or_else(|| {
            EmitError::unsupported(format!(
                "unsupported in porffor wasm-aot first slice: missing {builtin_name} receiver"
            ))
        })?;
        let constructor_tag_local = self.this_tag_local.ok_or_else(|| {
            EmitError::unsupported(format!(
                "unsupported in porffor wasm-aot first slice: missing {builtin_name} receiver tag"
            ))
        })?;
        let capability_record_local = self.reserve_temp_local();
        let promise_payload_local = self.reserve_temp_local();
