...
//!
//! The whole slice rests on one routine, [`FunctionBuilder::emit_intl_canonicalize_locale_tag`],
//! which implements *structural* `CanonicalizeUnicodeLocaleId`: it validates a
//! string against the `unicode_locale_id` grammar of UTS 35 and re-cases and
//! re-orders it into canonical form (language lowercase, script titlecase,
//! region uppercase, variants lowercase and sorted, extension sequences sorted
//! by singleton with `x-` last).
//!
//! What is deliberately **absent** rather than stubbed: CLDR alias replacement
//! (`iw` -> `he`, grandfathered tags, deprecated regions) and the internal
//! canonicalisation of `u-` keywords and `t-` transforms. Locale-sensitive
//! formatting (`Intl.NumberFormat`, `Intl.DateTimeFormat`, `Intl.Collator`,
//! ...) is not present at all. Everything absent stays absent so a caller sees
//! a missing feature instead of a wrong answer.
//!
...
//! is ASCII, so every pass below is a plain byte loop. Canonicalisation never
//! changes a tag's length — it only re-cases bytes and permutes whole subtags —
//! so the output buffer is allocated at exactly the input length.
