...
mod tests {
    ...
    #[test]
    fn class_field_initializer_super_call_module_parse_maps_to_an_early_syntax_error() {
        let source = "export default class { static #field = () => super(); }";
        let error = lila_front::parse(source, lila_front::ParseOptions::module())
        ...
        let span = diagnostic
            .span
            .expect("the class field rejection must retain its source span");
        assert!(span.start < span.end, "{source:?}: {diagnostic:?}");
    }
    ...
    #[test]
    fn function_declaration_super_module_parse_maps_to_an_early_syntax_error() {
        ...
        let span = diagnostic
            .span
            .expect("the FunctionDeclaration rejection must retain its source span");
        assert!(span.start < span.end, "{source:?}: {diagnostic:?}");
    }
    ...
    fn async_function_expression_super_module_parse_maps_to_an_early_syntax_error() {
        ...
        let span = diagnostic
            .span
            .expect("the AsyncFunctionExpression rejection must retain its source span");
        assert!(span.start < span.end, "{source:?}: {diagnostic:?}");
    }

    #[test]
    fn generator_expression_super_module_parse_maps_to_an_early_syntax_error() {
        let source = "export const invalid = function*(value = super.value) {};";
        let error = lila_front::parse(source, lila_front::ParseOptions::module())
        ...
        let span = diagnostic
            .span
            .expect("the GeneratorExpression rejection must retain its source span");
        assert!(span.start < span.end, "{source:?}: {diagnostic:?}");
    }
