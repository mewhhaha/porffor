...
pub use modules::{
    ...
    source_writes_dynamic_import, DynamicComponentIr, DynamicImportAttributesIr,
...
mod tests {
    ...
    fn operations_lowers_host_is_constructor_call_to_spec_operation() {
        let program =
            lower_test262_script("let value = function C() {}; __lilaIsConstructor(value);");
        assert!(program.is_wasm_supported());
  ...
  ["Symbol.iterator"]: 3
    ...
    fn registers_zero_suspension_generator_declarations() {
        ...
        let generators = script
            ...
            .iter()
            .filter(|function| {
                function.protocol.execution_kind() == FunctionExecutionKind::Generator
            })
            .collect::<Vec<_>>();
    ...
    fn global_binding_plan_resolves_predefined_var_and_duplicate_function_collisions_once() {
        ...
        let GlobalPropertyInitializerIr::SourceFunction(function_id) = &duplicate.initializer
        else {
