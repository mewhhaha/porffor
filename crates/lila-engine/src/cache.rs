...
/// own stencil/target/flags rather than by `LILA_COMPILER_FINGERPRINT`, so
...
const CACHE_LIMIT_ENV: &str = "LILA_CACHE_LIMIT_BYTES";
...
const FUNCTION_CACHE_LIMIT_ENV: &str = "LILA_FUNCTION_CACHE_LIMIT_BYTES";
const MODULE_CACHE_LIMIT_ENV: &str = "LILA_MODULE_CACHE_LIMIT_BYTES";
const PROGRAM_CACHE_LIMIT_ENV: &str = "LILA_PROGRAM_CACHE_LIMIT_BYTES";
...
pub struct CachePruneReport {
    pub lila_bytes_removed: u64,
    pub lila_files_removed: u64,
...
pub(crate) fn function_cache_directory() -> PathBuf {
    lila_cache_root().join(FUNCTION_CACHE_FORMAT)
}

pub(crate) fn module_cache_directory() -> PathBuf {
    lila_cache_root().join(MODULE_CACHE_DIR)
}

pub(crate) fn program_cache_directory() -> PathBuf {
    lila_cache_root().join(PROGRAM_CACHE_DIR)
}
...
fn module_cache_at(directory: PathBuf, limit_bytes: u64) -> io::Result<Cache> {
    ...
    // same bound synchronously whenever a process opens Lila's cache.
...
pub fn prune_caches(include_legacy_wasmtime: bool) -> io::Result<CachePruneReport> {
    ...
    Ok(CachePruneReport {
        lila_bytes_removed: function
            .0
            .saturating_add(module.0)
            .saturating_add(program.0),
        lila_files_removed: function
            .1
...
fn lila_cache_root() -> PathBuf {
    if let Some(path) = std::env::var_os("LILA_CACHE_DIR") {
        return PathBuf::from(path);
    }
    platform_cache_root().join("lila")
}
