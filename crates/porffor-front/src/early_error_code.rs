...
//!   see [`crate::ParseCode`] and `porffor_ir::IrDiagnosticKind`.
...
//!
//! That last point is why this module lives in `porffor-front` rather than in
//! `porffor-ir`. Both crates call boa's parser independently — [`crate::parse`]
//! for an entry file, `porffor_ir::modules::record::reparse_module` for every
//! dependency module — and `porffor-ir` depends on `porffor-front`, so the one
//! table has to sit in the lower crate for both to name it. Before this module
//! existed there were two tables, and they had measurably drifted: a duplicate
...
//! `porffor_ir::NativeErrorKind`): `Display`, `AsRef<str>`,
...
/// shipped here before. `porffor_ir::native_error` keeps its own private
...
macro_rules! early_error_codes {
    (
        ...
        impl EarlyErrorCode {
            ...
            /// injective. `porffor_ir::NativeErrorKind::from_str` is `pub`
...
impl EarlyErrorCode {
    ...
    ///
    /// `pub` because its consumer is `porffor_ir::early_error_code`'s assertion
...
/// dependency parse failure classify as one code, because the prefix is
/// prepended to all of them.
const fn reparse_prefix_matches_no_rule() -> bool {
    match classify_parse_failure(MODULE_REPARSE_PREFIX) {
        Some(_) => false,
        None => true,
    }
}
